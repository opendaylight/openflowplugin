...
public class SwitchConnectionProviderImpl implements SwitchConnectionProvider, ConnectionInitializer {
    ...
    private static final String OPENFLOW_JAVA_SERVICE_NAME = "OPENFLOW_SERVER";
    ...
    public SwitchConnectionProviderImpl(
        ...
        this.diagStatusIdentifier = createConnectionID(connConfig);
    ...
    private static String createConnectionID(@Nullable ConnectionConfiguration config) {
        if (config != null && config.getAddress() != null) {
            return OPENFLOW_JAVA_SERVICE_NAME + "-" + config.getAddress().toString() + ":" + config.getPort();
        } else if (config != null) {
            return OPENFLOW_JAVA_SERVICE_NAME + ":" + config.getPort();
        } else {
            return OPENFLOW_JAVA_SERVICE_NAME + "-null-config";
        }
    }
            ...
            Futures.addCallback(listeningExecutorService.submit(serverFacade), new FutureCallback<Object>() {
                ...
                public void onSuccess(@Nullable Object nullResult) {
                    diagStatusService.report(new ServiceDescriptor(
                            diagStatusIdentifier, ServiceState.ERROR, THREAD_NAME + " terminated"));
                }
