...
public class OfToSalSetFieldCase extends ConvertorCase<SetFieldCase, Action, ActionResponseConverterData> {
    ...
    @Override
    public Optional<Action> process(@Nonnull final SetFieldCase source, final ActionResponseConverterData data, ConverterExecutor converterExecutor) {
        final VersionDatapathIdConverterData datapathIdConvertorData = new VersionDatapathIdConverterData(data.getVersion());
        final SetFieldAction setFieldAction = source.getSetFieldAction();
        final SetFieldBuilder setField = new SetFieldBuilder();
        final Optional<MatchBuilder> matchOptional = converterExecutor.convert(setFieldAction, datapathIdConvertorData);
