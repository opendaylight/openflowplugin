...
public final class MatchExtensionHelper {
    ...
    AugmentTuple<EXT_POINT> processAllExtensions(Collection<MatchEntries> matchEntries,
        ...
        for (MatchEntries matchEntry : matchEntries) {
            Extension extension = processExtension(matchEntry, ofVersion, matchPath);
            ExtensionListBuilder extListBld = new ExtensionListBuilder();
            extListBld.setExtension(extension);
            extensionsList.add(extListBld.build());
        }
     ...
     * @return 
     */
    private static Extension processExtension(MatchEntries matchEntry, OpenflowVersion ofVersion, MatchPath matchPath) {
