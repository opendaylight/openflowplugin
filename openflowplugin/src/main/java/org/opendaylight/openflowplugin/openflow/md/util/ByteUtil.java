...
public abstract class ByteUtil {
    ...
    public static final long bytesToUnsignedInt(final byte[] bytes) {
        if (bytes.length != 4){
            throw new IllegalStateException("Input byte array must be exactly four bytes long.");
        }
        long unsignedInt = 0;
    ...
    public static final int bytesToUnsignedShort(final byte[] bytes) {
        if (bytes.length != 2) {
            throw new IllegalStateException("Input byte array must be exactly two bytes long.");
        }
        int unsignedShort = 0;
