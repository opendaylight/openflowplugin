...
public class MatchConvertorImpl implements MatchConvertor<List<MatchEntries>> {
    ...
    public List<MatchEntries> convert(
        ...
        if (match.getMatchExtensions() != null) {
            for (MatchExtensions matchExtensions : match.getMatchExtensions()) {
                ConverterExtensionKey<? extends MatchExtensions> key = new ConverterExtensionKey<>(matchExtensions.getClass());
                ConvertorToOFJava<MatchExtensions, MatchEntries> convertor = OFSessionUtil.getExtensionConvertorProvider().getConverter(key);
                MatchEntries ofMatch = convertor.convert(matchExtensions, null);
