...
public class MatchConvertorImpl implements MatchConvertor<List<MatchEntry>> {
            ...
            if (ofMatch.getOxmMatchField().equals(InPort.class)) {
                ...
                if (ipv4Address != null) {
                    ...
                    if (mask != null && IpConversionUtil.isArbitraryBitMask(mask)) {
                        // case where ipv4dst is of type ipv4MatchBuilder and ipv4src is of type ipv4MatchArbitrary. Needs to convert ipv4dst to ipv4MatchArbitrary.
                    ...
                    // case where destination is of type ipv4MatchArbitraryBitMask already exists in Layer3Match, source which of type ipv4Match needs to be converted to ipv4MatchArbitraryBitMask
                ...
                if (ipv4Address != null) {
                    byte[] mask = ipv4Address.getMask();
                    if(mask != null && IpConversionUtil.isArbitraryBitMask(mask)) {
                        // case where ipv4src is of type ipv4MatchBuilder and ipv4dst is of type ipv4MatchArbitrary. Needs to convert ipv4src to ipv4MatchArbitrary.
                    ...
                    // case where source is of type ipv4MatchArbitraryBitMask already exists in Layer3Match, destination which of type ipv4Match needs to be converted to ipv4MatchArbitraryBitMask
    ...
    private static void setIpv4MatchArbitraryBitMaskBuilderFields(
            final Ipv4MatchArbitraryBitMaskBuilder ipv4MatchArbitraryBitMaskBuilder,
            final MatchEntry ofMatch, final DottedQuad mask, final String ipv4AddressStr, final boolean inverseCase) {
        /*
        Default value of inverseCase is false, This field is set to true when one of the expected output
        is in ipv4arbitrarybitmask format and other is in cidr format.
        output :-
