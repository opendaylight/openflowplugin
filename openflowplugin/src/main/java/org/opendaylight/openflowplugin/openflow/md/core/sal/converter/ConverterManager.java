 ...
 */
public class ConverterManager implements ConverterExecutor, ConverterRegistration {
    ...
    private Map<Short, Map<Class<? extends DataContainer>, Converter<?, ?, ? extends ConverterData>>> converters;

    /**
     * Create new instance of Converter Manager
     * @param supportedVersions supported versions
     */
    public ConverterManager(final Short... supportedVersions) {
        ...
        if (supportedVersions.length == 1) {
            ...
            versionOptional.ifPresent(version -> converters = Collections.singletonMap(version, new ConcurrentHashMap<>()));
        } else {
    ...
    @Override
    public ConverterManager registerConverter(final short version, final Converter<?, ?, ? extends ConverterData> converter) {
        final Map<Class<? extends DataContainer>, Converter<?, ?, ? extends ConverterData>> convertersForVersion =
    ...
    public <FROM extends DataContainer, TO, DATA extends ConverterData> Optional<TO> convert(final FROM source, final DATA data) {
        ...
        if (Objects.isNull(type)) {
         ...
         return findConverter(data.getVersion(), type)
     ...
     */
    @VisibleForTesting
    Optional<Converter> findConverter(final short version, final Class<? extends DataContainer> type) {
        final Map<Class<? extends DataContainer>, Converter<?, ?, ? extends ConverterData>> convertersForVersion =
                converters.get(version);
        ...
        if (Objects.nonNull(convertersForVersion)) {
            ...
            if (!converter.isPresent()) {
                for (final Class<? extends DataContainer> converterType : convertersForVersion.keySet()) {
