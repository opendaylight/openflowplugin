...
public abstract class QueueKeeperFactory {
    ...
    public static QueueKeeper<OfHeader> createFairQueueKeeper(
            MessageSourcePollRegistrator<QueueKeeper<OfHeader>> sourceRegistry, int capacity) {
        QueueKeeperFairImpl queueKeeper = new QueueKeeperFairImpl();
        queueKeeper.setCapacity(capacity);
        queueKeeper.setHarvestLock(sourceRegistry.getHarvestLock());
        queueKeeper.init();
        
        AutoCloseable registration = sourceRegistry.registerMessageSource(queueKeeper);
