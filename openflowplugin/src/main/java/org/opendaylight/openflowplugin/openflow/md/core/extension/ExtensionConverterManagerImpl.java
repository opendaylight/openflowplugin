...
public class ExtensionConverterManagerImpl implements ExtensionConverterManager {
    ...
    @Override
    public <FROM extends DataContainer, TO extends DataContainer, PATH extends AugmentationPath> ConvertorToOFJava<FROM, TO, PATH> getConverter(
            ConverterExtensionKey<?> key) {
        return (ConvertorToOFJava<FROM, TO, PATH>) registryToOFJAva.get(key);
    }

    @SuppressWarnings("unchecked")
    @Override
    public <FROM extends DataContainer, TO extends DataContainer, PATH extends AugmentationPath> ConvertorFromOFJava<FROM, TO, PATH> getConverter(
            MessageTypeKey<?> key) {
        return (ConvertorFromOFJava<FROM, TO, PATH>) registryFromOFJAva.get(key);
    }

    @Override
    public AutoCloseable registerActionConvertor(
            ConverterExtensionKey<? extends ExtensionKey> key,
            ConvertorToOFJava<Action, org.opendaylight.yang.gen.v1.urn.opendaylight.openflow.common.action.rev130731.actions.grouping.Action, ActionPath> convertor) {
    ...
    public AutoCloseable registerActionConvertor(
            ...
            ConvertorFromOFJava<org.opendaylight.yang.gen.v1.urn.opendaylight.openflow.common.action.rev130731.actions.grouping.Action, Action, ActionPath> convertor) {
    ...
    public AutoCloseable registerMatchConvertor(ConverterExtensionKey<? extends ExtensionKey> key,
            ConvertorToOFJava<Match, MatchEntries, MatchPath> convertor) {
        registryToOFJAva.put(key, convertor);
        return hireJanitor(key, convertor);
    }
    ...
    public AutoCloseable registerMatchConvertor(
            ...
            ConvertorFromOFJava<MatchEntries, Match, MatchPath> convertor) {
        registryFromOFJAva.put(key, convertor);
        return hireJanitor(key, convertor);
    }
