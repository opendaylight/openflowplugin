...
public class ModelDrivenSwitchImpl extends AbstractModelDrivenSwitch {
	...
	// public static void setMethod(NodeId nId, Long xId)
	// {
	//
	// }
	@Override
	public Future<RpcResult<AddFlowOutput>> addFlow(AddFlowInput input) {
		...
		if (Objects.firstNonNull(input.isBarrier(), Boolean.FALSE)) {
			...
			Future<RpcResult<BarrierOutput>> barrierOFLib = messageService
					.barrier(barrierInput.build(), cookie);
		}

		// Convert the AddFlowInput to FlowModInput
		FlowModInputBuilder ofFlowModInput = FlowConvertor.toFlowModInput(
				input, version, this.getSessionContext().getFeatures()
						.getDatapathId());
		xId = session.getNextXid();
		...
		if (null != rpcNotificationProviderService) {
			...
			newFlow.setTransactionId(new TransactionId(BigInteger.valueOf(xId
					.intValue())));
			newFlow.setFlowRef(input.getFlowRef());
			newFlow.setTransactionUri(input.getTransactionUri());
			rpcNotificationProviderService.publish(newFlow.build());
		}

		LOG.debug("****** AddFlow Begin-Transaction****"
		...
		mapBulkTransaction.put(
				new TransactionKey(input.getNode().getValue()
						...
						.getValue().toString(), (new TransactionId(BigInteger
						.valueOf(xId.intValue()))).toString()), input);
		timeout = hashedwheeltimer.newTimeout(
				new EndTimerTask(input.getNode().getValue()
						.firstKeyOf(Node.class, NodeKey.class).getId()
						.getValue().toString(), (new TransactionId(BigInteger
						.valueOf(xId.intValue()))).toString()), 40000,
				TimeUnit.MILLISECONDS);
		LOG.debug("******AddFlow Timeout task assign****" + timeout.toString());

		Future<RpcResult<UpdateFlowOutput>> resultFromOFLib = messageService
				.flowMod(ofFlowModInput.build(), cookie);
		RpcResult<UpdateFlowOutput> rpcResultFromOFLib = null;

		try {
			rpcResultFromOFLib = resultFromOFLib.get();
		} catch (Exception ex) {
			LOG.error(" Error while getting result for AddFlow RPC"
					+ ex.getMessage());
		}
		...
		RpcResult<AddFlowOutput> rpcResult = Rpcs.getRpcResult(true, result,
				errors);
	...
	public Future<RpcResult<AddGroupOutput>> addGroup(AddGroupInput input) {
		...
		if (null != rpcNotificationProviderService) {
			...
			groupMod.setTransactionId(new TransactionId(BigInteger.valueOf(xId
					.intValue())));
			groupMod.setGroupRef(input.getGroupRef());
			groupMod.setTransactionUri(input.getTransactionUri());
		...
		Future<RpcResult<UpdateGroupOutput>> resultFromOFLib = messageService
				.groupMod(ofGroupModInput.build(), cookie);
		RpcResult<UpdateGroupOutput> rpcResultFromOFLib = null;

		try {
			rpcResultFromOFLib = resultFromOFLib.get();
		} catch (Exception ex) {
			LOG.error(" Error while getting result for AddGroup RPC"
					+ ex.getMessage());
		}
		...
		Collection<RpcError> errors = rpcResultFromOFLib.getErrors();
		RpcResult<AddGroupOutput> rpcResult = Rpcs.getRpcResult(true, result,
				errors);
	...
	public Future<RpcResult<AddMeterOutput>> addMeter(AddMeterInput input) {
		...
		if (null != rpcNotificationProviderService) {
			...
			meterMod.setTransactionId(new TransactionId(BigInteger.valueOf(xId
					.intValue())));
			meterMod.setMeterRef(input.getMeterRef());
			rpcNotificationProviderService.publish(meterMod.build());
		}

		Future<RpcResult<UpdateMeterOutput>> resultFromOFLib = messageService
				.meterMod(ofMeterModInput.build(), cookie);
		...
		try {
			...
			LOG.error(" Error while getting result for AddMeter RPC"
					+ ex.getMessage());
		}
		...
		Collection<RpcError> errors = rpcResultFromOFLib.getErrors();
		RpcResult<AddMeterOutput> rpcResult = Rpcs.getRpcResult(true, result,
				errors);
	...
	public Future<RpcResult<RemoveFlowOutput>> removeFlow(RemoveFlowInput input) {
		...
		if (null != rpcNotificationProviderService) {
			FlowRemovedBuilder removeFlow = new FlowRemovedBuilder(
					(org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.Flow) input);
			removeFlow.setTransactionId(new TransactionId(BigInteger
					.valueOf(xId.intValue())));
			removeFlow.setFlowRef(input.getFlowRef());
			removeFlow.setTransactionUri(input.getTransactionUri());
		...
		timeout = hashedwheeltimer.newTimeout(
				new EndTimerTask(input.getNode().getValue()
						...
						.valueOf(xId.intValue()))).toString()), 40000,
				TimeUnit.MILLISECONDS);
		LOG.debug("******RemoveFlow Timeout task assign****"
				+ timeout.toString());
		Future<RpcResult<UpdateFlowOutput>> resultFromOFLib = messageService
				.flowMod(ofFlowModInput.build(), cookie);
		...
		try {
			...
			LOG.error(" Error while getting result for remove Flow RPC"
					+ ex.getMessage());
		}
		...
		Collection<RpcError> errors = rpcResultFromOFLib.getErrors();
		RpcResult<RemoveFlowOutput> rpcResult = Rpcs.getRpcResult(true, result,
				errors);
	...
	public Future<RpcResult<RemoveGroupOutput>> removeGroup(
		...
		LOG.debug("****** RemoveGroup Begin-Transaction****"
				+ input.getTransactionUri().toString());
		LOG.debug("******RemoveGroup Plugin transaction-Id****"
				+ xId.intValue());
		mapBulkTransaction.put(
				new TransactionKey(input.getNode().getValue()
						.firstKeyOf(Node.class, NodeKey.class).getId()
		...
		timeout = hashedwheeltimer.newTimeout(
				new EndTimerTask(input.getNode().getValue()
						...
						.valueOf(xId.intValue()))).toString()), 40000,
				TimeUnit.MILLISECONDS);
		LOG.debug("******RemoveGroup Timeout task assign****"
				+ timeout.toString());
		...
		try {
			...
			LOG.error(" Error while getting result for RemoveGroup RPC"
					+ ex.getMessage());
		}
		...
		removeGroupOutput
				.setTransactionId(updateGroupOutput.getTransactionId());
		RemoveGroupOutput result = removeGroupOutput.build();

		Collection<RpcError> errors = rpcResultFromOFLib.getErrors();
		RpcResult<RemoveGroupOutput> rpcResult = Rpcs.getRpcResult(true,
				result, errors);
	...
	public Future<RpcResult<RemoveMeterOutput>> removeMeter(
		...
		try {
			...
			LOG.error(" Error while getting result for RemoveMeter RPC"
					+ ex.getMessage());
		}
		...
		removeMeterOutput
				.setTransactionId(updatemeterOutput.getTransactionId());
		RemoveMeterOutput result = removeMeterOutput.build();

		Collection<RpcError> errors = rpcResultFromOFLib.getErrors();
		RpcResult<RemoveMeterOutput> rpcResult = Rpcs.getRpcResult(true,
				result, errors);
	...
	public Future<RpcResult<Void>> transmitPacket(TransmitPacketInput input) {
		...
		PacketOutInput message = PacketOutConvertor.toPacketOutInput(input,
				version, sessionContext.getNextXid());
	...
	public Future<RpcResult<UpdateFlowOutput>> updateFlow(UpdateFlowInput input) {
		...
		SwitchConnectionDistinguisher cookie = null;
		if (Objects.firstNonNull(input.getUpdatedFlow().isBarrier(),
				Boolean.FALSE)) {
			BarrierInputBuilder barrierInput = new BarrierInputBuilder();
			xId = session.getNextXid();
			barrierInput.setVersion(version);
		...
		FlowModInputBuilder ofFlowModInput = FlowConvertor.toFlowModInput(
				input.getUpdatedFlow(), version, this.getSessionContext()
						.getFeatures().getDatapathId());
		xId = session.getNextXid();
		ofFlowModInput.setXid(xId);

		if (null != rpcNotificationProviderService) {
			FlowUpdatedBuilder updateFlow = new FlowUpdatedBuilder(
					(org.opendaylight.yang.gen.v1.urn.opendaylight.flow.types.rev131026.Flow) input
							.getUpdatedFlow());
			updateFlow.setTransactionId(new TransactionId(BigInteger
					.valueOf(xId.intValue())));
			updateFlow.setFlowRef(input.getFlowRef());
			updateFlow.setTransactionUri(input.getTransactionUri());
		...
		timeout = hashedwheeltimer.newTimeout(
				new EndTimerTask(input.getNode().getValue()
						...
						.valueOf(xId.intValue()))).toString()), 40000,
				TimeUnit.MILLISECONDS);
		LOG.debug("******UpdateFlow Timeout task assign****"
				+ timeout.toString());
		...
		try {
			...
			LOG.error(" Error while getting result for UpdateFlow RPC"
					+ ex.getMessage());
		}
		...
		updateFlowOutput.setTransactionId(updateFlowOutputOFLib
				.getTransactionId());
		UpdateFlowOutput result = updateFlowOutput.build();

		Collection<RpcError> errors = rpcResultFromOFLib.getErrors();
		RpcResult<UpdateFlowOutput> rpcResult = Rpcs.getRpcResult(true, result,
				errors);
	...
	public Future<RpcResult<UpdateGroupOutput>> updateGroup(
		...
		SwitchConnectionDistinguisher cookie = null;
		if (Objects.firstNonNull(input.getUpdatedGroup().isBarrier(),
				Boolean.FALSE)) {
			xId = session.getNextXid();
			BarrierInputBuilder barrierInput = new BarrierInputBuilder();
			barrierInput.setVersion(version);
		...
		GroupModInputBuilder ofGroupModInput = GroupConvertor.toGroupModInput(
				input.getUpdatedGroup(), version, this.getSessionContext()
						.getFeatures().getDatapathId());
		xId = session.getNextXid();
		ofGroupModInput.setXid(xId);

		if (null != rpcNotificationProviderService) {
			GroupUpdatedBuilder groupMod = new GroupUpdatedBuilder(
					(org.opendaylight.yang.gen.v1.urn.opendaylight.group.types.rev131018.Group) input
							.getUpdatedGroup());
			groupMod.setTransactionId(new TransactionId(BigInteger.valueOf(xId
					.intValue())));
		...
		LOG.debug("******UpdateGroup Begin-Transaction****"
				+ input.getTransactionUri().toString());
		...
		timeout = hashedwheeltimer.newTimeout(
				new EndTimerTask(input.getNode().getValue()
						...
						.valueOf(xId.intValue()))).toString()), 40000,
				TimeUnit.MILLISECONDS);
		LOG.debug("******UpdateGroup Timeout task assign****"
				+ timeout.toString());
		...
		try {
			...
			LOG.error(" Error while getting result for updateGroup RPC"
					+ ex.getMessage());
		}

		UpdateGroupOutput updateGroupOutputOFLib = rpcResultFromOFLib
				.getResult();

		UpdateGroupOutputBuilder updateGroupOutput = new UpdateGroupOutputBuilder();
		updateGroupOutput.setTransactionId(updateGroupOutputOFLib
				.getTransactionId());
		UpdateGroupOutput result = updateGroupOutput.build();

		Collection<RpcError> errors = rpcResultFromOFLib.getErrors();
		RpcResult<UpdateGroupOutput> rpcResult = Rpcs.getRpcResult(true,
				result, errors);
	...
	public Future<RpcResult<UpdateMeterOutput>> updateMeter(
		...
		// the request can be routed through any connection to the switch
		SwitchConnectionDistinguisher cookie = null;
		if (Objects.firstNonNull(input.getUpdatedMeter().isBarrier(),
				Boolean.FALSE)) {
			xId = session.getNextXid();
			BarrierInputBuilder barrierInput = new BarrierInputBuilder();
			barrierInput.setVersion(version);
		...
		MeterModInputBuilder ofMeterModInput = MeterConvertor.toMeterModInput(
				input.getUpdatedMeter(), version);
		xId = session.getNextXid();
		ofMeterModInput.setXid(xId);

		if (null != rpcNotificationProviderService) {
			MeterUpdatedBuilder meterMod = new MeterUpdatedBuilder(
					(org.opendaylight.yang.gen.v1.urn.opendaylight.meter.types.rev130918.Meter) input
							.getUpdatedMeter());
			meterMod.setTransactionId(new TransactionId(BigInteger.valueOf(xId
					.intValue())));
		...
		try {
			...
			LOG.error(" Error while getting result for UpdateMeter RPC"
					+ ex.getMessage());
		}

		UpdateMeterOutput updateMeterOutputFromOFLib = rpcResultFromOFLib
				.getResult();

		UpdateMeterOutputBuilder updateMeterOutput = new UpdateMeterOutputBuilder();
		updateMeterOutput.setTransactionId(updateMeterOutputFromOFLib
				.getTransactionId());
		UpdateMeterOutput result = updateMeterOutput.build();

		Collection<RpcError> errors = rpcResultFromOFLib.getErrors();
		RpcResult<UpdateMeterOutput> rpcResult = Rpcs.getRpcResult(true,
				result, errors);
	...
	public Future<RpcResult<GetAllGroupStatisticsOutput>> getAllGroupStatistics(
		...
		LOG.debug(
				"Prepare statistics request for all the groups - Transaction id - {}",
				xid);
		...
		mprGroupBuild.setGroupId(new GroupId(BinContent
				.intToUnsignedLong(Group.OFPGALL.getIntValue())));
		...
		LOG.debug("Send group statistics request to the switch :{}",
				mprGroupBuild);
		...
		RpcResult<GetAllGroupStatisticsOutput> rpcResult = Rpcs.getRpcResult(
				true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	...
	public Future<RpcResult<GetGroupDescriptionOutput>> getGroupDescription(
		...
		LOG.debug(
				"Prepare group description statistics request - Transaction id - {}",
				xid);
		...
		// Send the request, no cookies associated, use any connection
		LOG.debug("Send group desciption statistics request to switch : {}",
				mprGroupDescBuild);
		this.messageService.multipartRequest(mprInput.build(), null);
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetGroupDescriptionOutput> rpcResult = Rpcs.getRpcResult(
				true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	...
	public Future<RpcResult<GetGroupFeaturesOutput>> getGroupFeatures(
		...
		LOG.debug(
				"Prepare group features statistics request - Transaction id - {}",
				xid);
	...
	public Future<RpcResult<GetGroupStatisticsOutput>> getGroupStatistics(
		...
		LOG.debug(
				"Prepare statistics request for node {} group ({}) - Transaction id - {}",
				input.getNode(), input.getGroupId(), xid);
	...
	public Future<RpcResult<GetAllMeterConfigStatisticsOutput>> getAllMeterConfigStatistics(
		...
		LOG.debug(
				"Prepare config request for all the meters - Transaction id - {}",
				xid);
		...
		mprMeterConfigBuild.setMeterId(new MeterId(BinContent
				.intToUnsignedLong(Meter.OFPMALL.getIntValue())));
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetAllMeterConfigStatisticsOutput> rpcResult = Rpcs
	...
	public Future<RpcResult<GetAllMeterStatisticsOutput>> getAllMeterStatistics(
		...
		LOG.debug(
				"Prepare statistics request for all the meters - Transaction id - {}",
				xid);
		...
		mprMeterBuild.setMeterId(new MeterId(BinContent
				.intToUnsignedLong(Meter.OFPMALL.getIntValue())));
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetAllMeterStatisticsOutput> rpcResult = Rpcs.getRpcResult(
				true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	}
	...
	public Future<RpcResult<GetMeterFeaturesOutput>> getMeterFeatures(
		...
		LOG.debug(
				"Prepare features statistics request for all the meters - Transaction id - {}",
				xid);
	...
	public Future<RpcResult<GetMeterStatisticsOutput>> getMeterStatistics(
		...
		LOG.debug(
				"Preprae statistics request for Meter ({}) - Transaction id - {}",
				input.getMeterId().getValue(), xid);
	...
	public Future<RpcResult<GetAllNodeConnectorsStatisticsOutput>> getAllNodeConnectorsStatistics(
		...
		LOG.info(
				"Prepare port statistics request for all ports of node {} - TrasactionId - {}",
		...
		// Send the request, no cookies associated, use any connection
		LOG.debug("Send port statistics request :{}", mprPortStatsBuilder
				.build().toString());
		this.messageService.multipartRequest(mprInput.build(), null);
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetAllNodeConnectorsStatisticsOutput> rpcResult = Rpcs
				.getRpcResult(true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	}
	...
	public Future<RpcResult<GetNodeConnectorStatisticsOutput>> getNodeConnectorStatistics(
		...
		LOG.info(
				"Prepare port statistics request for port {} of node {} - TrasactionId - {}",
		...
		// Set specific port
		mprPortStatsBuilder.setPortNo(InventoryDataServiceUtil
				.portNumberfromNodeConnectorId(arg0.getNodeConnectorId()));
		caseBuilder.setMultipartRequestPortStats(mprPortStatsBuilder.build());
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetNodeConnectorStatisticsOutput> rpcResult = Rpcs
				.getRpcResult(true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	}
	...
	public Future<RpcResult<UpdatePortOutput>> updatePort(UpdatePortInput input) {
		...
		for (Port inputPort : inputPorts) {
			...
			Future<RpcResult<UpdatePortOutput>> resultFromOFLib = messageService
					.portMod(ofPortModInput, cookie);

			try {
				rpcResultFromOFLib = resultFromOFLib.get();
			} catch (Exception ex) {
				LOG.error(" Error while getting result for updatePort RPC"
						+ ex.getMessage());
			}
		...
		updatePortOutput.setTransactionId(updatePortOutputOFLib
				.getTransactionId());
		UpdatePortOutput result = updatePortOutput.build();

		Collection<RpcError> errors = rpcResultFromOFLib.getErrors();
		RpcResult<UpdatePortOutput> rpcResult = Rpcs.getRpcResult(true, result,
				errors);
	...
	public Future<RpcResult<UpdateTableOutput>> updateTable(
		...
		LOG.debug(
				"Prepare the Multipart Table Mod requests for Transaction Id {} ",
				xid);
		...
		// Set request body to main multipart request
		caseRequest.setMultipartRequestTableFeatures(tableFeaturesRequest
				.build());
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<UpdateTableOutput> rpcResult = Rpcs.getRpcResult(true,
				output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	}
	...
	public Future<RpcResult<GetAllFlowStatisticsFromFlowTableOutput>> getAllFlowStatisticsFromFlowTable(
		...
		LOG.debug(
				"Prepare statistics request to get flow stats for switch tables {} - Transaction id - {}",
		...
		mprInput.setMultipartRequestBody(multipartRequestFlowCaseBuilder
				.build());

		// Send the request, no cookies associated, use any connection
		LOG.debug("Send flow statistics request to the switch :{}",
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetAllFlowStatisticsFromFlowTableOutput> rpcResult = Rpcs
				.getRpcResult(true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	}
	...
	public Future<RpcResult<GetAllFlowsStatisticsFromAllFlowTablesOutput>> getAllFlowsStatisticsFromAllFlowTables(
		...
		LOG.info(
				"Prepare statistics request to get flow stats of all switch tables - Transaction id - {}",
				xid);
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetAllFlowsStatisticsFromAllFlowTablesOutput> rpcResult = Rpcs
				.getRpcResult(true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	...
	public Future<RpcResult<GetFlowStatisticsFromFlowTableOutput>> getFlowStatisticsFromFlowTable(
		...
		LOG.info(
				"Prepare statistics request to get stats for flow {} for switch tables {} - Transaction id - {}",
		...
		MatchReactor.getInstance().convert(arg0.getMatch(), version,
				mprFlowRequestBuilder,
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetFlowStatisticsFromFlowTableOutput> rpcResult = Rpcs
				.getRpcResult(true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	}
	...
	public Future<RpcResult<GetAggregateFlowStatisticsFromFlowTableForAllFlowsOutput>> getAggregateFlowStatisticsFromFlowTableForAllFlows(
		...
		FlowCreatorUtil.setWildcardedFlowMatch(version,
				mprAggregateRequestBuilder);
		...
		multipartRequestAggregateCaseBuilder
				.setMultipartRequestAggregate(mprAggregateRequestBuilder
						.build());
		mprInput.setMultipartRequestBody(multipartRequestAggregateCaseBuilder
				.build());

		// Send the request, no cookies associated, use any connection
		LOG.debug("Send request to the switch :{}",
				multipartRequestAggregateCaseBuilder.build().toString());
		this.messageService.multipartRequest(mprInput.build(), null);
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetAggregateFlowStatisticsFromFlowTableForAllFlowsOutput> rpcResult = Rpcs
				.getRpcResult(true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	}
	...
	public Future<RpcResult<GetAggregateFlowStatisticsFromFlowTableForGivenMatchOutput>> getAggregateFlowStatisticsFromFlowTableForGivenMatch(
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetAggregateFlowStatisticsFromFlowTableForGivenMatchOutput> rpcResult = Rpcs
				.getRpcResult(true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	}
	...
	public Future<RpcResult<GetFlowTablesStatisticsOutput>> getFlowTablesStatistics(
			GetFlowTablesStatisticsInput arg0) {
		// Generate xid to associate it with the request
		Long xid = this.getSessionContext().getNextXid();

		LOG.info(
				"Prepare flow table statistics request to get flow table stats for all tables "
						+ "from node {}- Transaction id - {}", arg0.getNode(),
				xid);
		...
		multipartRequestTableCaseBuilder
				.setMultipartRequestTable(multipartRequestTableBuilder.build());

		// Set request body to main multipart request
		mprInput.setMultipartRequestBody(multipartRequestTableCaseBuilder
				.build());

		// Send the request, no cookies associated, use any connection
		LOG.debug("Send request to the switch :{}",
				multipartRequestTableCaseBuilder.build().toString());
		this.messageService.multipartRequest(mprInput.build(), null);
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetFlowTablesStatisticsOutput> rpcResult = Rpcs.getRpcResult(
				true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	}
	...
	public Future<RpcResult<GetAllQueuesStatisticsFromAllPortsOutput>> getAllQueuesStatisticsFromAllPorts(
		...
		// Send the request, no cookies associated, use any connection
		LOG.debug("Send queue statistics request :{}", mprQueueBuilder.build()
				.toString());
		this.messageService.multipartRequest(mprInput.build(), null);
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetAllQueuesStatisticsFromAllPortsOutput> rpcResult = Rpcs
				.getRpcResult(true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	}
	...
	public Future<RpcResult<GetAllQueuesStatisticsFromGivenPortOutput>> getAllQueuesStatisticsFromGivenPort(
		...
		LOG.info(
				"Prepare queue statistics request to collect stats for "
						+ "all queues attached to given port {} of node {} - TrasactionId - {}",
				arg0.getNodeConnectorId().toString(),
				arg0.getNode().getValue(), xid);
		...
		mprQueueBuilder.setPortNo(InventoryDataServiceUtil
				.portNumberfromNodeConnectorId(arg0.getNodeConnectorId()));
	...
	public Future<RpcResult<GetQueueStatisticsFromGivenPortOutput>> getQueueStatisticsFromGivenPort(
		...
		LOG.info(
				"Prepare queue statistics request to collect stats for "
						+ "given queue attached to given port {} of node {} - TrasactionId - {}",
				arg0.getQueueId().toString(), arg0.getNodeConnectorId()
		...
		Collection<RpcError> errors = Collections.emptyList();
		RpcResult<GetQueueStatisticsFromGivenPortOutput> rpcResult = Rpcs
				.getRpcResult(true, output.build(), errors);
		return Futures.immediateFuture(rpcResult);
	}
