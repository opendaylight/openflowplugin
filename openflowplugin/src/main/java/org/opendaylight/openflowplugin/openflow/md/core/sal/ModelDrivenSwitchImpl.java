...
import org.opendaylight.yang.gen.v1.urn.opendaylight.flow.inventory.rev130819.GetNodeIdentifiersOutputBuilder;
...
public class ModelDrivenSwitchImpl extends AbstractModelDrivenSwitch {
    ...
    @Override
    public Future<RpcResult<GetNodeIdentifiersOutput>> getNodeIdentifiers(GetNodeIdentifiersInput input) {
        if (!sessionContext.isValid()) {
            return Futures.immediateFuture(RpcResultBuilder
                    .<GetNodeIdentifiersOutput> failed()
        ...
        GetNodeIdentifiersOutput result = new GetNodeIdentifiersOutputBuilder().setDatapathId(datapathId)
                .setIpAddress(ipAddress).build();
        return Futures.immediateFuture(RpcResultBuilder.<GetNodeIdentifiersOutput> success(result).build());
    }
