...
public class ConnectionConductorImpl implements OpenflowProtocolListener,
    ...
    public void init() {
        ...
        queue = QueueKeeperFactory.createFairQueueKeeper(queueProcessor, INGRESS_QUEUE_MAX_SIZE);
    }
    ...
    private void enqueueMessage(OfHeader message, QueueType queueType) {
        ListenableFuture<Void> enqueueResult = queue.push(message, this, queueType);
        if (enqueueResult != null) {
            LOG.debug("ingress throttling in use");
            try {
                enqueueResult.get();
                LOG.debug("ingress throttling is over");
            } catch (Exception e) {
                LOG.warn("Problem occured while waiting for message queue freed up.", e);
            }
        }
    }
