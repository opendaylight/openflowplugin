...
public abstract class ConvertReactor<FROM> {
    ...
    public <RESULT, TARGET> void convert(final FROM source, final short version, final TARGET target, final ConverterExecutor converterExecutor) {

        //lookup converter
        ConvertReactorConvertor<FROM, RESULT> convertor = (ConvertReactorConvertor<FROM, RESULT>) conversionMapping.get(version);
        if (convertor == null) {
            throw new IllegalArgumentException("converter for given version ["+version+"] not found");
        }
        RESULT convertedItem = convertor.convert(source, converterExecutor);
