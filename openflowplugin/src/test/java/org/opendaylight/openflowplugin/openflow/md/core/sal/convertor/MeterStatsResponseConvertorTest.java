...
public class MeterStatsResponseConvertorTest {
    ...
    public void testToSALMeterStatsList() {
        ...
        Optional<List<MeterStats>> meterStatsListOptional = ConvertorManager.getInstance().convert(
                org.opendaylight.yang.gen.v1.urn.opendaylight.openflow.protocol.rev130731.multipart.reply
                        .multipart.reply.body.multipart.reply.meter._case.multipart.reply.meter.MeterStats.class,
                createMeterStatsLit());

        Assert.assertTrue("Meter stats convertor not found", meterStatsListOptional.isPresent());
        List<MeterStats> meterStatsList = meterStatsListOptional.get();
