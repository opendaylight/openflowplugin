...
public class PacketOutConvertorTest{
    ...
    /**
     * Test for PacketOutConvertor with null parameters
     */
    @Test
    public void toPacketOutInputAllParmNullTest(){
        ...
        String NODE_ID = null;
        NodeKey NODE_KEY = new NodeKey(new NodeId(NODE_ID));
        InstanceIdentifier<Node> instanceId = InstanceIdentifier
                .builder(Nodes.class).child(Node.class, NODE_KEY).toInstance();
        NodeConnectorRef nodeConnRef = new NodeConnectorRef(instanceId);
        ...
        transmitPacketInputBuilder.setNode(null);
        transmitPacketInputBuilder.setPayload(null);
        transmitPacketInputBuilder.setEgress(nodeConnRef);
        transmitPacketInputBuilder.setIngress(nodeConnRef);
        ...
        PacketOutInput message = PacketOutConvertor.toPacketOutInput(
                transmitPacketInput, (short) 0, null, null);

        Assert.assertEquals(message.getAction(),
                this.buildActionForNullTransmitPacketInputAction(nodeConnRef));
        Assert.assertEquals(message.getBufferId(), OFConstants.OFP_NO_BUFFER);
        Assert.assertEquals(message.getInPort(), new PortNumber(0xfffffffdL));
        Assert.assertEquals(message.getVersion(), version);
        Assert.assertEquals(message.getXid(), xid);
    }
    ...
    public void toPacketOutInputAllParmTest(){
        ...
        NodeConnectorRef nEgressConfRef = new NodeConnectorRef(
                createNodeConnRef(NODE_ID, "0xfffffffd"));

        String inPort = "2";
        NodeConnectorRef nIngressConRef = new NodeConnectorRef(
                createNodeConnRef(NODE_ID, inPort));
        ...
        ArrayList<Byte> _converted_list = list;
        byte[] payload = ArrayUtils.toPrimitive(_converted_list
                .toArray(new Byte[0]));
        ...
        // Assert.assertEquals(transmitPacketInput.getAction(),
        // message.getAction());
    ...
    /**
     * Building action for null NullTransmitPacketInputAction
     * 
     * @param nodeConnRef
     * @return
     */
    private List<Action> buildActionForNullTransmitPacketInputAction(
            final NodeConnectorRef nodeConnRef){

        PortNumber outPort = null;
        List<PathArgument> outArgs = nodeConnRef.getValue().getPath();
        if(outArgs.size() >= 3){
            outPort = getPortNumber(outArgs.get(2), null);
        }
     ...
     * @return
     */
    private static PortNumber getPortNumber(final PathArgument pathArgument,
            final Short ofVersion){
        InstanceIdentifier.IdentifiableItem item = Arguments.checkInstanceOf(
                pathArgument, InstanceIdentifier.IdentifiableItem.class);
        NodeConnectorKey key = Arguments.checkInstanceOf(item.getKey(),
     ...
     * @return
     */
    public static NodeConnectorRef createNodeConnRef(final String nodeId,
            final String port){
