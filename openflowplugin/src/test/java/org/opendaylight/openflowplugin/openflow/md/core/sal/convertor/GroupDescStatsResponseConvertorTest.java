...
public class GroupDescStatsResponseConvertorTest {
    ...
    public void test() {
        ...
        Assert.assertFalse("Group desc stats should be not present", statsListOptional.isPresent());
    }
    ...
    public void testSingleGroupDescStat() {
        ...
        VersionConvertorData data = new VersionConvertorData(OFConstants.OFP_VERSION_1_3);
        Optional<List<GroupDescStats>> statsListOptional = ConvertorManager.getInstance().convert(groupDescStats, data);
        Assert.assertTrue("Group desc stats convertor not found", statsListOptional.isPresent());
        List<GroupDescStats> statsList = statsListOptional.get();
