...
import org.opendaylight.util.packet.LldpTlv.ChassisIdSubType;
...
/**
 * LLDP #2 packet unit tests.
 *
 * @author Frank Wood
 */
public class Lldp2Test extends PacketTest {

    private static final String ETH2_LLDP = "eth2-lldp2.hex";
    private static final int ETH2_LLDP_LEN = 269;
    ...
    private static final String CID = "HP J9587A";
    private static final MacAddress PID = mac("08:2e:5f:69:c4:40");
    ...
    private void verifyEthLldp(Packet pkt) {
        ...
        assertEquals(CID, lldp.chassisId().name());
        assertEquals(PID, lldp.portId().macAddr());
    ...
    protected Packet createEth2Lldp() {
        ...
        Lldp lldp = new Lldp.Builder()
            .chassisId(LldpTlv.chassisIdName(ChassisIdSubType.INTF_NAME, CID))
            .portId(LldpTlv.portIdMacAddr(PID))
            .ttl(LldpTlv.ttl(TTL))
            .options(new LldpTlv[] {
