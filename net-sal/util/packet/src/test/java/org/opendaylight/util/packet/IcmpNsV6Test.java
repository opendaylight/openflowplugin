...
import org.opendaylight.util.packet.IcmpV6.NeighborSolicitData;
...
import static org.opendaylight.util.packet.IcmpTypeCodeV6.NEIGHBOR_SOLICIT_NDP;
...
/**
 * ICMPv6 Neighbor Solicitation packet unit tests.
 *
 * @author Frank Wood
 */
public class IcmpNsV6Test extends PacketTest {

    private static final String IPV6_ICMPV6 = "eth2-ipv6-icmpv6-ns.hex";
    private static final int IPV6_ICMPV6_LEN = 86;

    private static final MacAddress DST_MAC = mac("00:60:97:07:69:ea");
    private static final MacAddress SRC_MAC = mac("00:00:86:05:80:da");

    private static final IpAddress DST_IP = ip("fe80::260:97ff:fe07:69ea");
    private static final IpAddress SRC_IP = ip("fe80::200:86ff:fe05:80da");

    private static final IpAddress NS_TGT_IP = ip("fe80::260:97ff:fe07:69ea");

    private static final MacAddress SRC_LL_MAC = mac("00:00:86:05:80:da");

    private static final int FLOW_LABEL = 0x00;
    private static final int UPPER_LAYER_LEN = 32;
    private static final int HOP_LIMIT = 255;

    private static final int CHECK_SUM = 0x68bd;
    ...
    private void verifyIp(Packet pkt) {
        ...
        assertEquals(NEIGHBOR_SOLICIT_NDP, icmp.typeCode());
        assertEquals(CHECK_SUM, icmp.checkSum());
        assertEquals(0, icmp.bytes().length);
        assertNull(icmp.routerAdvertiseData());
        assertNull(icmp.neighborAdvertiseData());
        assertNull(icmp.redirectData());

        assertEquals(NS_TGT_IP, icmp.neighborSolicitData().targetAddr());
    ...
    private Packet createIcmpReq() {
        ...
        IcmpV6 icmp = new IcmpV6.Builder()
            .neighborSolicitData(new NeighborSolicitData(NS_TGT_IP))
