...
import static org.opendaylight.util.packet.Dns.RecordType.TXT;
import static org.opendaylight.util.packet.Dns.ResponseCode.NO_ERROR;
...
/**
 * DNS TXT response unit tests.
 *
 * @author Frank Wood
 */
public class DnsTxtRespTest extends PacketTest {

    private static final String TEST_DATA_FILE
        = "eth2-ip-udp-dns-txt-resp.hex";
    
    private static final int PKT_LEN = 98;
    ...
    private static final int IP_CHECK_SUM = 0x993e;
    private static final int IP_IDENT = 0xcbec;
    ...
    private static final int UDP_LEN = 64;
    private static final int UDP_CHECK_SUM = 0xc725;

    private static final int TXID = 0x1032;
    private static final String QUERY_NAME = "google.com";
    private static final int TTL = 270;
    private static final String DATA = "\u000fv=spf1 ptr ?all";
    ...
    private void verify(Packet pkt) {
        ...
        assertEquals(NO_ERROR, dns.respCode());
        ...
        assertEquals(RecordType.TXT, q.recType());
        
        ResRecord<?>[] answers = dns.answers();
        assertEquals(1, answers.length);
        ResRecord<?> a = answers[0];
        
        assertEquals(QUERY_NAME, a.name());
        assertEquals(INTERNET, a.clsType());
        assertEquals(TXT, a.recType());
        assertEquals(TTL, a.ttl());
    ...
    private Packet create() {
        ...
        Dns dns = new Dns.Builder()
            ...
            .recurDesired(true)
            .svrRecurAvail(true)
            .query(QUERY_NAME, TXT)
            .answer(QUERY_NAME, TXT, TTL, DATA)     
            .build();
