...
/**
 * Mutable subclass of {@link OfmRoleReply}.
 *
 * @author Pramod Shanbhag
 */
public class OfmMutableRoleReply extends OfmRoleReply
        implements MutableMessage {

    private final Mutable mutt = new Mutable();

    /**
     * Constructs a mutable OpenFlow ROLE_REPLY message.
     *
     * @param header the message header
     */
    OfmMutableRoleReply(Header header) {
        super(header);
    }
    ...
    public OpenflowMessage toImmutable() {
        ...
        OfmRoleReply msg = new OfmRoleReply(header);
        msg.role = this.role;
     ...
     */
    public OfmMutableRoleReply role(ControllerRole role) {
        mutt.checkWritable(this);
     ...
     * @throws VersionMismatchException if version &lt; 1.2.
     */
    public OfmMutableRoleReply generationId(long generationId) {
        mutt.checkWritable(this);
        verMin12(header.version);
