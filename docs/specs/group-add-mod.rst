...
Proposed change
---------------
The implementation of OFPGC_ADD_OR_MOD command will be only for OVS2.6 and above.
Currently no other openflow swich supports this command, so the implementation will be specific 
to OVS for now and it can be widened based on the support from other openflow swiches.

Configuration parameter will be added in openflowplugin.cfg file, which users can enable it if 
the support GROUP ADD MOD is needed. This will be a runtime configuration update parameter.

Openflowplugin.cfg:
# GROUP ADD MOD Support for OVS2.6 and above
# group-add-mod-supported=false

FRM will use the above flag to decide which group command should be sent out.
Pipeline changes
----------------
None
