...
3. As applications register with reconciliation module
	...
	This step is required to make sure all applications have finished the replaying the flows/groups on the DPN.
...
5. OpenFlow plugin after establishing the connection with OVS, than invokes reconciliation.
...
12. Reconciliation module would co-relate status from all the modules and make sure all apps
have reported there status.
...
13. Based on co-relation data
		When all modules report success, Reconciliation module will advertise new OF switch connection to all the Application(s).
			The OF plugin would populate the Operation DS through Inventory Manager.
		else if any module fails to replay the flows / group, Reconciliation module requests OpenFlow plugin to disconnect the switch, also update the switch state to “unknown”.
		OpenFlow plugin will disconnect the switch, logging appropriate messages.
		

Implementation Details:
-----------------------
1. API will be exposed from reconciliation framework and the contract should be adhered by applications.
	i) 	Name of the application
	ii) Priority of the application
	iii)Intent Action
	iv) ReconciliationTaskFactory
...
Name: Name of the application who wants to register for reconcile

ReconciliationTaskFactory:
Applications who wants to register should provide a ReconciliationTaskFactory object.
	ReconciliationTaskFactory should implement api's like startReconciliation and cancelReconciliation
...
Other contributors:
 - Jozef Bacigal (jozef.bacigal@pantheon.tech) 
