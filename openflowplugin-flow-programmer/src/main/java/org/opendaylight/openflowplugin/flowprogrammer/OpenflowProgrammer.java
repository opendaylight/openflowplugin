...
public class OpenflowProgrammer {
    ...
    private static  HashMap<NodeId, Boolean> tbl_zero_initialized = new HashMap<NodeId, Boolean>();
    ...
    public static boolean writeFlow(FlowBuilder flow, NodeId nodeId, short tableID, short projectID) {
        ...
        LOG.info("Openflow Programmer: write flow to Node {}, table {}", nodeId.getValue(), flow.getTableId());

        if (!writeMergeTransactionAPI(flowInstanceId, flow.build(), LogicalDatastoreType.CONFIGURATION, projectID)) {
            LOG.info("{}: Failed to create Flow on node: {}",
    ...
    public static boolean removeFlow(FlowId flowId, NodeId nodeId, short tableId, short projectId) {
        ...
        if (! deleteTransactionAPI(flowInstanceId, LogicalDatastoreType.CONFIGURATION, projectId)) {
            LOG.info("{}: Failed to remove Flow on node: {}",
                    Thread.currentThread().getStackTrace()[1], nodeId.getValue());
            return false;
        }
        return true;
    ...
    private static boolean initializeTblZero(NodeId nodeId, short projectID) {
        FlowBuilder flow = OpenflowUtils.createDefaultDropFlow(TBL_ZERO, 0, new BigInteger("9876543210", 10), "OpenflowProgrammer_default");
        boolean ret = writeFlow(flow, nodeId, TBL_ZERO, projectID);
        if (ret == true) {
            tbl_zero_initialized.put(nodeId, Boolean.TRUE);
    ...
    public static boolean addFlowSpace(FlowBuilder flow, NodeId nodeId, short projectID) {
        ...
        LOG.info("addFlowSpace: call isFlowOk {}: {}", nodeId.getValue(), projectID);
        ...
        LOG.info("addFlowSpace: isFlowOk return {}", ret);
        if (ret == true) {
            /* Initialize table zero if not yet */
            if ((tbl_zero_initialized.get(nodeId) == null)
                || tbl_zero_initialized.get(nodeId).equals(Boolean.TRUE)) {
            ...
            if (ret == true) {
                ...
                LOG.info("addFlowSpace: writeFlow failed");
            }
        }
        LOG.info("addFlowSpace: return {}", ret);
        return ret;
    }
}
