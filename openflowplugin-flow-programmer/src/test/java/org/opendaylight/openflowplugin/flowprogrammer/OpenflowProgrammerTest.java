...
public class OpenflowProgrammerTest extends AbstractDataStoreManager {
    ...
    public void before() {
        int order = 0;
        setDataBroker();
        OpenflowProgrammer.setDataBroker(getDataBroker(), OpenflowProgrammer.PRJ_SFC);
        OpenflowProgrammer.setDataBroker(getDataBroker(), OpenflowProgrammer.PRJ_GBP);
    ...
    public void testWriteFlow() {
        ...
        boolean ret = OpenflowProgrammer.writeFlow(flowbuilder.build(), new NodeId("openflow:1:1"), OpenflowProgrammer.TBL_SFC_INGRESS_TRANSPORT, OpenflowProgrammer.PRJ_SFC);
    ...
    public void testRemoveFlow() {
        ...
        boolean ret = OpenflowProgrammer.removeFlow(new FlowId(bigInt.toString()), new NodeId("openflow:1:1"), OpenflowProgrammer.TBL_SFC_INGRESS_TRANSPORT, OpenflowProgrammer.PRJ_SFC);
        assertTrue("Must be true", ret);
    }

    @Test
    public void testAddFlowSpace() {
        MatchBuilder match = new MatchBuilder();
        addMatchEtherType(match, (long)0x0800);

        FlowBuilder flowbuilder1 = createFlowBuilder(OpenflowProgrammer.TBL_ZERO, 0, new BigInteger("9991", 10), "SFC_TEST_FLOW1", match, isb);
        boolean ret = OpenflowProgrammer.addFlowSpace(flowbuilder1.build(), new NodeId("openflow:1:1"), OpenflowProgrammer.PRJ_SFC);
        assertTrue("Must be true", ret);
        FlowBuilder flowbuilder2 = createFlowBuilder(OpenflowProgrammer.TBL_ZERO, 0, new BigInteger("9992", 10), "GBP_TEST_FLOW1", match, isb);
        ret = OpenflowProgrammer.addFlowSpace(flowbuilder2.build(), new NodeId("openflow:1:1"), OpenflowProgrammer.PRJ_GBP);
