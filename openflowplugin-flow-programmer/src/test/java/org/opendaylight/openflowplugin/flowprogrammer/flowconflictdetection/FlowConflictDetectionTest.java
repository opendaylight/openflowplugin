...
public class FlowConflictDetectionTest extends AbstractDataStoreManager {
    ...
    public void testSaveFlow() {
        ...
        FlowConflictDetection.saveFlow(flowbuilder.build(), new NodeId("openflow:1:1"), OpenflowProgrammer.TBL_SFC_INGRESS_TRANSPORT, OpenflowProgrammer.PRJ_SFC);
        assertNotNull("Must be not null", null);
    }

    @Test
    public void testIsFlowOkInProject() {
        MatchBuilder match = new MatchBuilder();
        addMatchEtherType(match, (long)0x0800);

        FlowBuilder flowbuilder = createFlowBuilder(OpenflowProgrammer.TBL_SFC_INGRESS_TRANSPORT, 0, new BigInteger("9999", 10), "SFC_TEST_FLOW", match, isb);
        boolean ret = FlowConflictDetection.isFlowOkInProject(flowbuilder.build(), new NodeId("openflow:1:1"), OpenflowProgrammer.TBL_SFC_INGRESS_TRANSPORT, OpenflowProgrammer.PRJ_SFC);
        assertFalse("Must be false", ret);
    }

    @Test
    public void testIsFlowOk() {
        MatchBuilder match = new MatchBuilder();
        addMatchEtherType(match, (long)0x0800);

        FlowBuilder flowbuilder = createFlowBuilder(OpenflowProgrammer.TBL_SFC_INGRESS_TRANSPORT, 0, new BigInteger("9999", 10), "SFC_TEST_FLOW", match, isb);
        boolean ret = FlowConflictDetection.isFlowOk(flowbuilder.build(), new NodeId("openflow:1:1"), OpenflowProgrammer.TBL_SFC_INGRESS_TRANSPORT, OpenflowProgrammer.PRJ_GBP);
