...
public class FlowProgrammerImplTest extends AbstractDataBrokerTest{
    ...
    public void testFlowProgrammerImpl() throws Exception {
        ...
        InstanceIdentifier<Node> TOAST_IID = InstanceIdentifier.builder(Nodes.class).child(Node.class, nodeBuilder.getKey()).build();

        DataBroker broker = getDataBroker();

        ReadOnlyTransaction rTx = broker.newReadOnlyTransaction();
        Optional<Node> optional = rTx.read( LogicalDatastoreType.OPERATIONAL, TOAST_IID ).get();
        assertNotNull( optional );
        assertFalse( "Didn't expect operational data for node", optional.isPresent() );

        Optional<Node> rNode = rTx.read( LogicalDatastoreType.CONFIGURATION, TOAST_IID ).get();
