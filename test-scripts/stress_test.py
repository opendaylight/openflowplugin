...
from openvswitch.testclass_components import  \
    CheckOperFlowsComponent, CheckConfigFlowsComponent, CheckSwitchDump
...
class MultiTest(unittest.TestCase):
    ...
    def test(self):
        ...
        for item in switch_flows_list:
            idByEntry = CheckSwitchDump().get_id_by_entry(item, self.active_map)
            if idByEntry is not None:
                MultiTest.log.debug('flow_id:{0} = {1}'.format(idByEntry, item))
                switch_flows += 1
