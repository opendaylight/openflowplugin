...
from tools.test_with_param_superclass import TestOpenFlowXml_Base
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Run switch <-> ODL tests '
                                     'defined by xmls.')
    parser.add_argument('--odlhost (default 127.0.0.1) ', default='127.0.0.1', help='host where '
                        'odl controller is running')
    parser.add_argument('--odlport (default 8080) ', type=int, default=8080, help='port on '
                        'which odl\'s RESTCONF is listening')
    parser.add_argument('--mnport (default 6653) ', type=int, default=6653, help='port on '
                        'which odl\'s controller is listening')
    parser.add_argument('--xmls (default None) ', default=None, help='generete tests only '
