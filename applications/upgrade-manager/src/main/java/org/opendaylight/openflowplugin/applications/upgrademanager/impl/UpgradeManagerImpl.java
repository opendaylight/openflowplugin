...
public class UpgradeManagerImpl implements ClusteredDataTreeChangeListener<OpenflowUpgradeMode>, UpgradeManager {
    ...
    private Mode upgradeMode = Mode.AUTONOMOUS;
    ...
    @Override
    public void onDataTreeChanged(@Nonnull Collection<DataTreeModification<OpenflowUpgradeMode>> changes) {
        ...
        for (DataTreeModification<OpenflowUpgradeMode> change : changes) {
            ...
            switch (mod.getModificationType()) {
                ...
                case SUBTREE_MODIFIED:
                    break;
                case WRITE:
                    upgradeMode = mod.getDataAfter().getUpgradeMode();
                    if (upgradeMode == Mode.AUTONOMOUS) {
                        for (Iterator<Map.Entry<InstanceIdentifier<FlowCapableNode>, BundleId>>
                            ...
                            Futures.addCallback(JdkFutureAdapters
                                ...
                                new UpgradeManagerCallback(nodeIdentity));
                        }
                    }
                    break;
                default:
    ...
    private class UpgradeReconciliationTask implements Callable<Boolean> {
        ...
        @Override
        public Boolean call() {
            if (upgradeMode.equals(Mode.AUTONOMOUS)) {
                return true;
            }
