...
public class SalGroupRpcsMock {
    ...
    private ListenableFuture<RpcResult<AddGroupOutput>> addGroup(AddGroupInput input) {
        addGroupCalls.add(input);
        return RpcResultBuilder.success(new AddGroupOutputBuilder().build()).buildFuture();
    }

    private ListenableFuture<RpcResult<RemoveGroupOutput>> removeGroup(RemoveGroupInput input) {
        removeGroupCalls.add(input);
        return RpcResultBuilder.success(new RemoveGroupOutputBuilder().build()).buildFuture();
    }

    private ListenableFuture<RpcResult<UpdateGroupOutput>> updateGroup(UpdateGroupInput input) {
