...
public class SalMeterRpcsMock {
    ...
    private ListenableFuture<RpcResult<AddMeterOutput>> addMeter(AddMeterInput input) {
        addMeterCalls.add(input);
        return RpcResultBuilder.success(new AddMeterOutputBuilder().build()).buildFuture();
    }

    private ListenableFuture<RpcResult<RemoveMeterOutput>> removeMeter(RemoveMeterInput input) {
        removeMeterCalls.add(input);
        return RpcResultBuilder.success(new RemoveMeterOutputBuilder().build()).buildFuture();
    }

    private ListenableFuture<RpcResult<UpdateMeterOutput>> updateMeter(UpdateMeterInput input) {
