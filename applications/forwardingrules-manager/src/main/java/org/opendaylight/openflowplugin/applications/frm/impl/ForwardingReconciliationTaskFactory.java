...
public class ForwardingReconciliationTaskFactory implements IReconciliationTaskFactory  {
        ...
        public ForwardingReconciliationTaskFactory(String serviceName, final int priority, final Intent intent,
                ForwardingRulesManager forwardingRulesManager) {
        ...
        @Override
        public Future startReconcileTask(NodeId nodeId) {
                DeviceMastership deviceMastership = forwardingRulesManager.getDeviceMastershipManager().getDeviceMasterships().computeIfAbsent(nodeId, device ->
        ...
        public Future reconcileTask(NodeId nodeId) {
                ...
                LOG.info("isnodeowner {} for frm {}", forwardingRulesManager.isNodeOwner(connectedNode),
                        forwardingRulesManager);
                if (forwardingRulesManager.isNodeOwner(connectedNode)) {
                        LOG.info("Triggering reconciliation for device {} by sevice {}",
                                connectedNode.firstKeyOf(Node.class), serviceName);
                        if (forwardingRulesManager.isStaleMarkingEnabled()) {
                                LOG.info(
                                        "Stale-Marking is ENABLED and proceeding with deletion of stale-marked entities on switch {}",
        ...
        @Override
        public Future cancelReconcileTask(NodeId nodeId) {
                future.cancel(true);
                return future;
        }
}
