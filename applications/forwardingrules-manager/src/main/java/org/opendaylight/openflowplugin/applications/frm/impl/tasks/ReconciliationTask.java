    ...
    public Boolean call() {
        ...
        if (flowNode.isPresent()) {
            ...
            while ((!(toBeInstalledGroups.isEmpty()) || !(suspectedGroups.isEmpty())) &&
                    (counter <= provider.getReconciliationRetryCount())) {
                //also check if the counter has not crossed the threshold
                    ...
                    for (Bucket bucket : bucketList) {
                        ...
                        for (Action action : actions) {
                            ...
                            if (action.getAction().getImplementedInterface().getName()
                                    .equals("org.opendaylight.yang.gen.v1.urn.opendaylight.action.types.rev131112" +
                                            ".action.action.OutputActionCase")) {
                                ...
                                if (isPresent) {
                                    ...
                                    LOG.debug("Not yet received the node-connector updated for {} " +
                                                    "for the group with id {}", nodeConnectorUri,
