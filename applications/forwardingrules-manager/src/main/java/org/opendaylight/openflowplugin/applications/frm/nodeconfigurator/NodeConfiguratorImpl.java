...
public class NodeConfiguratorImpl implements NodeConfigurator {
    ...
    public <T> ListenableFuture<T> enqueueJob(String key, Callable<ListenableFuture<T>> mainWorker) {
        ...
        JobQueue jobQueue = jobQueueMap.computeIfAbsent(key, mapKey -> new JobQueue(key));
    ...
    public void close() throws Exception {
        {
            ...
            for (Map.Entry<String, JobQueue> entry : jobQueueMap.entrySet()) {
                ...
                jobQueue.getSyncThreadPool().shutdownNow();
            }
            ...
            while (true) {
                try {
                    for (Map.Entry<String, JobQueue> entry : jobQueueMap.entrySet()) {
                        ...
                        executeTask(jobQueue.getSyncThreadPool(), worker) ;
                    }
                } catch (Exception e) {
    ...
    private boolean executeTask(ListeningExecutorService syncThreadPool, Runnable task) {
        try {
