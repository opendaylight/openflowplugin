...
public class FlowForwarder extends AbstractListeningCommiter<Flow> {
    ...
    public FlowForwarder(final ForwardingRulesManager provider, final DataBroker dataBroker) {
        super(provider, dataBroker);
    }
    ...
    public void remove(final InstanceIdentifier<Flow> identifier,
        ...
        if (tableIdValidationPrecondition(tableKey, removeDataObj)) {
            ...
            getProvider().getSalFlowService().removeFlow(builder.build());
        }
    }
    ...
    public Future<RpcResult<RemoveFlowOutput>> removeWithResult(final InstanceIdentifier<Flow> identifier,
        ...
        if (tableIdValidationPrecondition(tableKey, removeDataObj)) {
            ...
            builder.setTransactionUri(new Uri(getProvider().getNewTransactionId())).
                    setStrict(Boolean.TRUE);
            resultFuture = getProvider().getSalFlowService().removeFlow(builder.build());
        }
    ...
    public void update(final InstanceIdentifier<Flow> identifier,
        ...
        if (tableIdValidationPrecondition(tableKey, update)) {
            ...
            getProvider().getSalFlowService().updateFlow(builder.build());
        }
    }
    ...
    public Future<RpcResult<AddFlowOutput>> add(
        ...
        if (tableIdValidationPrecondition(tableKey, addDataObj)) {
            ...
            builder.setFlowTable(new FlowTableRef(nodeIdent.child(Table.class, tableKey)));
            builder.setTransactionUri(new Uri(getProvider().getNewTransactionId()));
            future = getProvider().getSalFlowService().addFlow(builder.build());
        } else {
    ...
    private InstanceIdentifier<org.opendaylight.yang.gen.v1.urn.opendaylight.flow.inventory.rev130819.tables.table
            .StaleFlow> getStaleFlowInstanceIdentifier(StaleFlow staleFlow,
                                                       InstanceIdentifier<FlowCapableNode> nodeIdent) {
        return nodeIdent
                .child(Table.class, new TableKey(staleFlow.getTableId()))
                .child(org.opendaylight.yang.gen.v1.urn.opendaylight.flow.inventory.rev130819.tables.table
                                .StaleFlow.class,
