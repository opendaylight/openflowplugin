...
public class FlowNodeReconciliationImpl implements FlowNodeReconciliation {
        ...
        if (flowNode.isPresent()) {
            ...
            while ((!(toBeInstalledGroups.isEmpty()) || !(suspectedGroups.isEmpty())) &&
                ...
                while (iterator.hasNext()) {
                    ...
                    if (okToInstall) {
                        addGroup(groupFutures, group, nodeIdent);
                        iterator.remove();
     ...
     * @param nodeIdent  The identifier for the target node.
     */
    ...
    private void awaitGroups(String nodeId,
        ...
        if (!futures.isEmpty()) {
            ...
            try {
                ...
                LOG.warn("add-group RPCs did not complete: node={}", nodeId);
            } catch (Exception e) {
                String msg = "Unhandled exception in awaitGroups(): node=" +
                    nodeId;
                LOG.error(msg, e);
            }
        }
