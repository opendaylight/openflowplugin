...
public class FlowNodeReconciliationImpl implements FlowNodeReconciliation {
    ...
    private void flowNodeConnected(InstanceIdentifier<FlowCapableNode> connectedNode, boolean force) {
        if (force || !provider.isNodeActive(connectedNode)) {
            ...
            ReconciliationTask reconciliationTask = new ReconciliationTask(connectedNode, this.provider);
    ...
    private class ReconciliationTask implements Runnable {
        ...
        InstanceIdentifier<FlowCapableNode> m_nodeIdent;
        ForwardingRulesManager m_provider;


        public ReconciliationTask(final InstanceIdentifier<FlowCapableNode> nodeIdent, final ForwardingRulesManager manager) {
            m_provider = Preconditions.checkNotNull(manager, "ForwardingRulesManager can not be null!");
            m_nodeIdent = nodeIdent;
        }

        @Override
        public void run() {

            String sNode = m_nodeIdent.firstKeyOf(Node.class, NodeKey.class).getId().getValue();
            ...
            counter.set(0);
            try {
                flowNode = trans.read(LogicalDatastoreType.CONFIGURATION, m_nodeIdent).get();
            } catch (Exception e) {
                LOG.error("Fail with read Config/DS for Node {} !", m_nodeIdent, e);
            }
            ...
            /* Tables - have to be pushed before groups */
                ...
                for (Table table : tableList) {
                    ...
                    KeyedInstanceIdentifier<TableFeatures, TableFeaturesKey> tableFeaturesII
                            = m_nodeIdent.child(Table.class, tableKey).child(TableFeatures.class, new TableFeaturesKey(tableKey.getId()));
                    ...
                    if (tableFeatures != null) {
                        for (TableFeatures tableFeaturesItem : tableFeatures) {
                            provider.getTableFeaturesCommiter().update(tableFeaturesII, tableFeaturesItem, null, m_nodeIdent);
                        }
                    }
                }
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (okToInstall) {
                            final KeyedInstanceIdentifier<Group, GroupKey> groupIdent =
                                    m_nodeIdent.child(Group.class, group.getKey());
                            this.m_provider.getGroupCommiter().add(groupIdent, group, m_nodeIdent);
            ...
            /* Meters */
                ...
                for (Meter meter : meters) {
                    final KeyedInstanceIdentifier<Meter, MeterKey> meterIdent =
                            m_nodeIdent.child(Meter.class, meter.getKey());
                    this.m_provider.getMeterCommiter().add(meterIdent, meter, m_nodeIdent);
                }
            /* Flows */
                ...
                for (Table table : tables) {
                    final KeyedInstanceIdentifier<Table, TableKey> tableIdent =
                            m_nodeIdent.child(Table.class, table.getKey());
                    ...
                    for (Flow flow : flows) {
                        ...
                        this.m_provider.getFlowCommiter().add(flowIdent, flow, m_nodeIdent);
                    }
                }
            }
        /* clean transaction */
