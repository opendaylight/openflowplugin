...
public class FlowNodeReconciliationImpl implements FlowNodeReconciliation {
    ...
    public void reconcileConfiguration(InstanceIdentifier<FlowCapableNode> connectedNode) {
        ...
        if (provider.isNodeOwner(connectedNode)) {
            ...
            LOG.debug("value of provider.isBundleBasedReconEnabled() parameter as read from config {}", provider.isBundleBasedReconEnabled());
    ...
    private class BundleBasedReconciliationTask implements Runnable
    {
        ...
        public void run() {
            ...
            LOG.info("Triggering bundle based reconciliation for device ID:{}", nDpId);
            ...
            if (flowNode.isPresent()) {
                LOG.info("FlowNode present for DPN ID {}", nDpId);
            ...
            LOG.info("Completing bundle based reconciliation for device ID:{}", nDpId);
            trans.close();
        }
    }
    ...
    private class ReconciliationTask implements Runnable {
        ...
        @Override
        public void run() {
            LOG.info("Triggering reconciliation");
            String sNode = nodeIdentity.firstKeyOf(Node.class, NodeKey.class).getId().getValue();
            BigInteger nDpId = getDpnIdFromNodeName(sNode);
            ...
            if (flowNode.isPresent()) {
                LOG.debug("flowNode present for DPN ID in reconciliation{}",nDpId);
        ...
        /* clean transaction */
            LOG.info("Completing reconciliation for DPN ID {}", nDpId);
            trans.close();
        }
    ...
    private static Flow preDeleteAllFlow(){
        FlowBuilder flowBuilder = new FlowBuilder();
        flowBuilder.setTableId(OFConstants.OFPTT_ALL);
        LOG.debug("Within preDeleteAllFlow() :: existing flows are cleared from the dpn");
        return flowBuilder.build();
    }

    private static Group preDeleteAllGroup(){
        ...
        LOG.debug("within preDeleteAllGroup() :: existing groups are cleared from the dpn");
        return groupBuilder.build();
    }

    private List<Message> createMessages(NodeRef nodeRef , Optional<FlowCapableNode> flowNode) {
        LOG.info(" Inside createMessages() for bundle based reconciliation ");
        ...
        LOG.debug("The size of the group messages created {}",groupCounter);
        ...
        LOG.debug("The size of the flow messages created {}", flowCounter);
