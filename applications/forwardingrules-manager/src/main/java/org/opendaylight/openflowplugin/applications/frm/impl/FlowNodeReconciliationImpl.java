...
public class FlowNodeReconciliationImpl implements FlowNodeReconciliation {
    ...
    public ListenableFuture<Boolean> startReconciliation(DeviceInfo node) {
        ...
        return futureMap.computeIfAbsent(node, future -> {
            LOG.info("Triggering tasks for device {}", connectedNode.firstKeyOf(Node.class));
            ...
            return Futures.transformAsync(resultFuture, (result) -> executor.submit(provider.isBundleBasedReconciliationEnabled()
