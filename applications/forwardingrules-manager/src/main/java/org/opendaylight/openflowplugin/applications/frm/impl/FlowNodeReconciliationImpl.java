...
public class FlowNodeReconciliationImpl implements FlowNodeReconciliation {
       ...
       // flowNode.
        ...
        if (flowNode.isPresent()) {
            ...
            /* Groups - have to be first */
                ...
                while ((!(toBeInstalledGroups.isEmpty()) || !(suspectedGroups.isEmpty())) && (counter.get()<provider.getConfiguration().getReconciliationRetryCount())) { //also check if the counter has not crossed the threshold
                    ...
                    if(toBeInstalledGroups.isEmpty() && ! suspectedGroups.isEmpty()){
                        ...
                        toBeInstalledGroups = suspectedGroups;
                        break;
                    }
            ...
            if(!toBeInstalledGroups.isEmpty()){
                for(Group group :toBeInstalledGroups){
                    LOG.error("Installing the group {} even-though the port is not up",group.getGroupId().toString());
                    final KeyedInstanceIdentifier<Group, GroupKey> groupIdent =
                            nodeIdent.child(Group.class, group.getKey());
                    this.provider.getGroupCommiter().add(groupIdent, group, nodeIdent);
                }
    ...
    public void continueGroupInstallation(List<Group> groupList){
