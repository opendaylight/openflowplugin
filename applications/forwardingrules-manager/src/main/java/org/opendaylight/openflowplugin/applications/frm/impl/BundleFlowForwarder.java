...
public class BundleFlowForwarder {
    ...
    public void update(final InstanceIdentifier<Flow> identifier, final Flow originalFlow, final Flow updatedFlow,
        ...
        nodeConfigurator.enqueueJob(nodeId.getValue(), () -> {
            ...
            Message message = new MessageBuilder().setNode(new NodeRef(nodeIdent.firstIdentifierOf(Node.class)))
                    .setBundleInnerMessage(bundleInnerMessage).build();
            ListenableFuture<RpcResult<Void>> groupFuture = pushDependentGroup(nodeIdent,
    ...
    public Future<? extends RpcResult<?>> add(final InstanceIdentifier<Flow> identifier, final Flow flow,
        ...
        return nodeConfigurator.enqueueJob(nodeId.getValue(), () -> {
            ...
            Message message = new MessageBuilder().setNode(new NodeRef(nodeIdent.firstIdentifierOf(Node.class)))
                    .setBundleInnerMessage(bundleInnerMessage).build();
            ListenableFuture<RpcResult<Void>> groupFuture = pushDependentGroup(nodeIdent, flow,
                    identifier, bundleId);
            ListenableFuture<RpcResult<AddFlowOutput>> flowFuture = SettableFuture.create();
            Futures.addCallback(groupFuture, new BundleFlowCallBack(nodeIdent, bundleId, message, flowFuture),
    ...
    private ListenableFuture<RpcResult<Void>> pushDependentGroup(
        ...
        if (groupId != null) {
            ...
            if (isGroupExistsOnDevice(nodeIdent, groupId, forwardingRulesManager)) {
                ...
                resultFuture = Futures.immediateFuture(RpcResultBuilder.<Void>success().build());
            } else {
                ...
                try (ReadOnlyTransaction readTransaction = forwardingRulesManager.getReadTransaction()) {
                    ...
                    if (group.isPresent()) {
                        ...
                        resultFuture = JdkFutureAdapters.listenInPoolThread(forwardingRulesManager
                        ...
                        resultFuture = Futures.immediateFuture(RpcResultBuilder.<Void>success()
                                .build());
                    }
    ...
    private final class BundleFlowCallBack implements FutureCallback<RpcResult<Void>> {
        private final InstanceIdentifier<FlowCapableNode> nodeIdent;
        private final BundleId bundleId;
        private final Message message;
        private final NodeId nodeId;
        private ListenableFuture<RpcResult<Void>> flowFuture;
        ...
        public void onSuccess(RpcResult<Void> rpcResult) {
            if (rpcResult.isSuccessful()) {
                ...
                flowFuture = JdkFutureAdapters.listenInPoolThread(forwardingRulesManager.getSalBundleService()
                        .addBundleMessages(addBundleMessagesInput));
            } else {
        ...
        public void onFailure(Throwable throwable) {
            ...
            JdkFutures.addErrorLogging(flowFuture, LOG, "Service call for adding bundle flow with id"
                    + bundleId.getValue() + "failed for node" +  "with error" + throwable.getCause());
        }
    }
}
