...
public class ArbitratorReconcileServiceImpl implements ArbitratorReconcileService {
    ...
    @Override
    public Future<RpcResult<CommitBundleNodeOutput>> commitBundleNode(
            CommitBundleNodeInput input) {
        ...
        ListenableFuture<RpcResult<CommitBundleNodeOutput>> result = Futures.transform(
                JdkFutureAdapters
                        .listenInPoolThread(this.arbitratorReconciliationManager.commitActiveBundle(connectedNode)),
    ...
    private Node buildNode(long nodeIid) {
        NodeId nodeId = new NodeId(OPENFLOW_PREFIX + nodeIid);
