...
public class FlowForwarder extends AbstractListeningCommiter<Flow> {
    ...
    public FlowForwarder(final ForwardingRulesManager manager, final DataBroker db, NodeConfigurator nodeConfigurator) {
    ...
    public void update(final InstanceIdentifier<Flow> identifier, final Flow original, final Flow update,
        ...
        if (tableIdValidationPrecondition(tableKey, update)) {
            ...
            Future<RpcResult<? extends RpcResult<?>>> future = Futures.immediateFuture(null);
            ...
            nodeConfigurator.enqueueJob(nodeIdent.firstKeyOf(Node.class, NodeKey.class).getId().getValue(), ()-> {
                ...
                Future<RpcResult<? extends RpcResult<?>>> future1 = Futures.immediateFuture(null);
                if (groupId != null) {
                    if (isGroupExistsOnDevice(nodeIdent, groupId)) {
                        future1 = provider.getSalFlowService().updateFlow(builder.build());
    ...
    @Override
    public Future<RpcResult<? extends RpcResult<?>>> add(final InstanceIdentifier<Flow> identifier, final Flow addDataObj,
            final InstanceIdentifier<FlowCapableNode> nodeIdent) {
        ...
        if (tableIdValidationPrecondition(tableKey, addDataObj)) {
            ...
            nodeConfigurator.enqueueJob(nodeIdent.firstKeyOf(Node.class, NodeKey.class).getId().getValue(), () -> {
                ...
                if (groupId != null) {
                    if (isGroupExistsOnDevice(nodeIdent, groupId)) {
                        future1 = provider.getSalFlowService().addFlow(builder.build());
                    } else {
                        Future<? extends RpcResult<?>> groupFuture = pushDependentGroup(nodeIdent, groupId);
                        Futures.addCallback(JdkFutureAdapters.listenInPoolThread(groupFuture),
    ...
    private final class AddFlowCallBack implements FutureCallback<RpcResult<?>> {
        ...
        private Future<RpcResult<? extends RpcResult<?>>> future;

        private AddFlowCallBack(final AddFlowInput addFlowInput, Future<RpcResult<? extends RpcResult<?>>> future) {
    ...
    private final class UpdateFlowCallBack implements FutureCallback<RpcResult<?>> {
        ...
        private UpdateFlowCallBack(final UpdateFlowInput updateFlowInput, Future<RpcResult<? extends RpcResult<?>>> future) {
