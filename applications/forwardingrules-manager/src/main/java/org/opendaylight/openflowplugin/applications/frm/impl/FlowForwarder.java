...
public class FlowForwarder extends AbstractListeningCommiter<Flow> {
    ...
    public void update(final InstanceIdentifier<Flow> identifier, final Flow original, final Flow update,
        ...
        if (tableIdValidationPrecondition(tableKey, update)) {
            ...
            Future<RpcResult<UpdateFlowOutput>> future = jobCoordinator.enqueueJob(nodeIdent.firstKeyOf(Node.class, NodeKey.class).getId().getValue(), ()-> {
                ...
                if (groupId != null) {
                    if (isGroupExistsOnDevice(nodeIdent, groupId)) {
                        future1 = JdkFutureAdapters.listenInPoolThread(provider.getSalFlowService().updateFlow(builder.build()));
                        ...
                        Futures.addCallback(JdkFutureAdapters.listenInPoolThread(groupFuture),
                                new UpdateFlowCallBack(builder.build(), future1) , MoreExecutors.directExecutor());
                    }
                } else {
                    future1 = JdkFutureAdapters.listenInPoolThread(provider.getSalFlowService().updateFlow(builder.build()));
                    JdkFutures.addErrorLogging(future1, LOG, "updateFlow");
    ...
    public Future<RpcResult<AddFlowOutput>> add(final InstanceIdentifier<Flow> identifier, final Flow addDataObj,
        ...
        if (tableIdValidationPrecondition(tableKey, addDataObj)) {
            ...
            builder.setFlowTable(new FlowTableRef(nodeIdent.child(Table.class, tableKey)));
            builder.setTransactionUri(new Uri(provider.getNewTransactionId()));
                Future<RpcResult<AddFlowOutput>> future = jobCoordinator.enqueueJob(nodeIdent.firstKeyOf(Node.class, NodeKey.class).getId().getValue(), () -> {
                Long groupId = isFlowDepenendsOnGroup(addDataObj);
                ListenableFuture<RpcResult<AddFlowOutput>> future1 = SettableFuture.create();
                if (groupId != null) {
                    if (isGroupExistsOnDevice(nodeIdent, groupId)) {
                        future1 = JdkFutureAdapters.listenInPoolThread(provider.getSalFlowService().addFlow(builder.build()));
                    } else {
                        Future<? extends RpcResult<?>> groupFuture = pushDependentGroup(nodeIdent, groupId);
                        Futures.addCallback(JdkFutureAdapters.listenInPoolThread(groupFuture),
