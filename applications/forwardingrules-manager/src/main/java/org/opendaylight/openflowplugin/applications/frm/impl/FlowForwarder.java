...
public class FlowForwarder extends AbstractListeningCommiter<Flow> {
                ...
                nodeConfigurator.enqueueJob(nodeId.getValue(), getFlowId(new FlowRef(identifier)), () -> {
                    ...
                    if (groupId != null) {
                        ...
                        if (isGroupExistsOnDevice(nodeIdent, groupId, provider)) {
                            ...
                            future = JdkFutureAdapters.listenInPoolThread(provider.getSalFlowService()
    ...
    private ListenableFuture<RpcResult<AddGroupOutput>> pushDependentGroup(
        ...
        try (ReadOnlyTransaction readTransaction = provider.getReadTransaction()) {
            ...
            if (group.isPresent()) {
                ...
                resultFuture = JdkFutureAdapters.listenInPoolThread(this.provider.getSalGroupService()
                        .addGroup(addGroupInput));
            } else {
    ...
    private final class AddFlowCallBack implements FutureCallback<RpcResult<AddGroupOutput>> {
        ...
        public void onSuccess(RpcResult<AddGroupOutput> rpcResult) {
            if (rpcResult.isSuccessful()) {
                ...
                future = JdkFutureAdapters.listenInPoolThread(provider.getSalFlowService().addFlow(addFlowInput));
        ...
        @Override
        public void onFailure(Throwable throwable) {
            JdkFutures.addErrorLogging(future, LOG, "Service call for adding flow"
                    + getFlowId(addFlowInput.getFlowRef()) + "failed for node" +  "with error" + throwable.getCause());
    ...
    private final class UpdateFlowCallBack implements FutureCallback<RpcResult<AddGroupOutput>> {
        ...
        public void onSuccess(RpcResult<AddGroupOutput> rpcResult) {
            if (rpcResult.isSuccessful()) {
                provider.getDevicesGroupRegistry().storeGroup(nodeId, groupId);
                future = JdkFutureAdapters.listenInPoolThread(provider.getSalFlowService().updateFlow(updateFlowInput));
        ...
        @Override
        public void onFailure(Throwable throwable) {
            JdkFutures.addErrorLogging(future, LOG, "Service call for updating flow"
                    + getFlowId(updateFlowInput.getFlowRef()) + "failed for node" +  "with error"
                    + throwable.getCause());
