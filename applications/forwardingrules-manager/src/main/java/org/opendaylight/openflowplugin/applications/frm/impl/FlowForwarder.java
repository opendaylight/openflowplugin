...
public class FlowForwarder extends AbstractListeningCommiter<Flow> {
    ...
    public void update(final InstanceIdentifier<Flow> identifier, final Flow original, final Flow update,
        ...
        if (tableIdValidationPrecondition(tableKey, update)) {
            ...
            nodeConfigurator.enqueueJob(nodeIdent.firstKeyOf(Node.class, NodeKey.class).getId().getValue(), ()-> {
                ...
                Future<RpcResult<UpdateFlowOutput>> future1 = Futures.immediateFuture(null);
                if (groupId != null) {
    ...
    public Future<? extends RpcResult<?>> add(final InstanceIdentifier<Flow> identifier, final Flow addDataObj,
        ...
        Future<? extends RpcResult<?>> future = Futures.immediateFuture(null);
        final TableKey tableKey = identifier.firstKeyOf(Table.class, TableKey.class);
        if (tableIdValidationPrecondition(tableKey, addDataObj)) {
            ...
            nodeConfigurator.enqueueJob(nodeIdent.firstKeyOf(Node.class, NodeKey.class).getId().getValue(), () -> {
                ...
                Future<RpcResult<AddFlowOutput>> future1 = Futures.immediateFuture(null);
                if (groupId != null) {
                    if (isGroupExistsOnDevice(nodeIdent, groupId)) {
