    ...
    private Long isFlowDepenendsOnGroup(final Flow flow) {
        ...
        if (flow.getInstructions() != null) {
            ...
            for (Instruction instruction : instructions) {
                ...
                if (instruction.getInstruction().getImplementedInterface().getName().equals(Actiontype.APPLY_ACTION)) {
                    actions = ((ApplyActionsCase) (instruction.getInstruction())).getApplyActions().getAction();
                }
                for (Action action : actions) {
                    if (action.getAction().getImplementedInterface().getName().equals(Actiontype.GROUP_ACTION)) {
