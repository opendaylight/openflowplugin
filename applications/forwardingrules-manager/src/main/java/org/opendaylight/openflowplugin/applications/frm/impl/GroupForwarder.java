...
public class GroupForwarder extends AbstractListeningCommiter<Group> {
    ...
    public void remove(final InstanceIdentifier<Group> identifier, final Group removeDataObj,
        ...
        Future<RpcResult<RemoveGroupOutput>> future = Futures.immediateFuture(null);
        future = jobCoordinator.enqueueJob(nodeIdent.firstKeyOf(Node.class, NodeKey.class).getId().getValue(), ()-> {
    ...
    public void update(final InstanceIdentifier<Group> identifier, final Group original, final Group update,
        ...
        Future<RpcResult<UpdateGroupOutput>> future = Futures.immediateFuture(null);
        future = jobCoordinator.enqueueJob(nodeIdent.firstKeyOf(Node.class, NodeKey.class).getId().getValue(), ()->{
    ...
    public Future<RpcResult<AddGroupOutput>> add(final InstanceIdentifier<Group> identifier, final Group addDataObj,
        ...
        Future<RpcResult<AddGroupOutput>> future = jobCoordinator.enqueueJob(nodeIdent.firstKeyOf(Node.class, NodeKey.class).getId().getValue(),()->{
            final ListenableFuture<RpcResult<AddGroupOutput>> resultFuture;
            resultFuture = JdkFutureAdapters.listenInPoolThread(this.provider.getSalGroupService().addGroup(addGroupInput));
            Futures.addCallback(JdkFutureAdapters.listenInPoolThread(resultFuture),
