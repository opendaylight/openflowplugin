...
public final class BulkOMaticUtils {
    ...
    public static String ipIntToStr(final int number) {
        return new StringBuilder()
            .append(number >> 24 & 0xFF).append(".")
            .append(number >> 16 & 0xFF).append(".")
            .append(number >> 8 & 0xFF).append(".")
            .append(number & 0xFF).append("/32")
            .toString();
    }
    ...
    public static Flow buildFlow(final short tableId, final String flowId, final Match match) {
    ...
    public static @NonNull WithKey<Flow, FlowKey> getFlowInstanceIdentifier(final short tableId, final String flowId,
            final String dpId) {
    ...
    public static @NonNull WithKey<Node, NodeKey> getFlowCapableNodeId(final String dpId) {
        return DataObjectIdentifier.builder(Nodes.class).child(Node.class, new NodeKey(new NodeId(dpId))).build();
    }

    public static @NonNull WithKey<Table, TableKey> getTableId(final short tableId, final String dpId) {
        return DataObjectIdentifier.builder(Nodes.class)
            .child(Node.class, new NodeKey(new NodeId(dpId)))
            .augmentation(FlowCapableNode.class)
            .child(Table.class, new TableKey(Uint8.valueOf(tableId)))
    ...
    public static @NonNull WithKey<Flow, FlowKey> getFlowId(final DataObjectIdentifier<Table> tablePath,
            final String flowId) {
        return tablePath.toBuilder().child(Flow.class, new FlowKey(new FlowId(flowId))).build();
    }
}
