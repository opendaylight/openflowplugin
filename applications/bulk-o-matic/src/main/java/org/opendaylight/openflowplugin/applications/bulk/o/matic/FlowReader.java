...
public class FlowReader implements Runnable, FlowCounterMBean {
    ...
    private AtomicInteger readOpStatus = new AtomicInteger(FlowCounter.OperationStatus.INIT.ordinal());
    ...
    private long readFlowsX(Integer dpnCount, Integer flowsPerDPN, boolean verbose) {
        readOpStatus.set(FlowCounter.OperationStatus.INPROGRESS.ordinal());
        for (int i = 1; i <= dpnCount; i++) {
            String dpId = "openflow:" + i;
            for (int j = 0; j < flowsPerDPN; j++) {
                ...
                try {
                    ...
                    readOpStatus.set(FlowCounter.OperationStatus.FAILURE.ordinal());
                    LOG.error(e.getMessage(), e);
                }
            }
        }
        if(readOpStatus.get() != FlowCounter.OperationStatus.FAILURE.ordinal()) {
            readOpStatus.set(FlowCounter.OperationStatus.SUCCESS.ordinal());
        }
