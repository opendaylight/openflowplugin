...
class FlowWriterDirectOFRpc {
    ...
    void rpcFlowAdd(boolean direct, String nodeId, int flowsPerNode, int batchSize){
        fetchAllNodes()
    ...
    void rpcFlowAddAll(boolean direct, int flowsPerNode, int batchSize){
        fetchAllNodes().forEach(pushFlow(direct, flowsPerNode, batchSize));
    }

    private Consumer<String> pushFlow(boolean direct, int flowsPerNode, int batchSize) {
    ...
    private class FlowRPCHandlerTask implements Runnable {
        ...
        private Stream<Runnable> createJobs(final String nodeId, final int flowsPerNode) {
            return direct ? prepareFlows(nodeId, flowsPerNode).map(flow -> {
