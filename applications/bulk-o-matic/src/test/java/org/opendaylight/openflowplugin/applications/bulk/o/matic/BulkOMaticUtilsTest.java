...
public class BulkOMaticUtilsTest {
    ...
    public void testBuildFlow() throws Exception {
        final Match match = BulkOMaticUtils.getMatch(0xffffffff);
        final Flow flow = BulkOMaticUtils.buildFlow((short)1, "500", match);
        Assert.assertEquals("500" ,flow.getId().getValue());
        Assert.assertEquals(Short.valueOf("1") ,flow.getTableId());
    }
