...
public class ReconciliationManagerImpl implements ReconciliationManager, ReconciliationFrameworkEvent {
    ...
    public NotificationRegistration registerService(ReconciliationNotificationListener reconciliationTask) {
        ...
        ReconciliationServiceDelegate registration = new ReconciliationServiceDelegate(this, reconciliationTask);
        return registration;
    }

    @Override
    public void unregisterService(ReconciliationNotificationListener service) {
        LOG.debug("Unregistered service {} from reconciliation framework", service.getResultState());
        registeredServices.computeIfPresent(service.getPriority(),(priority, services) -> services).remove(service);
    }

    // TODO This method has to be called when cancelReconcile notification is
    // received from OFP
