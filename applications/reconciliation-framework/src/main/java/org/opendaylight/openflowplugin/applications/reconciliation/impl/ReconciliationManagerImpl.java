...
import org.opendaylight.openflowplugin.applications.reconciliation.util.ReconciliationUtil;
...
public class ReconciliationManagerImpl implements ReconciliationManager, ReconciliationFrameworkEvent {
    ...
    private ResultState resultState = ResultState.DONOTHING;
    ...
    public NotificationRegistration registerService(ReconciliationNotificationListener reconciliationTask) {
        ...
        ReconciliationServiceDelegate registration = new ReconciliationServiceDelegate(reconciliationTask, () -> {
            ...
            resultState = ReconciliationUtil.decideResultState(registeredServices.values().stream().flatMap(List::stream).collect(Collectors.toList())
                    .stream().map(ReconciliationNotificationListener::getResultState).collect(Collectors.toList()));
        });
        resultState = ReconciliationUtil.decideResultState(registeredServices.values().stream().flatMap(List::stream).collect(Collectors.toList())
                .stream().map(ReconciliationNotificationListener::getResultState).collect(Collectors.toList()));
        return registration;
