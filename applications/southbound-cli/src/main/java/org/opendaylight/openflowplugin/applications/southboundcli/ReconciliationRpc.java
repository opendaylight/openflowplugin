...
import org.opendaylight.openflowplugin.applications.frm.impl.FrmReconciliationRpc;
...
public class ReconciliationRpc implements AutoCloseable {
    private static final Logger LOG = LoggerFactory.getLogger(ReconciliationRpc.class);

    private final DataBroker broker;
    private final FrmReconciliationRpc frmReconciliationRpc;
    ...
    public ReconciliationRpc(final DataBroker broker, final FrmReconciliationRpc frmReconciliationRpc,
                                     final AlarmAgent alarmAgent, final NodeListener nodeListener,
                                     final FlowGroupCacheManager flowGroupCacheManager) {
        this.broker = broker;
        this.frmReconciliationRpc = frmReconciliationRpc;
    ...
    private ListenableFuture<RpcResult<ReconcileOutput>> reconcile(final ReconcileInput input) {
    ...
    public ClassToInstanceMap<Rpc<?,?>> getRpcClassToInstanceMap() {
        return ImmutableClassToInstanceMap.<Rpc<?, ?>>builder()
            .put(Reconcile.class, this::reconcile)
            .build();
    }
    ...
    private final class ReconciliationTask implements Runnable {
        ...
        public void run() {
            ...
            Future<RpcResult<ReconcileNodeOutput>> reconOutput = frmReconciliationRpc.getRpcClassToInstanceMap()
                .getInstance(ReconcileNode.class).invoke(reconInput);
            try {
