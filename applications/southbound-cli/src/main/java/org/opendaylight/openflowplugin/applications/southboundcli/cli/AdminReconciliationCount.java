...
public class AdminReconciliationCount extends OsgiCommandSupport {
    ...
    protected Object doExecute() throws Exception {
        ...
        if (result.isEmpty()) {
            session.getConsole().println("Admin Reconciliation is not triggered for any of the node");
        } else {
            StringBuilder stringBuilder = new StringBuilder();
            final Formatter formatter = new Formatter(stringBuilder);
            session.getConsole().println(getAdminReconcileCountHeaderOutput());
            session.getConsole().println("--------------------------------------------------------------------------");
            for (ReconcileCounter reconcile : result) {
                session.getConsole().println(formatter.format("%-15s %3s %-15s %7s %-20s %n",
                        reconcile.getNodeId(), "", reconcile.getCount(), "",
    ...
    private String getAdminReconcileCountHeaderOutput() {
        final Formatter formatter = new Formatter();
        String header = formatter.format("%-15s %3s %-15s %3s %-15s %n", "NodeId", "", "AdminReconcileCount", "",
                "LastReconcileTime").toString();
        formatter.close();
        return header;
    }
}
