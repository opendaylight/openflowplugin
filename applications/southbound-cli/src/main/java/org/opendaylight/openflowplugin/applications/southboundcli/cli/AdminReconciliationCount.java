...
public class AdminReconciliationCount extends OsgiCommandSupport {
    ...
    protected Object doExecute() throws Exception {
        ...
        if (result.isEmpty()) {
            ...
            for (ReconcileCounter reconcile : result) {
                session.getConsole().println(formatter.format("%-15s %3s %-15s %7s %-15s %n",
                        reconcile.getNodeId(), "", reconcile.getAdminReconciliationCount(), "", reconcile.getReconciliationRequestTime()).toString());
    ...
    private String getAllLocalNodesHeaderOutput() {
        ...
        String header = formatter.format("%-15s %3s %-15s %3s %-15s %n", "NodeId", "", "AdminReconcileCount", "", "LastReconcileTime").toString();
        formatter.close();
        return header;
    }
}
