...
public class GetFlowGroupCacheProvider extends OsgiCommandSupport {
    ...
    protected Object doExecute() throws Exception {
        ...
        for (FlowGroupInfo cache : flowGroupCacheList) {
            System.out.println(fmt.format("%-10s %1s %-8s %1s %-23s %1s %-60s", cache.getDescription(), "",
                cache.getStatus(), "", cache.getTime(), "", cache.getId()).toString());
            sb.setLength(0);
        }
        fmt.close();
        return null;
    ...
    private void printAllNodes() {
        final Map<String, Collection<FlowGroupInfo>> allGroupInfos = flowGroupCacheManager.getAllNodesFlowGroupCache();
        ...
        System.out.println(getLineSeparator());
        for (Map.Entry<String, Collection<FlowGroupInfo>> cacheEntry : allGroupInfos.entrySet()) {
            ...
            synchronized (flowGroupCacheList) {
                for (FlowGroupInfo cache : flowGroupCacheList) {
                    System.out.println(fmt.format("%-15s %1s %-10s %1s %-8s %1s %-21s %1s %-60s", node, "",
                        cache.getDescription(), "", cache.getStatus(), "", cache.getTime(), "",
