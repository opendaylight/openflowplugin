...
public class GetFlowGroupCacheProvider extends OsgiCommandSupport {
    ...
    protected Object doExecute() {
        ...
        FlowGroupInfoHistory history = histories.getFlowGroupHistory(new NodeId(nodeId));
        if (history == null) {
            session.getConsole().println("No node available for this NodeID");
            return null;
        }
        final Collection<FlowGroupInfo> entries = history.entries();
        if (entries.isEmpty()) {
    ...
    private void printAllNodes() {
        ...
        for (Entry<NodeId, FlowGroupInfoHistory> entry : allHistories.entrySet()) {
            ...
            String node = temp[1];
            entry.getValue().entries().forEach(info -> {
