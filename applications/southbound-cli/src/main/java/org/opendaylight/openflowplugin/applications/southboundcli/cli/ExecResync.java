    ...
    protected Object doExecute() throws Exception {
        ...
        for(String dpn : dpnId) {
            ...
            BigInteger dpid = new BigInteger(dpn);
                final NodeKey key = new NodeKey(new NodeId(dpn));
                InstanceIdentifier<Node> path = InstanceIdentifier.builder(Nodes.class).child(Node.class, key).build();
                InitReconciliationInputBuilder initReconInput = new InitReconciliationInputBuilder();
                initReconInput.setDpnId(dpid);
                initReconInput.setNode(new NodeRef(path));
                Future<RpcResult<InitReconciliationOutput>> initReconOutput = frmReconciliationService.initReconciliation(initReconInput.build());
            try {
