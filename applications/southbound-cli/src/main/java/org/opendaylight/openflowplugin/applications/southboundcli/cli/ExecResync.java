...
public class ExecResync extends OsgiCommandSupport {
    ...
    @Argument(name = "nodeId", description = "The NODE Id", required = false, multiValued = true)
    List<Long> nodeIds;
    ...
    protected Object doExecute() throws Exception {
        ...
        for (BigInteger node : nodes) {
            LOG.debug("Triggering admin resync for DPN {}", node);
            ...
            try {
                ...
                if (rpcResult.isSuccessful()) {
                    System.out.println("Resync successfully completed for NODE " + node);
                    LOG.info("Resync successfully completed for NODE {}", node);
                } else {
                    System.out.println("Resync failed for NODE " + node + ", please check logs");
                    LOG.error("Resync failed for NODE {} with error {}", node, rpcResult.getErrors());
                }
            } catch (ExecutionException e) {
                LOG.error("Error occurred while invoking execresync RPC for NODE {}", node, e);
            }
        }
        return null;
    }
