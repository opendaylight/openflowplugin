...
public class FlowCapableTopologyProvider extends AbstractBindingAwareProvider implements AutoCloseable {
    ...
    private void unregisterListener(final InventoryDataChangeListener inventoryDataChangeListener) {
        if (inventoryDataChangeListener != null) {
            try {
                inventoryDataChangeListener.close();
            } catch (Exception e) {
                LOG.error("Failed to close listener registration", e);
            }
        }
