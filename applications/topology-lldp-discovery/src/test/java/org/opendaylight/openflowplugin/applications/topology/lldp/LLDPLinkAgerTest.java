...
public class LLDPLinkAgerTest {
    ...
    @Test
    public void testLLDPAgingTaskNotOwned() throws InterruptedException {
        Mockito.when(eos.getOwnershipState(Mockito.any(Entity.class))).thenReturn(Optional.of(new EntityOwnershipState(false, true)));
        lldpLinkAger.put(link);
        Thread.sleep(SLEEP);
        verify(notificationService, never()).publish(Matchers.any(LinkRemoved.class));
    }
}