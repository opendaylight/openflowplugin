...
public class OldNotifModule extends AbstractOldNotifModule {
    ...
    public java.lang.AutoCloseable createInstance() {
        ...
        final OldNotifProvider provider = new OldNotifProviderImpl(config);
        provider.start();
//        statisticsManagerProvider.start(getNotificationServiceDependency(), getRpcRegistryDependency());
    ...
    private OldNotifProviderConfig createConfig() {
        ...
        if (settings.getPortSupport() != null) {
            builder.setPortSupport(settings.getPortSupport());
        } else {
            LOG.warn(LOAD_SETTINGS_XML_FAIL + " PortSupport value is set to {} ", DEFAULT_ITEM_NOTIF_ALLOWED);
            builder.setPortSupport(DEFAULT_ITEM_NOTIF_ALLOWED);
        }
