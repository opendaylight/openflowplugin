...
public class Jbench {
    ...
    private void initializeFakeSwitches() {
        ...
        for (int index = 0; index < this.numSwitches; index++) {
            fakeSwitches[index] = new FakeSwitch(this);
    ...
    private void displayPerLoopResults(int loop) {
        ...
        for (int index = 0; index < this.numSwitches; index++) {
            result = String.valueOf((fakeSwitches[index].getFlowModCount() * 1000.0) / msPerTest);
            flowsPerSecLoop += (fakeSwitches[index].getFlowModCount() * 1000.0) / msPerTest;
