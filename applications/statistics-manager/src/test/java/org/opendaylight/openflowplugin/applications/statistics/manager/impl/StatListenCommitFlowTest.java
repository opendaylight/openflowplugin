...
public class StatListenCommitFlowTest {
  ...
  public void testStatsFlowCommitAllWithAlienSystemFlowId() throws InvocationTargetException {
    ...
    InstanceIdentifier<Node> nodeIdent = InstanceIdentifier.create(Nodes.class)
    ...
    InstanceIdentifier<Table> path = InstanceIdentifier.create(Nodes.class).child(Node.class, new NodeKey(flowsStatisticsUpdate.getId()))
    ...
    doReturn(Futures.immediateCheckedFuture(expected)).when(mockReadWriteTx).read(LogicalDatastoreType.OPERATIONAL, fNodeIdent);
    ...
    doReturn(Futures.immediateCheckedFuture(expected1)).when(mockReadTx).read(LogicalDatastoreType.CONFIGURATION, path);
  ...
  private FlowAndStatisticsMapList createFlowAndStatisticsMapList() {
    return new FlowAndStatisticsMapListBuilder().setFlowName("testFlow").setFlowId(
         new FlowId("F1")).setTableId(tableKey.getId()).setMatch(new MatchBuilder().build())
        .setPriority(2).setCookie(new FlowCookie(new java.math.BigInteger("18446744073709551615")))
        .build();
  }
  ...
  private FlowCapableNode createFlowCapableNode(String flowId){
	...
	flowList.add(new FlowBuilder().setFlowName("testFlow").setId(new org.opendaylight.yang.gen.v1.urn.opendaylight.flow.inventory.rev130819.FlowId("F1")).build());
	List<FlowHashIdMap> flowHashIdMapList = new ArrayList<FlowHashIdMap>();
	flowHashIdMapList.add(new FlowHashIdMapBuilder().setFlowId(new org.opendaylight.yang.gen.v1.urn.opendaylight.flow.inventory.rev130819.FlowId(flowId))
	    .setHash("TestFlow").setKey(new FlowHashIdMapKey("FM1")).build());
	tableList.add(new TableBuilder().setFlow(flowList).setId((short) 12).setKey(tableKey).addAugmentation(FlowHashIdMapping.class, new FlowHashIdMappingBuilder().setFlowHashIdMap(flowHashIdMapList)
	    .build()).build());
