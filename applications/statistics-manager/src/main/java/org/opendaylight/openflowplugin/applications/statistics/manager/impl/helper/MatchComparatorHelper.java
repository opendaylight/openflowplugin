...
public class MatchComparatorHelper {
    ...
    private static final byte[] convertIpv6ArbitraryMaskToByteArray ( Ipv6ArbitraryMask mask) {
        ...
        try {
            ...
            LOG.error("Failed to recognize the host while converting IPv6 Arbitrary bitmask ", e);
        }
        byte[] bytes = maskInIpv6Format.getAddress();
        return bytes;
    }
    ...
    private static byte[] convertIpv6ToBytes(String ipv6Address) {
        byte[] bytes = extractIpv6CanonicalForm(ipv6Address).getAddress();
        return bytes;
    }
