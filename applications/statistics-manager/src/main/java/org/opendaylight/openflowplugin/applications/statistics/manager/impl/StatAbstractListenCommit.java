...
public abstract class StatAbstractListenCommit<T extends DataObject, N extends NotificationListener>
    ...
    private ListenerRegistration<DataTreeChangeListener<T>> listenerRegistration;
    ...
    private final DataTreeIdentifier<T> treeId =
            new DataTreeIdentifier<>(LogicalDatastoreType.OPERATIONAL, getWildCardedRegistrationPath());
    ...
    public StatAbstractListenCommit(final StatisticsManager manager, final DataBroker db,
        ...
        try{
            listenerRegistration =  looper.loopUntilNoException(new Callable<ListenerRegistration<DataTreeChangeListener<T>>>() {
                    @Override
                    public ListenerRegistration<DataTreeChangeListener<T>> call() throws Exception {
                        return db.registerDataTreeChangeListener(treeId, StatAbstractListenCommit.this);
                    }
                });
        }catch(final Exception ex){
            LOG.debug(" StatAbstractListenCommit DataChange listener registration fail ..{}", ex.getMessage());
