...
public class StorageWrapper {
    ...
    private StorageWrapper() {
        session = ConnectionManager.getSession("subscribercontrol");
    }
    ...
    public void addSubscriber(Subsriber subsriber) {
        LOG.info("Adding subscriber filter:"+ subsriber.toString());
        String query = "insert into subscribercontrol.subscriberfilterswl(filterid ,subprofileid, srcipprefix ,vni ,portstart ,portend ,priority ,displayame) values (?,?,?,?,?,?,?,?)";
        PreparedStatement prepared = session.prepare(query);
        BoundStatement bound = prepared.bind(String.valueOf(subsriber.getId()),subsriber.getProfileId(),String.valueOf(subsriber.getIpv4Prefix()),subsriber.getVni(),subsriber.getPortStart(),subsriber.getPortEnd(),subsriber.getPriority(),subsriber.getDisplayName());
        session.execute(bound);
    }

    public void removeSubscriber(Subsriber subsriber) {
        LOG.info("Removing subscriber filter:" + subsriber.toString());
