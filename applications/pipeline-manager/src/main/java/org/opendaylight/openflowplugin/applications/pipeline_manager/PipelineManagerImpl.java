...
public class PipelineManagerImpl implements DataChangeListener, PipelineManager {
    ...
    public PipelineManagerImpl(DataBroker dataBroker) {
    ...
    private void createPipeline(Node node) {
        ...
        for (Table table : tableList) {
            List<Short> nextIds = getNextTables(table);
            if (nextIds.isEmpty())
                break;
    ...
    private List<Short> getNextTables(Table table) {
        for (TableFeatureProperties tableFeatureProperties : getTableFeatureProperties(table)) {
            if (tableFeatureProperties.getTableFeaturePropType() instanceof NextTable) {
                NextTable nextTable = (NextTable) tableFeatureProperties.getTableFeaturePropType();
                return nextTable.getTables().getTableIds();
            }
        }
        return Collections.emptyList();
    }
