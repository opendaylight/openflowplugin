...
public class ArbitratorReconciliationManagerImpl implements ClusteredDataTreeChangeListener<ArbitratorConfig>,
    ...
    public ListenableFuture<RpcResult<CommitActiveBundleOutput>> commitActiveBundle(
        ...
        BundleId bundleId = bundleIdMap.get(input.getNodeId()).getBundleId();
    ...
    public ListenableFuture<RpcResult<GetActiveBundleOutput>> getActiveBundle(GetActiveBundleInput input) {
        ...
        if (bundleDetails != null) {
            SettableFuture<RpcResult<GetActiveBundleOutput>> rpcResult = SettableFuture.create();
            try {
        ...
        return RpcResultBuilder.success((GetActiveBundleOutput) null).buildFuture();
    }
    ...
    private class ArbitratorReconciliationTask implements Callable<Boolean> {
        ...
        public Boolean call() {
            ...
            Future<RpcResult<ControlBundleOutput>> closeBundle = salBundleService.controlBundle(closeBundleInput);
            ...
            try {
                if (addBundleMessagesFuture.get().isSuccessful()) {
                    bundleIdMap.put(new BigInteger(node), new BundleDetails(bundleIdValue, Futures.immediateFuture(null)));
