...
@Singleton
public class ArbitratorReconciliationRpcs implements ReconciliationNotificationListener, AutoCloseable {

    private static final Logger LOG = LoggerFactory.getLogger(ArbitratorReconciliationRpcs.class);
    ...
    @Inject
    public ArbitratorReconciliationRpcs(final ReconciliationManager reconciliationManager,
        ...
        this.addBundleMessagesRpc = rpcRegistry.getRpc(AddBundleMessages.class);
        this.controlBundleRpc = rpcRegistry.getRpc(ControlBundle.class);
    ...
    @VisibleForTesting
    ListenableFuture<RpcResult<CommitActiveBundleOutput>> commitActiveBundle(
        ...
        if (bundleIdMap.containsKey(nodeId)) {
            ...
            if (bundleId != null) {
                final ControlBundleInput commitBundleInput = new ControlBundleInputBuilder()
                        ...
                        .build();
                ListenableFuture<RpcResult<ControlBundleOutput>> rpcResult = controlBundleRpc.invoke(commitBundleInput);
    ...
    @VisibleForTesting
    ListenableFuture<RpcResult<GetActiveBundleOutput>> getActiveBundle(final GetActiveBundleInput input) {
        Uint64 nodeId = input.getNodeId();
    ...
    private class ArbitratorReconciliationTask implements Callable<Boolean> {
        ...
        public Boolean call() {
            ...
            ListenableFuture<RpcResult<ControlBundleOutput>> closeBundle = controlBundleRpc.invoke(closeBundleInput);

            ListenableFuture<RpcResult<ControlBundleOutput>> openBundleMessagesFuture = Futures
                    .transformAsync(closeBundle, rpcResult -> controlBundleRpc.invoke(openBundleInput),
                        MoreExecutors.directExecutor());
            ...
            ListenableFuture<RpcResult<AddBundleMessagesOutput>> addBundleMessagesFuture = Futures
                    .transformAsync(openBundleMessagesFuture, rpcResult -> {
                        if (rpcResult.isSuccessful()) {
                            return addBundleMessagesRpc.invoke(addBundleMessagesInput);
                        }
    ...
    private void deregisterRpc(final DeviceInfo node) {
        ...
        final var registrationToClose = rpcRegistrations.remove(node.getNodeId().getValue());
