...
public class SimplifiedOperationalListenerTest {
    ...
    public void setUp() throws Exception {
        ...
        Mockito.when(deviceManager.getReconciliationRegistry()).thenReturn(reconciliationRegistry);
    }
    ...
    public void testOnDataTreeChangeAddSyncup() throws Exception {
        ...
        Mockito.verify(deviceManager).onDeviceConnected(NODE_ID);
    ...
    public void testOnDataTreeChangedSyncupDeletePhysical() throws Exception {
        ...
        Mockito.when(deviceManager.isDeviceMastered(NODE_ID)).thenReturn(true);
        Mockito.when(reconciliationRegistry.isRegisteredForReconcile(NODE_ID)).thenReturn(false);

        nodeListenerOperational.onDataTreeChanged(Collections.singleton(dataTreeModification));

        Mockito.verify(deviceManager).onDeviceDisconnected(NODE_ID);
        Mockito.verifyZeroInteractions(reactor);
    }
    ...
    public void testOnDataTreeChangedSlave() {
        Mockito.when(deviceManager.isDeviceMastered(NODE_ID)).thenReturn(false);
    ...
    private void prepareFreshOperational(final boolean afterRegistration) throws ParseException {
        ...
        if (afterRegistration) {
            ...
            Mockito.when(reconciliationRegistry.getReconcileRegistrationTimestamp(NODE_ID)).thenReturn(simpleDateFormat.parse(timestampBefore));
        } else {
            Mockito.when(snapshotGatheringStatusEnd.getEnd().getValue()).thenReturn(timestampBefore);
            Mockito.when(reconciliationRegistry.getReconcileRegistrationTimestamp(NODE_ID)).thenReturn(simpleDateFormat.parse(timestampAfter));
        }
    }
