...
public class SimplifiedOperationalListenerTest {
    ...
    public void testOnDataTreeChangedSyncupDeletePhysical() throws Exception {
        ...
        Mockito.when(deviceManager.isRegisteredForReconcile(NODE_ID)).thenReturn(false);
    ...
    private void prepareFreshOperational(final boolean afterRegistration) throws ParseException {
        ...
        if (afterRegistration) {
            ...
            Mockito.when(deviceManager.getReconcileRegistrationTimestamp(NODE_ID)).thenReturn(simpleDateFormat.parse(timestampBefore));
        } else {
            Mockito.when(snapshotGatheringStatusEnd.getEnd().getValue()).thenReturn(timestampBefore);
            Mockito.when(deviceManager.getReconcileRegistrationTimestamp(NODE_ID)).thenReturn(simpleDateFormat.parse(timestampAfter));
        }
    }
