...
public class SimplifiedOperationalListener extends AbstractFrmSyncListener<Node> {
    ...
    private boolean isRegisteredAndConsistentForReconcile(DataTreeModification<Node> modification) {
        ...
        if (!deviceManager.getReconciliationRegistry().isRegisteredForReconcile(nodeId)) {
            return false;
        }
        ...
        try {
            Date timestampOfRegistration = deviceManager.getReconciliationRegistry().getReconcileRegistrationTimestamp(nodeId);
