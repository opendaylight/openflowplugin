...
public class SyncReactorFutureZipDecorator extends SyncReactorFutureDecorator {
    ...
    private boolean updateCompressionState(final InstanceIdentifier<FlowCapableNode> flowcapableNodePath,
        ...
        if (previousEntry != null && syncupEntry.isOptimizedConfigChange() && previousEntry.isOptimizedConfigChange()) {
            putOptimizedConfigDelta(flowcapableNodePath, syncupEntry, previousEntry);
        } else {
    ...
    private void putOptimizedConfigDelta(InstanceIdentifier<FlowCapableNode> flowcapableNodePath, SyncupEntry actual,
