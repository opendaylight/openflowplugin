...
public class SyncReactorFutureZipDecorator extends SyncReactorFutureDecorator {
    ...
    /**
     * If there is something in compression queue for the device and new configuration is coming,
     * update its zip queue entry. If there is nothing or operational data are coming, create/replace
     * zip queue its entry with new values.
     */
    protected boolean updateCompressionState(final InstanceIdentifier<FlowCapableNode> flowcapableNodePath,
        ...
        final ZipQueueEntry previous = compressionQueue.get(flowcapableNodePath);

        if (previous != null && dsType == LogicalDatastoreType.CONFIGURATION) {
