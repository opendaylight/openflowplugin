...
/**
 * this implementation listens to DS/operational and reads DS/config counterpart when notified
 */
public class NodeListenerOperationalImpl extends AbstractNodeListener {

    private static final Logger LOG = LoggerFactory.getLogger(NodeListenerOperationalImpl.class);

    public NodeListenerOperationalImpl(SyncReactor reactor, DataBroker dataBroker, final SemaphoreKeeperImpl<NodeId> semaphoreKeeper) {
        ...
        return new AsyncFunction<Optional<FlowCapableNode>, RpcResult<Void>>() {
            ...
            public ListenableFuture<RpcResult<Void>> apply(final Optional<FlowCapableNode> input) throws Exception {
                ...
                if (input.isPresent() && triggerModification.isPresent()) {
                    nextResult = reactor.syncup(nodePath, input.get(), triggerModification.get());
                } else {
