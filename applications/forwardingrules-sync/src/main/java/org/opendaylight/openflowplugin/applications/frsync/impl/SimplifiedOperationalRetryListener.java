...
public class SimplifiedOperationalRetryListener extends SimplifiedOperationalListener {
    ...
    protected boolean inRetry(DataTreeModification<Node> modification) {
        ...
        if (isConsistentForRetry(modification)) {
     ...
     */
    private boolean isConsistentForRetry(DataTreeModification<Node> modification) {
        final NodeId nodeId = PathUtil.digNodeId(modification.getRootPath().getRootIdentifier());
        ...
        if (!retryRegistry.isRegistered(nodeId)) {
            LOG.trace("Device not registered for fresh operational [{}]", nodeId.getValue());
            return false;
        }
        ...
        try {
            Map<NodeId, Date> registration = retryRegistry.getRegistration();
