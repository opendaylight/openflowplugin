...
public class TransactionChainManager implements TransactionChainListener, AutoCloseable {
    ...
    @GuardedBy("txLock")
    public boolean submitTransaction(boolean doAsync) {
        synchronized (txLock) {
            if (!submitIsEnabled) {
            ...
            if (initCommit || !doAsync) {
                try {
                    submitFuture.get(5L, TimeUnit.SECONDS);
                } catch (InterruptedException | ExecutionException | TimeoutException ex) {
                    LOG.error("Exception during INITIAL({}) || doAsync({}) transaction submitting. ",
                            initCommit, doAsync, ex);
                    return false;
                }
                initCommit = false;
                return true;
