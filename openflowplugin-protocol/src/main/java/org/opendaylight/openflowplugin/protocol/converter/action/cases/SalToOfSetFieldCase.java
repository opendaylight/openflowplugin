...
public class SalToOfSetFieldCase extends ConvertorCase<SetFieldCase, Action, ActionConverterData> {
    ...
    @Nonnull
    @Override
    public Optional<Action> process(@Nonnull final SetFieldCase source, final ActionConverterData data, ConverterExecutor converterExecutor, final ExtensionConverterProvider extensionConverterProvider) {
        final short version = data.getVersion();
        final SetFieldActionBuilder setFieldBuilder = new SetFieldActionBuilder();

        MatchReactor.getInstance().convert(source.getSetField(), version, setFieldBuilder, converterExecutor, extensionConverterProvider);
