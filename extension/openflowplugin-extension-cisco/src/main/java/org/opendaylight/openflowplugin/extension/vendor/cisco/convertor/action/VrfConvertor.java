...
import org.opendaylight.yang.gen.v1.urn.opendaylight.openflowplugin.extension.cof.hi.action.rev141010.CofActionVrfGrouping;
import org.opendaylight.yang.gen.v1.urn.opendaylight.openflowplugin.extension.cof.hi.action.rev141010.CofAtVrfType;
import org.opendaylight.yang.gen.v1.urn.opendaylight.openflowplugin.extension.cof.hi.action.rev141010.VrfExtra;
import org.opendaylight.yang.gen.v1.urn.opendaylight.openflowplugin.extension.cof.hi.action.rev141010.VrfName;
import org.opendaylight.yang.gen.v1.urn.opendaylight.openflowplugin.extension.cof.hi.action.rev141010.VrfVpnId;
import org.opendaylight.yang.gen.v1.urn.opendaylight.openflowplugin.extension.cof.hi.action.rev141010.cof.action.vrf.grouping.ActionVrfHi;
import org.opendaylight.yang.gen.v1.urn.opendaylight.openflowplugin.extension.cof.hi.action.rev141010.cof.action.vrf.grouping.ActionVrfHiBuilder;
...
public class VrfConvertor implements
    ...
    public org.opendaylight.yang.gen.v1.urn.opendaylight.action.types.rev131112.action.Action convert(Action input, ActionPath path) {
        ActionVrf actionVrfHi = ((OfjCofActionVrf) input.getActionChoice()).getActionVrf();
        ActionVrfHiBuilder actionHiBld = new ActionVrfHiBuilder();
        actionHiBld.setVpnType(CofAtVrfType.forValue(actionVrfHi.getVpnType()));
        VrfExtra vrfExtra = null;
        switch (actionHiBld.getVpnType()) {
            case VPNID:
                vrfExtra = new VrfExtra(new VrfVpnId(actionVrfHi.getVrfExtra()));
                break;
            case NAME:
                vrfExtra = new VrfExtra(new VrfName(new String(actionVrfHi.getVrfExtra())));
                break;
            default:
    ...
    public Action convert(org.opendaylight.yang.gen.v1.urn.opendaylight.action.types.rev131112.action.Action cofActionArg) {
        ...
        switch (actionVrfHi.getVpnType()) {
            case VPNID:
                ...
                break;
            case NAME:
                Object dummy = actionVrfHi.getVrfExtra();
                dummy = actionVrfHi.getVrfExtra().getVrfName().getValue();
                dummy = actionVrfHi.getVrfExtra().getVrfName().getValue().getBytes();
        ...
        OfjCofActionVrfBuilder builder = new OfjCofActionVrfBuilder().setActionVrf(actionVrfBld.build());
