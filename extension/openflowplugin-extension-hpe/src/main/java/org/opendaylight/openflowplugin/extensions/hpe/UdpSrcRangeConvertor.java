...
import org.opendaylight.yang.gen.v1.urn.opendaylight.openflowplugin.extensions.hpe.rev151112.HpeUdpSrcRangeKey;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.openflowplugin.extensions.hpe.rev151112.OfjHpeOfMatchUdpSrcRangeGrouping;
...
public class UdpSrcRangeConvertor extends HpeAbstractConvertor<OfjHpeOfMatchUdpSrcRangeGrouping> {
    public UdpSrcRangeConvertor() {
        super(HpeUdpSrcRangeKey.class, HpeOfUdpSrcRange.class, false, OfjHpeOfMatchUdpSrcRangeGrouping.class);
    }
        ...
        switch (path) {
            case FLOWSSTATISTICSUPDATE_FLOWANDSTATISTICSMAPLIST_MATCH:
                return new ExtensionAugment<>(HpeAugMatchNotifUpdateFlowStats.class,
                        new HpeAugMatchNotifUpdateFlowStatsBuilder().setUdpSrcRangeValues(udpSrcRangeValues).build(), this.key);
            case PACKETRECEIVED_MATCH:
                return new ExtensionAugment<>(HpeAugMatchNotifPacketIn.class,
                        new HpeAugMatchNotifPacketInBuilder().setUdpSrcRangeValues(udpSrcRangeValues).build(), this.key);
            case SWITCHFLOWREMOVED_MATCH:
                return new ExtensionAugment<>(HpeAugMatchNotifSwitchFlowRemoved.class,
                        new HpeAugMatchNotifSwitchFlowRemovedBuilder().setUdpSrcRangeValues(udpSrcRangeValues).build(), this.key);
            default:
    ...
    protected void convertToOFJava(Extension extension, OfjAugHpeMatchBuilder ofjAugHpeMatchBuilder) {
        OfjHpeOfMatchUdpSrcRangeGrouping udpSrcRangeGrouping = getExtension(extension);
        UdpSrcRangeValues udpSrcRangeValues = udpSrcRangeGrouping.getUdpSrcRangeValues();
        UdpSrcRangeValuesBuilder udpSrcRangeValuesBuilder = new UdpSrcRangeValuesBuilder();
        udpSrcRangeValuesBuilder.setBegin(udpSrcRangeValues.getBegin());
        udpSrcRangeValuesBuilder.setEnd(udpSrcRangeValues.getEnd());
