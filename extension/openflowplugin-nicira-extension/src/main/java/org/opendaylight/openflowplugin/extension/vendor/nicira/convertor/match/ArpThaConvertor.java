...
import org.opendaylight.yang.gen.v1.urn.opendaylight.openflow.extension.nicira.match.rev140421.ofj.nxm.nx.match.arp.tha.grouping.ArpThaValues;
...
public class ArpThaConvertor implements ConvertorToOFJava<MatchEntries>, ConvertorFromOFJava<MatchEntries, MatchPath> {
    ...
    public ExtensionAugment<? extends Augmentation<Extension>> convert(MatchEntries input, MatchPath path) {
        ...
        return resolveAugmentation(values, path, NxmNxArpThaKey.class);
    }

    private static ExtensionAugment<? extends Augmentation<Extension>> resolveAugmentation(ArpThaValues values,
            MatchPath path, Class<? extends ExtensionKey> key) {
        switch (path) {
        case FLOWSSTATISTICSUPDATE_FLOWANDSTATISTICSMAPLIST_MATCH:
            return new ExtensionAugment<>(NxAugMatchNotifUpdateFlowStats.class,
                    new NxAugMatchNotifUpdateFlowStatsBuilder().setArpThaValues(values).build(), key);
        case PACKETRECEIVED_MATCH:
            return new ExtensionAugment<>(NxAugMatchNotifPacketIn.class, new NxAugMatchNotifPacketInBuilder()
                    .setArpThaValues(values).build(), key);
        case SWITCHFLOWREMOVED_MATCH:
            return new ExtensionAugment<>(NxAugMatchNotifSwitchFlowRemoved.class,
                    new NxAugMatchNotifSwitchFlowRemovedBuilder().setArpThaValues(values).build(), key);
        default:
    ...
    public MatchEntries convert(Extension extension) {
        Optional<OfjNxmNxMatchArpThaGrouping> matchGrouping = MatchUtil.arpThaResolver.getExtension(extension);
        if (!matchGrouping.isPresent()) {
            throw new CodecPreconditionException(extension);
        }
        OfjAugNxMatchBuilder augNxMatchBuilder = new OfjAugNxMatchBuilder().setArpThaValues(matchGrouping.get()
                .getArpThaValues());
        return MatchUtil.createNiciraMatchEntries(Nxm1Class.class, NxmNxArpTha.class, false, augNxMatchBuilder.build());
    }
