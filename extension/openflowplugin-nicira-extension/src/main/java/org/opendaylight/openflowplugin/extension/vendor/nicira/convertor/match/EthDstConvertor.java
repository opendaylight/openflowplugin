...
import org.opendaylight.yang.gen.v1.urn.opendaylight.openflow.extension.nicira.match.rev140421.ofj.nxm.of.match.eth.dst.grouping.EthDstValues;
...
public class EthDstConvertor implements ConvertorToOFJava<MatchEntries>, ConvertorFromOFJava<MatchEntries, MatchPath> {
    ...
    public ExtensionAugment<? extends Augmentation<Extension>> convert(MatchEntries input, MatchPath path) {
        ...
        return resolveAugmentation(values, path, NxmOfEthDstKey.class);
    }

    private static ExtensionAugment<? extends Augmentation<Extension>> resolveAugmentation(EthDstValues values,
            MatchPath path, Class<? extends ExtensionKey> key) {
        switch (path) {
        case FLOWSSTATISTICSUPDATE_FLOWANDSTATISTICSMAPLIST_MATCH:
            return new ExtensionAugment<>(NxAugMatchNotifUpdateFlowStats.class,
                    new NxAugMatchNotifUpdateFlowStatsBuilder().setEthDstValues(values).build(), key);
        case PACKETRECEIVED_MATCH:
            return new ExtensionAugment<>(NxAugMatchNotifPacketIn.class, new NxAugMatchNotifPacketInBuilder()
                    .setEthDstValues(values).build(), key);
        case SWITCHFLOWREMOVED_MATCH:
            return new ExtensionAugment<>(NxAugMatchNotifSwitchFlowRemoved.class,
                    new NxAugMatchNotifSwitchFlowRemovedBuilder().setEthDstValues(values).build(), key);
        default:
    ...
    public MatchEntries convert(Extension extension) {
        Optional<OfjNxmOfMatchEthDstGrouping> matchGrouping = MatchUtil.ethDstResolver.getExtension(extension);
        if (!matchGrouping.isPresent()) {
            throw new CodecPreconditionException(extension);
        }
        OfjAugNxMatchBuilder augNxMatchBuilder = new OfjAugNxMatchBuilder().setEthDstValues(matchGrouping.get()
                .getEthDstValues());
        return MatchUtil.createNiciraMatchEntries(Nxm1Class.class, NxmOfEthDst.class, false, augNxMatchBuilder.build());
    }
