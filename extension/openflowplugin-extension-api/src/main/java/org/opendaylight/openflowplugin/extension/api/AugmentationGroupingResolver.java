...
public final class AugmentationGroupingResolver<G extends DataObject, T extends Augmentable<T>> implements Immutable {
    ...
    public Optional<G> findExtension(final T data) {
        requireNonNull(data);
    ...
    public static <G extends DataObject, T extends Augmentable<T>> Builder<G, T> builder(final Class<G> groupingClass) {
        return new Builder<>(groupingClass);
    }

    @SuppressWarnings("unchecked")
    public static <T extends Augmentable<T>> Factory<T> factory(final Class<T> augmentableClass,
        ...
        final Object[] array = augmentationClasses.toArray().clone();

        // Defensive check of all array elements
        for (Object obj : array) {
            checkArgument(obj instanceof Class, "Element %s is not a Class", obj);
            checkArgument(Augmentation.class.isAssignableFrom((Class<?>) obj), "Class %s is not an Augmentation", obj);
        }
    ...
    public static final class Builder<G extends DataObject, T extends Augmentable<T>>
        ...
        public <X extends Augmentation<T>> Builder<G, T> addAugmentationClass(final Class<X> augmentationClass) {
        ...
        public AugmentationGroupingResolver<G, T> build() {
            return new AugmentationGroupingResolver<>(grouping,
                    (Class<? extends Augmentation<T>>[]) augmentations.toArray());
        }
    }
    ...
    public static final class Factory<T extends Augmentable<T>> implements Immutable {
        ...
        public <G extends DataObject> AugmentationGroupingResolver<G, T> createResolver(final Class<G> groupingClass) {
            for (Class<? extends Augmentation<T>> cls : augmentations) {
