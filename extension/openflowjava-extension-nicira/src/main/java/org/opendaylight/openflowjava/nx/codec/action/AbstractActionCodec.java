...
public abstract class AbstractActionCodec implements OFSerializer<Action>, OFDeserializer<Action> {
    ...
    protected void serializeHeader(int msgLength, int subtype, ByteBuf outBuffer) {
        outBuffer.writeShort(EncodeConstants.EXPERIMENTER_VALUE);
        writeMsgLengthVendorIdSubtypeToBuffer(msgLength, subtype, outBuffer);
    }

    private void writeMsgLengthVendorIdSubtypeToBuffer(int msgLength, int subtype, ByteBuf outBuffer) {
    ...
    protected static ExperimenterId getExperimenterId(){
