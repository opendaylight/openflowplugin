...
public class ConntrackCodec extends AbstractActionCodec {
    ...
    public static final int NX_NAT_RANGE_IPV4_MIN  = 1 << 0;
    public static final int NX_NAT_RANGE_IPV4_MAX  = 1 << 1;
    public static final int NX_NAT_RANGE_IPV6_MIN  = 1 << 2;
    public static final int NX_NAT_RANGE_IPV6_MAX  = 1 << 3;
    public static final int NX_NAT_RANGE_PROTO_MIN = 1 << 4;
    public static final int NX_NAT_RANGE_PROTO_MAX = 1 << 5;
    ...
    private int getActionLength(ActionConntrack action) {
        ...
        for(CtActions ctActions : ctActionsList){
            if(ctActions.getOfpactActions() instanceof NxActionNatCase){
                ...
                if(0 != (rangePresent & NX_NAT_RANGE_IPV4_MIN)) {
                    length += INT_LENGTH;
                }
                if(0 != (rangePresent & NX_NAT_RANGE_IPV4_MAX)) {
                    length += INT_LENGTH;
                }
                if(0 != (rangePresent & NX_NAT_RANGE_PROTO_MIN)) {
                    length += SHORT_LENGTH;
                }
                if(0 != (rangePresent & NX_NAT_RANGE_PROTO_MAX)) {
                    length += SHORT_LENGTH;
                }
            }
        }
