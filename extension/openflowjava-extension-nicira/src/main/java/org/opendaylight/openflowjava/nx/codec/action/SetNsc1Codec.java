 ...
 */
public class SetNsc1Codec extends AbstractActionCodec {

    private static final Logger LOG = LoggerFactory.getLogger(SetNsc1Codec.class);

    public static final int LENGTH = 16;
    public static final byte NXAST_SET_NSC_SUBTYPE = 34;
    public static final NiciraActionSerializerKey SERIALIZER_KEY =
            new NiciraActionSerializerKey(EncodeConstants.OF13_VERSION_ID, ActionSetNsc1.class);
    ...
    public void serialize(Action input, ByteBuf outBuffer) {
        ActionSetNsc1 action = ((ActionSetNsc1) input.getActionChoice());
        serializeHeader(LENGTH, NXAST_SET_NSC_SUBTYPE, outBuffer);
        outBuffer.writeZero(padding);
        outBuffer.writeInt(action.getNxActionSetNsc1().getNsc().intValue());
    }
    ...
    public Action deserialize(ByteBuf message) {
        ...
        NxActionSetNsc1Builder nxActionSetNspBuilder = new NxActionSetNsc1Builder();
        message.skipBytes(padding);
        nxActionSetNspBuilder.setNsc(message.readUnsignedInt());
        builder.setNxActionSetNsc1(nxActionSetNspBuilder.build());
