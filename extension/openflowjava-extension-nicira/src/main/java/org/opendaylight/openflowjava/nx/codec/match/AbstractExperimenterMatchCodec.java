...
public abstract class AbstractExperimenterMatchCodec extends AbstractMatchCodec {
    ...
    protected NxmHeader buildHeader(boolean hasMask) {
        return new NxmHeader(
                getNxmFieldCode(),
                hasMask,
                hasMask ? getValueLengthWithMask() : getValueLength(),
                getExperimenterId());
    }
    ...
    public final int getValueLength() {
        return EncodeConstants.SIZE_OF_INT_IN_BYTES + getExperimenterValueLength();
    }

    @Override
    public final int getValueLengthWithMask() {
        return getValueLength() + getExperimenterValueLength();
    }

    protected abstract long getExperimenterId();

    protected abstract int getExperimenterValueLength();
