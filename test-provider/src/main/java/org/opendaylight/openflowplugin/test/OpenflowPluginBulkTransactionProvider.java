...
import org.opendaylight.openflowplugin.test.OpenflowpluginTestCommandProvider.FlowEventListener;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.flow.service.rev130819.node.error.notification.object.reference.FlowRef;
...
//	private Node testNode1;
	...
	public void onSessionInitiated(ProviderContext session) {
		...
		listener1Reg = notificationService
				.registerNotificationListener(flowEventListener);
	...
	final class FlowEventListener implements SalFlowListener {
		...
		public void onFlowAdded(FlowAdded notification) {
			LOG.info("flow to be added.........................."
					+ notification.toString());
			LOG.info("added flow Xid........................."
					+ notification.getTransactionId().getValue());
			LOG.info("-----------------------------------------------------------------------------------");
			System.out.println(notification.getFlowRef().getValue().toString());
			System.out.println(notification.getFlowRef().getValue()
					.firstKeyOf(Node.class, NodeKey.class).getId().getValue()
					.toString());
			System.out.println("flow to be added.........................."
					+ notification.toString());
			System.out.println("added flow Xid........................."
					+ notification.getTransactionId().getValue());
			System.out
					.println("-----------------------------------------------------------------------------------");
		...
		public void onFlowRemoved(FlowRemoved notification) {
			LOG.debug("removed flow.........................."
					+ notification.toString());
			LOG.debug("remove flow Xid........................."
					+ notification.getTransactionId().getValue());
			LOG.debug("-----------------------------------------------------------------------------------");
			System.out.println("removed flow.........................."
					+ notification.toString());
			System.out.println("remove flow Xid........................."
					+ notification.getTransactionId().getValue());
			// mapTransaction.put(new
			// TransactionKey(notification.getFlowRef().getValue().firstKeyOf(Node.class,NodeKey.class).getId().getValue().toString(),notification.getTransactionId().toString()),notification);
			System.out
					.println("-----------------------------------------------------------------------------------");
		...
		public void onFlowUpdated(FlowUpdated notification) {
			LOG.debug("updated flow.........................."
					+ notification.toString());
			LOG.debug("updated flow Xid........................."
					+ notification.getTransactionId().getValue());
			LOG.debug("-----------------------------------------------------------------------------------");
			System.out.println("updated flow.........................."
					+ notification.toString());
			System.out.println("updated flow Xid........................."
					+ notification.getTransactionId().getValue());
			// mapTransaction.put(new
			// TransactionKey(notification.getFlowRef().getValue().firstKeyOf(Node.class,NodeKey.class).getId().getValue().toString(),notification.getTransactionId().toString()),notification);
			// mapTransaction.size();
		...
		public void onNodeErrorNotification(NodeErrorNotification notification) {
			...
			LOG.debug("notification Begin-Transaction:"
					+ notification.getTransactionUri().toString()
					+ "-----------------------------------------------------------------------------------");
			NodeErrorNotificationBuilder bb = new NodeErrorNotificationBuilder();
			bb.setTransactionId(notification.getTransactionId());
			bb.setObjectReference(notification.getObjectReference());
			bb.setData(notification.getData());
			bb.setTransactionUri(notification.getTransactionUri());
			bb.setType(notification.getType());
		}
	...
	private static NodeRef createNodeRef(String string) {
		...
		InstanceIdentifier<Node> path = InstanceIdentifier.builder()
				.node(Nodes.class).node(Node.class, key).toInstance();
	...
	private FlowBuilder createTestFlow(NodeBuilder nodeBuilder,
			String flowTypeArg, String tableId) {
		...
		case "f31":
			id += 31;
			flow.setMatch(createMatch1().build());
			flow.setInstructions(createAppyActionInstruction23(
					nodeBuilder.getId()).build());
			break;
		case "f32":
			id += 32;
			flow.setMatch(createMatch1().build());
	...
	public void _addGroups(CommandInterpreter ci) {
		...
		switch (count) {
		case 1:
			GroupBuilder group = createTestGroup("a2", "g1", "add", "1");
			GroupBuilder group1 = createTestGroup("a3", "g1", "add", "2");
			writeGroup(ci, group.build(), group1.build());
			break;
		case 2:
			GroupBuilder group2 = createTestGroup("a4", "g1", "add", "5");
			GroupBuilder group3 = createTestGroup("a29", "g1", "add", "6");
	 ...
	 public void _modifyGroups(CommandInterpreter ci) {
			...
			case 1:
				...
				break;
			case 2:
				GroupBuilder group2 = createTestGroup("a6", "g1", "modify", "5");
				GroupBuilder group3 = createTestGroup("a29", "g1", "modify", "6");
				writeGroup(ci, group2.build(), group3.build());
				break;
			}
	...
	private InstanceIdentifier<Node> nodeToInstanceId(Node node) {
		return InstanceIdentifier.builder(Nodes.class)
				.child(Node.class, node.getKey()).toInstance();
	}
...
//    private org.opendaylight.yang.gen.v1.urn.opendaylight.flow.inventory.rev130819.meters.MeterBuilder createTestMeter() {
//        // Sample data , committing to DataStore
//        DataModification modification = dataBrokerService.beginTransaction();
//
//        long id = 12;
//        MeterKey key = new MeterKey(new MeterId(id));
//        org.opendaylight.yang.gen.v1.urn.opendaylight.flow.inventory.rev130819.meters.MeterBuilder meter;
//        meter = new org.opendaylight.yang.gen.v1.urn.opendaylight.flow.inventory.rev130819.meters.MeterBuilder();
...
//        modification.putOperationalData(nodeToInstanceId1(testNode), testNode);
//        modification.putOperationalData(path1, meter);
//        modification.putConfigurationData(nodeToInstanceId1(testNode), testNode);
...
//    private InstanceIdentifier<Node> nodeToInstanceId1(Node node) {
...
//	
	 public void _removeGroups(CommandInterpreter ci){
			...
			case 1:
				...
				break;
			case 2:
				GroupBuilder group2 = createTestGroup("a4", "g1", "remove", "5");
				GroupBuilder group3 = createTestGroup("a29", "g1", "remove", "6");
		...
		private void writeGroup(CommandInterpreter ci, Group group, Group group1) {
			DataModification modification = dataBrokerService.beginTransaction();
			InstanceIdentifier<Group> path1 = InstanceIdentifier
					.builder(Nodes.class).child(Node.class, testNode12.getKey())
					.augmentation(FlowCapableNode.class)
					.child(Group.class, new GroupKey(group.getGroupId())).build();
			modification.putOperationalData(nodeToInstanceId(testNode12),
					testNode12);
			modification.putOperationalData(path1, group);
			modification.putConfigurationData(nodeToInstanceId(testNode12),
					testNode12);
			modification.putConfigurationData(path1, group);

			InstanceIdentifier<Group> path2 = InstanceIdentifier
					.builder(Nodes.class).child(Node.class, testNode12.getKey())
					.augmentation(FlowCapableNode.class)
					.child(Group.class, new GroupKey(group1.getGroupId())).build();
			modification.putOperationalData(nodeToInstanceId(testNode12),
					testNode12);
			modification.putOperationalData(path2, group1);
			modification.putConfigurationData(nodeToInstanceId(testNode12),
					testNode12);
			modification.putConfigurationData(path2, group1);
			Future<RpcResult<TransactionStatus>> commitFuture = modification
					.commit();
			try {
				RpcResult<TransactionStatus> result = commitFuture.get();
				TransactionStatus status = result.getResult();
	...
	private void deleteGroup(CommandInterpreter ci, Group group, Group group1) {
        ...
        InstanceIdentifier<Group> path1 = InstanceIdentifier.builder(Nodes.class).child(Node.class, testNode12.getKey())
        ...
        InstanceIdentifier<Group> path2 = InstanceIdentifier.builder(Nodes.class).child(Node.class, testNode12.getKey())
                .augmentation(FlowCapableNode.class).child(Group.class, new GroupKey(group1.getGroupId())).build();
        // DataObject cls = (DataObject)
        // modification.readConfigurationData(path1);
        // modification.removeOperationalData(nodeToInstanceId(testNode));
	...
	private GroupBuilder createTestGroup(String actiontype, String type,
	...
	private List<Action> createPopVlanAction() {
		...
		action.setAction(new PopVlanActionCaseBuilder().setPopVlanAction(
				vlanAction.build()).build());
	...
	private List<Action> createPushVlanAction() {
		...
		ActionBuilder action = new ActionBuilder();
		action.setAction(new PushVlanActionCaseBuilder().setPushVlanAction(
				vlan.build()).build());
		List<Action> actions = new ArrayList<Action>();
		actions.add(action.build());
		return actions;
	...
	private List<Action> createPushMplsAction() {
		...
		ActionBuilder action = new ActionBuilder();
		action.setAction(new PushMplsActionCaseBuilder().setPushMplsAction(
				push.build()).build());
		List<Action> actions = new ArrayList<Action>();
		actions.add(action.build());
		return actions;
	...
	private List<Action> createPopMplsAction() {
		...
		ActionBuilder action = new ActionBuilder();
		action.setAction(new PopMplsActionCaseBuilder().setPopMplsAction(
				popMplsActionBuilder.build()).build());
		List<Action> actions = new ArrayList<Action>();
		actions.add(action.build());
		return actions;
	}
	...
	private List<Action> createPopPbbAction() {
		...
		ActionBuilder action = new ActionBuilder();
		action.setAction(new PopPbbActionCaseBuilder().setPopPbbAction(
				popPbbActionBuilder.build()).build());
		List<Action> actions = new ArrayList<Action>();
		actions.add(action.build());
		return actions;
	}
	...
	private List<Action> createPushPbbAction() {
		...
		ActionBuilder action = new ActionBuilder();
		action.setAction(new PushPbbActionCaseBuilder().setPushPbbAction(
				pbb.build()).build());
		List<Action> actions = new ArrayList<Action>();
		actions.add(action.build());
		return actions;
	...
	private List<Action> createSetNwTtlAction() {
		...
		ActionBuilder action = new ActionBuilder();
		action.setAction(new SetNwTtlActionCaseBuilder().setSetNwTtlAction(
				setNwTtlActionBuilder.build()).build());
		List<Action> actions = new ArrayList<Action>();
		actions.add(action.build());
		return actions;
	}
	...
	private List<Action> createSetQueueAction() {
		...
		action.setAction(new SetQueueActionCaseBuilder().setSetQueueAction(
				setQueueActionBuilder.build()).build());
		action.setKey(new ActionKey(0));
		List<Action> actions = new ArrayList<Action>();
		actions.add(action.build());
		return actions;
	...
	private List<Action> createCopyTtlInAction() {
		...
		ActionBuilder action = new ActionBuilder();
		action.setAction(new CopyTtlInCaseBuilder().setCopyTtlIn(ttlin.build())
				.build());
		List<Action> actions = new ArrayList<Action>();
		actions.add(action.build());
		return actions;
	...
	private List<Action> createCopyTtlOutAction() {
		...
		ActionBuilder action = new ActionBuilder();
		action.setAction(new CopyTtlOutCaseBuilder().setCopyTtlOut(
				ttlout.build()).build());
		List<Action> actions = new ArrayList<Action>();
		actions.add(action.build());
		return actions;
	...
	private List<Action> createDecMplsTtlAction() {
		...
		ActionBuilder action = new ActionBuilder();
		action.setAction(new DecMplsTtlCaseBuilder()
				.setDecMplsTtl(mpls.build()).build());
		List<Action> actions = new ArrayList<Action>();
		actions.add(action.build());
		return actions;
	}
	...
	private List<Action> createDecNwTtlAction() {
		...
		ActionBuilder action = new ActionBuilder();
		action.setAction(new DecNwTtlCaseBuilder().setDecNwTtl(nwttl.build())
				.build());
		List<Action> actions = new ArrayList<Action>();
	...
	private List<Action> createGroupAction() {
		...
		ActionBuilder action = new ActionBuilder();
		action.setAction(new GroupActionCaseBuilder().setGroupAction(
				groupActionB.build()).build());
		action.setKey(new ActionKey(0));
		List<Action> actions = new ArrayList<Action>();
		actions.add(action.build());
		return actions;
	...
	private static List<Action> createNonAppyPushMplsAction() {
		...
		ab.setAction(new PushMplsActionCaseBuilder().setPushMplsAction(
				push.build()).build());
		actionList.add(ab.build());
		return actionList;
	...
	private static List<Action> createNonAppyPushPbbAction() {
		...
		ab.setAction(new PushPbbActionCaseBuilder().setPushPbbAction(
				pbb.build()).build());
		actionList.add(ab.build());
		return actionList;
	...
	public void _addMDFlows(CommandInterpreter ci) {
		...
		case 2:
			...
			break;
		case 3:
			tf = createTestFlow(tn, "f12", "12");
			tf1 = createTestFlow(tn, "f13", "13");
			tf2 = createTestFlow(tn, "f14", "14");
			tf3 = createTestFlow(tn, "f15", "15");
			break;
		case 4:
			//Add Flows -
			tf = createTestFlow(tn, "f22", "22");
			tf1 = createTestFlow(tn, "f23", "23");
			tf2 = createTestFlow(tn, "f24", "24");
			tf3 = createTestFlow(tn, "f25", "25");
			break;
		case 10:
			tf = createTestFlow(tn, "f3", "3");
			tf1 = createTestFlow(tn, "f4", "4");
		...
		// ci.nextArgument());
		// FlowBuilder tf1 = createTestFlow(tn, "f3", ci.nextArgument());
		// writeFlow(ci, tf, tn);
//		if (flowcnt >= 10) {
//			writeFlow(ci, tf, tf1, tf2, tf3, tn, "update");
//		} else {
...
//		}
	...
	public void _modifyMDFlows(CommandInterpreter ci) {
		// NodeBuilder tn = createTestNode(ci.nextArgument());
		// FlowBuilder tf = createTestFlow(tn, ci.nextArgument(),
		// ci.nextArgument());
	...
	public void _removeMDFlows(CommandInterpreter ci) {
		DataModification<InstanceIdentifier<?>, DataObject> modification = dataBrokerService
				.beginTransaction();
		NodeBuilder tn = createTestNode(ci.nextArgument());
		String flowtype = ci.nextArgument();
		Integer flowcnt = Integer.parseInt(flowtype);
		...
		InstanceIdentifier<Flow> path1 = InstanceIdentifier
				.builder(Nodes.class).child(Node.class, tn.getKey())
				.augmentation(FlowCapableNode.class)
		...
		modification.removeConfigurationData(path1);
		InstanceIdentifier<Flow> path2 = InstanceIdentifier
				.builder(Nodes.class).child(Node.class, tn.getKey())
				.augmentation(FlowCapableNode.class)
				.child(Table.class, new TableKey(tf1.getTableId()))
				.child(Flow.class, tf1.getKey()).build();
		modification.removeOperationalData(path2);
		modification.removeConfigurationData(nodeBuilderToInstanceId(tn));
		modification.removeConfigurationData(path2);

		InstanceIdentifier<Flow> path3 = InstanceIdentifier
				.builder(Nodes.class).child(Node.class, tn.getKey())
				.augmentation(FlowCapableNode.class)
				.child(Table.class, new TableKey(tf2.getTableId()))
				.child(Flow.class, tf2.getKey()).build();
		modification.removeOperationalData(path3);
		modification.removeConfigurationData(nodeBuilderToInstanceId(tn));
		modification.removeConfigurationData(path3);
		InstanceIdentifier<Flow> path4 = InstanceIdentifier
				.builder(Nodes.class).child(Node.class, tn.getKey())
				.augmentation(FlowCapableNode.class)
				.child(Table.class, new TableKey(tf3.getTableId()))
	...
	private void writeFlow(CommandInterpreter ci, FlowBuilder flow,
			FlowBuilder flow1, FlowBuilder flow2, FlowBuilder flow3,
		...
		modification.putOperationalData(nodeBuilderToInstanceId(nodeBuilder),
				nodeBuilder.build());
		modification.putOperationalData(path1, flow.build());
		modification.putConfigurationData(nodeBuilderToInstanceId(nodeBuilder),
				nodeBuilder.build());
		modification.putConfigurationData(path1, flow.build());
		InstanceIdentifier<Flow> path2 = InstanceIdentifier
				.builder(Nodes.class).child(Node.class, nodeBuilder.getKey())
				.augmentation(FlowCapableNode.class)
				.child(Table.class, new TableKey(flow1.getTableId()))
				.child(Flow.class, flow1.getKey()).build();
		modification.putOperationalData(nodeBuilderToInstanceId(nodeBuilder),
				nodeBuilder.build());
		...
		InstanceIdentifier<Flow> path3 = InstanceIdentifier
				.builder(Nodes.class).child(Node.class, nodeBuilder.getKey())
				.augmentation(FlowCapableNode.class)
				.child(Table.class, new TableKey(flow2.getTableId()))
				.child(Flow.class, flow2.getKey()).build();
		modification.putOperationalData(nodeBuilderToInstanceId(nodeBuilder),
				nodeBuilder.build());
		...
		InstanceIdentifier<Flow> path4 = InstanceIdentifier
				.builder(Nodes.class).child(Node.class, nodeBuilder.getKey())
				.augmentation(FlowCapableNode.class)
				.child(Table.class, new TableKey(flow3.getTableId()))
				.child(Flow.class, flow3.getKey()).build();
		modification.putOperationalData(nodeBuilderToInstanceId(nodeBuilder),
				nodeBuilder.build());
	...
	private void writeFlow(CommandInterpreter ci, FlowBuilder flow,
			FlowBuilder flow1, FlowBuilder flow2, FlowBuilder flow3,
			NodeBuilder nodeBuilder, String s) {
		NodeBuilder tn = createTestNode(ci.nextArgument());
		DataModification<InstanceIdentifier<?>, DataObject> modification = dataBrokerService
				.beginTransaction();
		InstanceIdentifier<Flow> path1 = InstanceIdentifier
		...
		// InstanceIdentifier<Flow> path2 =
		// InstanceIdentifier.builder(Nodes.class)
		// .child(Node.class,
		// nodeBuilder.getKey()).augmentation(FlowCapableNode.class)
		// .child(Table.class, new
		// TableKey(flow1.getTableId())).child(Flow.class,
		// flow1.getKey()).build();
		// modification.putOperationalData(nodeBuilderToInstanceId(nodeBuilder),
		// nodeBuilder.build());
		// modification.putOperationalData(path2, flow1.build());
	...
	private static InstructionsBuilder createMetadataInstructions() {
		...
		InstructionBuilder ib = new InstructionBuilder();
		ib.setInstruction(new WriteMetadataCaseBuilder().setWriteMetadata(
				aab.build()).build());
	...
	private static InstructionsBuilder createGotoTableInstructions() {
		...
		InstructionBuilder ib = new InstructionBuilder();
		ib.setInstruction(new GoToTableCaseBuilder().setGoToTable(aab.build())
				.build());
	...
	private static InstructionsBuilder createDropInstructions() {
		...
		ActionBuilder ab = new ActionBuilder();
		ab.setAction(new DropActionCaseBuilder().setDropAction(dropAction)
				.build());
	...
	private static InstructionsBuilder createAppyActionInstruction() {
		...
		ab.setAction(new ControllerActionCaseBuilder().setControllerAction(
	...
	private static InstructionsBuilder createOutputInstructions(
			String outputType, int outputValue) {
		List<Action> actionList = new ArrayList<Action>();
		ActionBuilder ab = new ActionBuilder();
	...
	private static InstructionsBuilder createStripVlanInstructions() {
		...
		ab.setAction(new StripVlanActionCaseBuilder().setStripVlanAction(
				stripActionBuilder.build()).build());
		ab.setOrder(0);
		ab.setKey(new ActionKey(0));
		actionList.add(ab.build());
		// Create an Apply Action
	...
	private static InstructionsBuilder createAppyActionInstruction6() {
		...
		ab.setAction(new SetDlSrcActionCaseBuilder().setSetDlSrcAction(
				src.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction8() {
		...
		pcp.setVlanPcp(pcp1);
		ab.setAction(new SetVlanPcpActionCaseBuilder().setSetVlanPcpAction(
				pcp.build()).build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
	...
	private static InstructionsBuilder createAppyActionInstruction9() {
		...
		CopyTtlInBuilder ttlin = new CopyTtlInBuilder();
		ab.setAction(new CopyTtlInCaseBuilder().setCopyTtlIn(ttlin.build())
				.build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
	...
	private static InstructionsBuilder createAppyActionInstruction10() {
		...
		CopyTtlOutBuilder ttlout = new CopyTtlOutBuilder();
		ab.setAction(new CopyTtlOutCaseBuilder().setCopyTtlOut(ttlout.build())
				.build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
	...
	private static InstructionsBuilder createAppyActionInstruction11() {
		...
		DecMplsTtlBuilder mpls = new DecMplsTtlBuilder();
		ab.setAction(new DecMplsTtlCaseBuilder().setDecMplsTtl(mpls.build())
				.build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
	...
	private static InstructionsBuilder createAppyActionInstruction12() {
		...
		DecNwTtlBuilder nwttl = new DecNwTtlBuilder();
		ab.setAction(new DecNwTtlCaseBuilder().setDecNwTtl(nwttl.build())
				.build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction13() {
		...
		ab.setAction(new DropActionCaseBuilder().setDropAction(drop.build())
				.build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction14() {
		...
		ab.setAction(new FloodActionCaseBuilder().setFloodAction(fld.build())
				.build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
	...
	private static InstructionsBuilder createAppyActionInstruction15() {
		...
		ab.setAction(new FloodAllActionCaseBuilder().setFloodAllAction(
				fldall.build()).build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
	...
	private static InstructionsBuilder createAppyActionInstruction17() {
		...
		ab.setAction(new HwPathActionCaseBuilder().setHwPathAction(
				hwPathB.build()).build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
	...
	private static InstructionsBuilder createAppyActionInstruction18() {
		...
		ab.setAction(new LoopbackActionCaseBuilder().setLoopbackAction(
				loopbackActionBuilder.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction19() {
		...
		PopMplsActionBuilder popMplsActionBuilder = new PopMplsActionBuilder();
		popMplsActionBuilder.setEthernetType(0XB);
		ab.setAction(new PopMplsActionCaseBuilder().setPopMplsAction(
				popMplsActionBuilder.build()).build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
	...
	private static InstructionsBuilder createAppyActionInstruction20() {
		...
		PopPbbActionBuilder popPbbActionBuilder = new PopPbbActionBuilder();
		ab.setAction(new PopPbbActionCaseBuilder().setPopPbbAction(
				popPbbActionBuilder.build()).build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
	...
	private static InstructionsBuilder createAppyActionInstruction21() {
		...
		ab.setAction(new PopVlanActionCaseBuilder().setPopVlanAction(
				popVlanActionBuilder.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction22() {
		...
		ab.setAction(new SetDlTypeActionCaseBuilder().setSetDlTypeAction(
				setDlTypeActionBuilder.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction24() {
		...
		SetMplsTtlActionBuilder setMplsTtlActionBuilder = new SetMplsTtlActionBuilder();
		setMplsTtlActionBuilder.setMplsTtl((short) 0X1);
		ab.setAction(new SetMplsTtlActionCaseBuilder().setSetMplsTtlAction(
				setMplsTtlActionBuilder.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction25() {
		...
		ab.setAction(new SetNextHopActionCaseBuilder().setSetNextHopAction(
				setNextHopActionBuilder.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction26() {
		...
		ab.setAction(new SetNwDstActionCaseBuilder().setSetNwDstAction(
				setNwDstActionBuilder.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction27() {
		...
		ab.setAction(new SetNwSrcActionCaseBuilder().setSetNwSrcAction(
				setNwsrcActionBuilder.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction28() {
		...
		ab.setAction(new SetNwTosActionCaseBuilder().setSetNwTosAction(
				setNwTosActionBuilder.build()).build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
		aab.setAction(actionList);
	...
	private static InstructionsBuilder createAppyActionInstruction29() {
		...
		SetNwTtlActionBuilder setNwTtlActionBuilder = new SetNwTtlActionBuilder();
		setNwTtlActionBuilder.setNwTtl((short) 1);
		ab.setAction(new SetNwTtlActionCaseBuilder().setSetNwTtlAction(
				setNwTtlActionBuilder.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction30() {
		...
		SetQueueActionBuilder setQueueActionBuilder = new SetQueueActionBuilder();
		setQueueActionBuilder.setQueueId(1L);
		ab.setAction(new SetQueueActionCaseBuilder().setSetQueueAction(
				setQueueActionBuilder.build()).build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
	...
	private static InstructionsBuilder createAppyActionInstruction31() {
		...
		ab.setAction(new SetTpDstActionCaseBuilder().setSetTpDstAction(
				setTpDstActionBuilder.build()).build());
		actionList.add(ab.build());
		// Create an Apply Action
		ApplyActionsBuilder aab = new ApplyActionsBuilder();
		aab.setAction(actionList);
	...
	private static InstructionsBuilder createAppyActionInstruction32() {
		...
		ab.setAction(new SetTpSrcActionCaseBuilder().setSetTpSrcAction(
				setTpSrcActionBuilder.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction33() {
		...
		ab.setAction(new SetVlanCfiActionCaseBuilder().setSetVlanCfiAction(
				setVlanCfiActionBuilder.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction34() {
		...
		ab.setAction(new SwPathActionCaseBuilder().setSwPathAction(
				swPathAction.build()).build());
		actionList.add(ab.build());
	...
	private static InstructionsBuilder createAppyActionInstruction35() {
		...
		ab.setAction(new SetFieldCaseBuilder().setSetField(
				setFieldBuilder.build()).build());
		ab.setKey(new ActionKey(0));
		actionList.add(ab.build());

		ab1.setAction(new SetFieldCaseBuilder().setSetField(
				setFieldBuilder1.build()).build());
		ab1.setKey(new ActionKey(1));
		actionList.add(ab1.build());

		ab2.setAction(new SetFieldCaseBuilder().setSetField(
				setFieldBuilder2.build()).build());
	...
	private static InstructionsBuilder createAppyActionInstruction36() {
		...
		vlanBuilder1.setVlanId(vlanIdBuilder.setVlanId(vlanId)
				.setVlanIdPresent(true).build());
	...
	private static InstructionsBuilder createAppyActionInstruction43() {
		...
		ab3.setAction(new SetFieldCaseBuilder().setSetField(
				setFieldBuilder3.build()).build());
		ab3.setKey(new ActionKey(3));
		actionList.add(ab3.build());

		setFieldBuilder4.setLayer3Match(arpmatch4.build());
		ab4.setAction(new SetFieldCaseBuilder().setSetField(
				setFieldBuilder4.build()).build());
	...
	private static InstructionsBuilder createAppyActionInstruction44() {
		...
		ab5.setAction(new SetFieldCaseBuilder().setSetField(
				setFieldBuilder5.build()).build());
		ab5.setKey(new ActionKey(5));
		actionLists.add(ab5.build());

		setFieldBuilder6.setLayer3Match(ipv6Builder6.build());
		ab6.setAction(new SetFieldCaseBuilder().setSetField(
				setFieldBuilder6.build()).build());
	...
	private static InstructionsBuilder createAppyActionInstruction47() {
		...
		// PBB
		ProtocolMatchFieldsBuilder protomatch = new ProtocolMatchFieldsBuilder();
	//	protomatch.setPbb(new PbbBuilder().setPbbIsid(4L)
	//			.setPbbMask(new byte[] { 0, 1, 0, 0 }).build());
		setFieldBuilder.setProtocolMatchFields(protomatch.build());
		ab.setAction(new SetFieldCaseBuilder().setSetField(
				setFieldBuilder.build()).build());
		actionList.add(ab.build());
	...
	private static MatchBuilder createL3IPv6Match() {
		...
		Ipv6Address ndtarget = new Ipv6Address(
				"2001:db8:0:1:fd97:f9f0:a810:782e");
		MacAddress ndsll = new MacAddress("c2:00:54:f5:00:00");
		MacAddress ndtll = new MacAddress("00:0c:29:0e:4c:67");
		Ipv6ExtHeaderBuilder nextheader = new Ipv6ExtHeaderBuilder();
		nextheader.setIpv6Exthdr(58);
	...
	private static MatchBuilder createMetadataMatch() {
		MatchBuilder match = new MatchBuilder();
		byte[] metamask = new byte[] { (byte) -1, (byte) -1, (byte) -1, 0, 0,
				0, (byte) 1, (byte) 1 };
	...
	private static MatchBuilder createTunnelIDMatch() {
		...
		tunnel.setTunnelId(BigInteger.valueOf(10668));
		byte[] mask = new byte[] { (byte) -1, (byte) -1, (byte) -1, 0, 0, 0,
				(byte) 1, (byte) 1 };
