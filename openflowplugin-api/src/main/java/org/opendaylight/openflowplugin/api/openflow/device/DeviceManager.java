...
public interface DeviceManager extends
    ...
    CheckedFuture<Void, TransactionCommitFailedException> removeDeviceFromOperationalDS(@Nonnull KeyedInstanceIdentifier<Node, NodeKey> ii);

    DeviceContext createContext(@Nonnull ConnectionContext connectionContext);


    void sendNodeAddedNotification(@Nonnull KeyedInstanceIdentifier<Node, NodeKey> instanceIdentifier);

    void sendNodeRemovedNotification(@Nonnull KeyedInstanceIdentifier<Node, NodeKey> instanceIdentifier);
}
