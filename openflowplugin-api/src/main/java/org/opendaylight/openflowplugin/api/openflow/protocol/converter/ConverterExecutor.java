...
public interface ConverterExecutor {
     ...
     */
    <FROM, TO, DATA extends ConverterData> Optional<TO> convert(final FROM source, final DATA data);
     ...
     * @return the result (can be empty, if no converter was found)
     */
    <FROM, TO, DATA extends ConverterData> Optional<TO> convert(final Collection<FROM> source, final DATA data);
}