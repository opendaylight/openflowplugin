...
final class GroupService<I extends Group, O extends DataObject> extends AbstractSimpleService<I, O> {
    ...
    protected OfHeader buildRequest(final Xid xid, final I input) {
        ...
        final Optional<GroupModInputBuilder> ofGroupModInput =
                ConvertorManager.getInstance().convert(input, data);

        if (ofGroupModInput.isPresent()) {
            final GroupModInputBuilder groupModInputBuilder = ofGroupModInput.get();
            groupModInputBuilder.setXid(xid.getValue());
