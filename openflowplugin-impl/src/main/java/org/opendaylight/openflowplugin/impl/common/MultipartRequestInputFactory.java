...
public final class MultipartRequestInputFactory {
    ...
    public static MultipartRequestInput makeMultipartRequestInput(final long xid, final short ofVersion,
        ...
        validationOfMultipartTypeAndRequestBody(type, body);
    ...
    private static void validationOfMultipartTypeAndRequestBody(@CheckForNull final MultipartType type,
        ...
        switch (type) {
        case OFPMPDESC:
            Preconditions.checkArgument(body instanceof MultipartRequestDescCase);              return;
        case OFPMPFLOW:
            Preconditions.checkArgument(body instanceof MultipartRequestFlowCase);              return;
        case OFPMPAGGREGATE:
            Preconditions.checkArgument(body instanceof MultipartRequestAggregateCase);         return;
        case OFPMPTABLE:
            Preconditions.checkArgument(body instanceof MultipartRequestTableCase);             return;
        case OFPMPPORTSTATS:
            Preconditions.checkArgument(body instanceof MultipartRequestPortStatsCase);         return;
        case OFPMPQUEUE:
            Preconditions.checkArgument(body instanceof MultipartRequestQueueCase);             return;
        case OFPMPGROUP:
            Preconditions.checkArgument(body instanceof MultipartRequestGroupCase);             return;
        case OFPMPGROUPDESC:
            Preconditions.checkArgument(body instanceof MultipartRequestGroupDescCase);         return;
        case OFPMPGROUPFEATURES:
            Preconditions.checkArgument(body instanceof MultipartRequestGroupFeaturesCase);     return;
        case OFPMPMETER:
            Preconditions.checkArgument(body instanceof MultipartRequestMeterCase);             return;
        case OFPMPMETERCONFIG:
            Preconditions.checkArgument(body instanceof MultipartRequestMeterConfigCase);       return;
        case OFPMPMETERFEATURES:
            Preconditions.checkArgument(body instanceof MultipartRequestMeterFeaturesCase);     return;
        case OFPMPTABLEFEATURES:
            Preconditions.checkArgument(body instanceof MultipartRequestTableFeaturesCase);     return;
        case OFPMPPORTDESC:
            Preconditions.checkArgument(body instanceof MultipartRequestPortDescCase);          return;
        case OFPMPEXPERIMENTER:
            Preconditions.checkArgument(body instanceof MultipartRequestExperimenterCase);      return;
        default:
            throw new IllegalArgumentException("Unknown MultipartType " + type);
        }
    }
