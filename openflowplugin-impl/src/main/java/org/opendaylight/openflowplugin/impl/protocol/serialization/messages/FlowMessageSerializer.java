...
public class FlowMessageSerializer extends AbstractMessageSerializer<FlowMessage> implements SerializerRegistryInjector {
    ...
    private void writeInstructions(final FlowMessage message, final ByteBuf outBuffer) {
        ...
        Optional.ofNullable(message.getInstructions()).flatMap(is -> Optional.ofNullable(is.getInstruction()))
                .ifPresent(is -> is
                        ...
                        .map(i -> protocol.flatMap(p -> ApplyActionsCase.class.isInstance(i)
                                ? Optional.ofNullable(ApplyActionsCase.class.cast(i).getApplyActions())
                                .flatMap(aa -> Optional.ofNullable(aa.getAction()))
                                .map(as -> (Instruction) new ApplyActionsCaseBuilder()
                                        .setApplyActions(new ApplyActionsBuilder()
                                                .setAction(as.stream()
                                                        .map(a -> updateSetTpActions(a, p))
    ...
    private static Action updateSetTpActions(Action action, Short protocol) {
        if (SetTpSrcActionCase.class.isInstance(action.getAction())) {
            ...
            if (actionCase.getSetTpSrcAction().getIpProtocol() > 0) {
            ...
            if (actionCase.getSetTpDstAction().getIpProtocol() > 0) {
                return new ActionBuilder(action)
