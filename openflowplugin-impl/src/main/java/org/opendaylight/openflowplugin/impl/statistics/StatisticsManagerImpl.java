...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.openflowplugin.sm.control.rev150812.SetStatisticsTimeInput;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.openflowplugin.sm.control.rev150812.GetStatisticsTimeOutput;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.openflowplugin.sm.control.rev150812.GetStatisticsTimeOutputBuilder;
...
public class StatisticsManagerImpl implements StatisticsManager, StatisticsManagerControlService {
    ...
    @Override
    public Future<RpcResult<GetStatisticsTimeOutput>> getStatisticsTime() {
      RpcResultBuilder<GetStatisticsTimeOutput> rpcResultBuilder = RpcResultBuilder.success();
      GetStatisticsTimeOutputBuilder getStatisticsTimeOutputBuilder = new GetStatisticsTimeOutputBuilder();
      getStatisticsTimeOutputBuilder.setGetTime(basicTimerDelay);
      rpcResultBuilder.withResult(getStatisticsTimeOutputBuilder.build());
      return Futures.immediateFuture(rpcResultBuilder.build());
    }
    
    @Override
    public Future<RpcResult<Void>> setStatisticsTime(SetStatisticsTimeInput input) {
        basicTimerDelay = input.getSetTime();
        final Future<RpcResult<Void>> result;
        // iterate through stats-ctx: propagate mode
      for (Map.Entry<DeviceInfo, StatisticsContext> entry : contexts.entrySet()) {
          final DeviceInfo deviceInfo = entry.getKey();
