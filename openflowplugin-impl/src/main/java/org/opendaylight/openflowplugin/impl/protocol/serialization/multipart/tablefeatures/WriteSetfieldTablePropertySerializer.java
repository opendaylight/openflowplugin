...
public class WriteSetfieldTablePropertySerializer extends AbstractTablePropertySerializer<WriteSetfield> {
    private SerializerLookup registry;

    public WriteSetfieldTablePropertySerializer(final SerializerLookup registry) {
        registry = requireNonNull(registry);
    }

    @Override
    protected void serializeProperty(final WriteSetfield property, final ByteBuf byteBuf) {
        property.getWriteSetfield()
