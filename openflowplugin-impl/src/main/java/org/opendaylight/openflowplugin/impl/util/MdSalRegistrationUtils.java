...
public class MdSalRegistrationUtils {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(MdSalRegistrationUtils.class);
    ...
    public static void registerMasterServices(@CheckForNull final RpcContext rpcContext,
        ...
        LOG.info("Registering Rpc Services for device:{}", deviceContext.getDeviceState().getNodeId());
    ...
    /**
     * Method unregisters all services in first step. So we don't need to call {@link MdSalRegistrationUtils#unregisterServices(RpcContext,DeviceContext)}
     ...
     */
    public static void registerSlaveServices(@CheckForNull final RpcContext rpcContext, @CheckForNull final DeviceContext deviceContext, @CheckForNull final OfpRole newRole) {
        Preconditions.checkArgument(rpcContext != null);
        Preconditions.checkArgument(deviceContext != null);
        Preconditions.checkArgument(newRole != null);
        Verify.verify(OfpRole.BECOMESLAVE.equals(newRole), "Service call with bad Role {} we expect role BECOMESLAVE", newRole);

        unregisterServices(rpcContext, deviceContext);
    }
     ...
     */
    public static void unregisterServices(@CheckForNull final RpcContext rpcContext, @CheckForNull final DeviceContext deviceContext) {
        Preconditions.checkArgument(rpcContext != null);
        Preconditions.checkArgument(deviceContext != null);
        LOG.info("Un-registering Rpc Services for device:{}", deviceContext.getDeviceState().getNodeId());
