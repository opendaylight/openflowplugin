...
public class RoleManagerImpl implements RoleManager, EntityOwnershipListener, ServiceChangeListener {
    ...
    private void changeOwnershipForMainEntity(final EntityOwnershipChange ownershipChange, final RoleContext roleContext) {
    ...
    private void makeDeviceRoleChange(final OfpRole role, final RoleContext roleContext, final boolean init) {
        ...
        Futures.addCallback(roleChangeFuture, new FutureCallback<RpcResult<SetRoleOutput>>() {
            ...
            public void onFailure(final Throwable throwable) {
                LOG.warn("Enable to set role {} on device {}", role, roleContext.getNodeId());
    ...
    private ListenableFuture<RpcResult<SetRoleOutput>> sendRoleChangeToDevice(final OfpRole newRole, final RoleContext roleContext) {
        ...
        if (LifecycleConductor.getInstance().getVersion(roleContext.getNodeId()) < OFConstants.OFP_VERSION_1_3) {
    ...
    private CheckedFuture<Void, TransactionCommitFailedException> removeDeviceFromOperationalDS(final NodeId nodeId) {
