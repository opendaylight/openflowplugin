...
public @interface OSGiConfiguration {
    ...
    // FIXME: min/max
    @AttributeDefinition(description = "Minimum (starting) number of threads in thread pool")
    int thread$_$pool$_$min$_$threads() default 1;

    // FIXME: min/max
    // FIXME: cap at a more reasonable default (or unlimited)
    @AttributeDefinition(description = "Maximum number of threads in thread pool")
    ...
    int thread$_$pool$_$timeout() default 60;
    ...
    // FIXME: min/max
    @AttributeDefinition(description = "Initial delay used in polling the statistics, value is in milliseconds")
    ...
    @AttributeDefinition(description = """
        ...
        milliseconds""")
    int maximum$_$timer$_$delay() default 900_000;
    ...
    // FIXME: max
    @AttributeDefinition(min = "0", description = """
    ...
    int device$_$datastore$_$removal$_$delay() default 500;

    // FIXME: split these out into a separate property namespace
    /*
     ...
     */
    @AttributeDefinition(description = "Periodic interval for sending LLDP packet for link discovery.")
    // FIXME: min/max
    int topology$_$lldp$_$interval() default 5_000;

    // FIXME: min/max
    @AttributeDefinition(description = "Timeout duration for LLDP response message")
    int topology$_$lldp$_$expiration$_$interval() default 60_000;
}