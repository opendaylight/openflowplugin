...
public class ApplyActionsTablePropertySerializer extends AbstractTablePropertySerializer<ApplyActions> {
    private final SerializerLookup registry;

    public ApplyActionsTablePropertySerializer(final SerializerLookup registry) {
        this.registry = requireNonNull(registry);
    }

    @Override
    protected void serializeProperty(final ApplyActions property, final ByteBuf byteBuf) {
        property.getApplyActions()
                .nonnullAction()
                .values()
                .stream()
