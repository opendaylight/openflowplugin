...
final class GroupMessageService<O extends DataObject> extends AbstractMessageService<Group, GroupMessageBuilder, O> {
    protected GroupMessageService(final RequestContextStack requestContextStack, final DeviceContext deviceContext, final Class<O> clazz) {
        super(requestContextStack, deviceContext, clazz);
    }

    @Override
    protected OfHeader buildRequest(final Xid xid, final GroupMessageBuilder input) throws ServiceException {
        return input
    ...
    /**
     * Check this service is supported in current OpenGroupPlugin configuration
    ...
    @Override
    public ListenableFuture<RpcResult<O>> processInput(Group input) {
        ...
        return Futures
                .withFallback(handleServiceCall(groupMessageBuilder), t -> RpcResultBuilder.<O>failed().buildFuture());
    }
}
