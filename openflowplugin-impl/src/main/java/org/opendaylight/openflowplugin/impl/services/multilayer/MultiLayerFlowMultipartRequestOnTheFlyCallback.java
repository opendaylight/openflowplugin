...
public class MultiLayerFlowMultipartRequestOnTheFlyCallback<T extends OfHeader> extends AbstractMultipartRequestOnTheFlyCallback<T> {
    ...
    public MultiLayerFlowMultipartRequestOnTheFlyCallback(final RequestContext<List<T>> context, Class<?> requestType,
    ...
    @Override
    protected ListenableFuture<List<? extends DataContainer>> processStatistics(T result) {
        final ListenableFuture<List<? extends DataContainer>> future = Futures.transform(
            StatisticsGatheringUtils.deleteAllKnownFlows(
                ...
                !virgin),
            (Function<Void, List<? extends DataContainer>>) input -> SinglePurposeMultipartReplyTranslator
                .translate(deviceInfo, result, convertorExecutor)
