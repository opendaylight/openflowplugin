...
public class SetNwDstActionSerializer extends AbstractSetFieldActionSerializer {
    ...
    protected SetFieldCase buildAction(Action input) {
        ...
        if (Ipv4Address.class.isInstance(address)) {
            builder.setLayer3Match(new Ipv4MatchBuilder()
                    .setIpv4Destination(new Ipv4Prefix(Ipv4Address.class.cast(address).getValue()))
                    .build());
        } else if (Ipv6Address.class.isInstance(address)) {
            builder.setLayer3Match(new Ipv6MatchBuilder()
                    .setIpv6Destination(new Ipv6Prefix(Ipv6Address.class.cast(address).getValue()))
                    .build());
        }
