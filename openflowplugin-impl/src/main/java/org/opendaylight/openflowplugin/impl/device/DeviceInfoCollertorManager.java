...
/**
 * This manager class contains all method for collecting all needed information from new Connected device
 * for cluster MASTER Node Controller. Cluster SLAVE Node Controller doesn't need to collect any data
 * from Device so it has to jump this initialization phase. {@link OpenFlowPluginProviderImpl} has to set
 * {@link org.opendaylight.openflowplugin.api.openflow.role.RoleManager} for {@link DeviceInitializationPhaseHandler}
 * used as handler.
 */
public class DeviceInfoCollertorManager implements DeviceInitializator, DeviceInitializationPhaseHandler, AutoCloseable {

    private static final Logger LOG = LoggerFactory.getLogger(DeviceInfoCollertorManager.class);

    private DeviceInitializationPhaseHandler handler;

    private final boolean switchFeaturesMandatory;

    /**
     * Initialization method
     * @param switchFeaturesMandatory - configSubsystem value
     */
    public DeviceInfoCollertorManager(final boolean switchFeaturesMandatory) {
        LOG.debug("DeviceInfoCollectorManager start");
    ...
    public void onDeviceContextLevelUp(final DeviceContext deviceContext) {
        LOG.trace("onDeviceContextLevelUp for node {} ", deviceContext.getDeviceState().getNodeId());
        Preconditions.checkArgument(deviceContext != null);
        if (deviceContext.isContextMasterFlagHolder()) {
            LOG.debug("Start collecting Device {} initial information.", deviceContext.getDeviceState().getNodeId());
            populateDevice(deviceContext);
        } else {
            LOG.debug("Jump collecting Device {} initial information. We are SLAVE.", deviceContext.getDeviceState().getNodeId());
            handler.onDeviceContextLevelUp(deviceContext);
        }
    }
    ...
    /**
     * Method prepare all needed information for node and submit to Operation DataStore
     * @param deviceContext
     */
    private void populateDevice(final DeviceContext deviceContext) {
        ...
        if (OFConstants.OFP_VERSION_1_0 == version) {
            final CapabilitiesV10 capabilitiesV10 = deviceState.getFeatures().getCapabilitiesV10();
            ...
            final BigInteger dataPathId = connectionContext.getFeatures().getDatapathId();
    ...
    static ListenableFuture<List<RpcResult<List<MultipartReply>>>> createDeviceFeaturesForOF10(
    ...
    ListenableFuture<List<RpcResult<List<MultipartReply>>>> createDeviceFeaturesForOF13(
            final DeviceContext deviceContext, final DeviceState deviceState) {
        ...
        return Futures.transform(replyDesc,
                new AsyncFunction<RpcResult<List<MultipartReply>>, List<RpcResult<List<MultipartReply>>>>() {
                    ...
                    public ListenableFuture<List<RpcResult<List<MultipartReply>>>> apply(
                        ...
                        if (getSwitchFeaturesMandatory()) {
                            return Futures.allAsList(Arrays.asList(replyMeterFeature, replyGroupFeatures,
                                    replyTableFeatures, replyPortDescription));
                        } else {
                            return Futures.successfulAsList(Arrays.asList(replyMeterFeature, replyGroupFeatures, replyTableFeatures, replyPortDescription));
                        }
                    }
                });
    }

    static ListenableFuture<RpcResult<List<MultipartReply>>> getNodeStaticInfo(final MultipartType type,
    ...
    static void makeEmptyTables(final DeviceContext dContext, final InstanceIdentifier<Node> nodeII,
