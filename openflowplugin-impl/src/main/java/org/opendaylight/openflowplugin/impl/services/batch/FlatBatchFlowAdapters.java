...
import org.opendaylight.yang.gen.v1.urn.opendaylight.flat.batch.service.rev160321.process.flat.batch.input.batch.batch.choice.flat.batch.add.flow._case.FlatBatchAddFlowContainer;
import org.opendaylight.yang.gen.v1.urn.opendaylight.flat.batch.service.rev160321.process.flat.batch.input.batch.batch.choice.flat.batch.remove.flow._case.FlatBatchRemoveFlowContainer;
import org.opendaylight.yang.gen.v1.urn.opendaylight.flat.batch.service.rev160321.process.flat.batch.input.batch.batch.choice.flat.batch.update.flow._case.FlatBatchUpdateFlowContainer;
...
public class FlatBatchFlowAdapters {
     ...
     */
    public static AddFlowsBatchInput adaptFlatBatchAddFlow(final BatchPlanStep<FlatBatchAddFlowCase> planStep, final NodeRef node) {
        final List<BatchAddFlows> batchFlows = new ArrayList<>();
        for (FlatBatchAddFlowCase choice : planStep.getTaskBag()) {
            final FlatBatchAddFlowContainer flatAddBatchFlowContainer = choice.getFlatBatchAddFlowContainer();
            final BatchAddFlows addFlows = new BatchAddFlowsBuilder((Flow) flatAddBatchFlowContainer)
                    .setFlowId(flatAddBatchFlowContainer.getFlowId())
                    .build();
     ...
     */
    public static RemoveFlowsBatchInput adaptFlatBatchRemoveFlow(final BatchPlanStep<FlatBatchRemoveFlowCase> planStep, final NodeRef node) {
        final List<BatchRemoveFlows> batchFlows = new ArrayList<>();
        for (FlatBatchRemoveFlowCase choice : planStep.getTaskBag()) {
            final FlatBatchRemoveFlowContainer flatRemoveBatchFlowContainer = choice.getFlatBatchRemoveFlowContainer();
            final BatchRemoveFlows removeFlows = new BatchRemoveFlowsBuilder((Flow) flatRemoveBatchFlowContainer)
                    .setFlowId(flatRemoveBatchFlowContainer.getFlowId())
                    .build();
     ...
     */
    public static UpdateFlowsBatchInput adaptFlatBatchUpdateFlow(final BatchPlanStep<FlatBatchUpdateFlowCase> planStep, final NodeRef node) {
        final List<BatchUpdateFlows> batchFlows = new ArrayList<>();
        for (FlatBatchUpdateFlowCase choice : planStep.getTaskBag()) {
            final FlatBatchUpdateFlowContainer flatUpdateBatchFlowContainer = choice.getFlatBatchUpdateFlowContainer();
            final BatchUpdateFlows updateFlows = new BatchUpdateFlowsBuilder(flatUpdateBatchFlowContainer)
                    .build();
    ...
    adaptFlowBatchFutureForChain(final RpcResult<ProcessFlatBatchOutput> chainInput,
        ...
        return Futures.transform(JdkFutureAdapters.listenInPoolThread(resultUpdateFlowFuture),
                createBatchFlowChainingFunction(chainInput, currentOffset));
    }
}
