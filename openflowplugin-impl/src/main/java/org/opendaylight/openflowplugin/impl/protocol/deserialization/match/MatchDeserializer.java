...
public class MatchDeserializer implements OFDeserializer<Match>, HeaderDeserializer<Match>,
    ...
    public void deserializeEntry(ByteBuf inBuffer, MatchBuilder builder) {
        ...
        int oxmClass = inBuffer.readUnsignedShort();
        int oxmField = inBuffer.readUnsignedByte() >>> 1;
        ...
        if (oxmClass == EncodeConstants.EXPERIMENTER_VALUE) {
            key.setExperimenterId(inBuffer.readUnsignedInt());
        }

        entryRegistry.get(key).deserializeEntry(inBuffer, builder);
    }
