 ...
 */
package org.opendaylight.openflowplugin.impl.services.callbacks;
...
public abstract class AbstractMultipartRequestCallback<T extends OfHeader> extends AbstractRequestCallback<List<T>> {
    ...
    public void onSuccess(final OfHeader result) {
        ...
        if (!isMultipart(result)) {
            ...
            collector.addMultipartMsg((T) result, isReqMore(result), getEventIdentifier());
        }
    }
     ...
     */
    protected abstract boolean isReqMore(final OfHeader result);
