...
public class Ipv6LabelEntrySerializer extends AbstractMatchEntrySerializer<Ipv6Label, Ipv6FlowLabel> {
    public Ipv6LabelEntrySerializer() {
        super(OxmMatchConstants.IPV6_FLABEL, OxmMatchConstants.OPENFLOW_BASIC_CLASS,
    ...
    @Override
    protected Ipv6FlowLabel extractEntryMask(Ipv6Label entry) {
        return entry.getFlabelMask();
    }

    @Override
    protected void serializeEntry(Ipv6Label entry, Ipv6FlowLabel mask, ByteBuf outBuffer) {
        outBuffer.writeInt(entry.getIpv6Flabel().getValue().intValue());
        if (mask != null) {
            writeMask(ByteUtil.unsignedIntToBytes(mask.getValue()), outBuffer, getValueLength());
        }
    }
}
