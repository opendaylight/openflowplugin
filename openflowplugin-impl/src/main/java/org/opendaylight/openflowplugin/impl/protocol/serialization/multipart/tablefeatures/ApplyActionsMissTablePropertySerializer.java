...
public class ApplyActionsMissTablePropertySerializer extends AbstractTablePropertySerializer<ApplyActionsMiss> {
    private final SerializerLookup registry;

    public ApplyActionsMissTablePropertySerializer(final SerializerLookup registry) {
        this.registry = requireNonNull(registry);
    }

    @Override
    protected void serializeProperty(final ApplyActionsMiss property, final ByteBuf byteBuf) {
        property.getApplyActionsMiss()
                .nonnullAction().values()
                .stream()
                .sorted(OrderComparator.build())
                .map(Action::getAction)
                .forEach(action -> ActionUtil.writeActionHeader(action, EncodeConstants.OF13_VERSION_ID,
                                registry, byteBuf));
    }
