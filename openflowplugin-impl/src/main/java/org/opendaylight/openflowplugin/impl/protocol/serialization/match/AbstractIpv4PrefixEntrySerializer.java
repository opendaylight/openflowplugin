...
public abstract class AbstractIpv4PrefixEntrySerializer extends AbstractMatchEntrySerializer<Ipv4Prefix, Empty> {
    ...
    @Override
    protected final Empty extractEntryMask(Ipv4Prefix entry) {
        return extractMask(entry);
    }

    @Override
    protected final void serializeEntry(Ipv4Prefix entry, Empty mask, ByteBuf outBuffer) {
        writeIpv4Prefix(entry, outBuffer);
    }

    public static final @Nullable Empty extractMask(@NonNull Ipv4Prefix entry) {
