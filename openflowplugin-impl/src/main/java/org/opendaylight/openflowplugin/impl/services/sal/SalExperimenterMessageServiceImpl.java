    ...
    protected OfHeader buildRequest(Xid xid, SendExperimenterInput input) throws ServiceException {
        ...
        if (messageConverter instanceof BundleMessageDataInjector) {
            ...
            if (LOG.isTraceEnabled()) {
                LOG.trace("Flows and groups pushed to the node: {} with transaction id : {} are {}",
                        ((BundleAddMessageSal) input.getExperimenterMessageOfChoice()).getSalAddMessageData()
                                .getBundleInnerMessage(), input.getNode().getValue()
                                .firstKeyOf(Node.class).getId(), xid);
            }
        }
