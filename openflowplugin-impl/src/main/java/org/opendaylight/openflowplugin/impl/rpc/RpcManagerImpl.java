...
public class RpcManagerImpl implements RpcManager {
    ...
    public void close() {
        for (final Iterator<RpcContext> iterator = Iterators.consumingIterator(contexts.values().iterator());
                iterator.hasNext();) {
            iterator.next().stopClusterServices(true);
        }
    }

    @Override
    public void onDeviceContextLevelDown(final DeviceInfo deviceInfo, LifecycleService lifecycleService) {
        Optional.ofNullable(contexts.get(deviceInfo)).ifPresent(context -> Futures.transform(
                context.stopClusterServices(lifecycleService.isConnectionInterrupted()),
                new Function<Void, Void>() {
                    @Nullable
                    @Override
                    public Void apply(@Nullable Void input) {
                        deviceTerminationPhaseHandler.onDeviceContextLevelDown(deviceInfo, lifecycleService);
                        return null;
                    }
                }));
    }
