...
public class TcpSourcePortEntrySerializer extends AbstractPortNumberWithMaskEntrySerilizer {
    ...
    protected Uint16 extractPort(final Layer4Match l4match) {
        PortNumber portNumber = l4match instanceof TcpMatch ? ((TcpMatch) l4match).getTcpSourcePort() : null;
    ...
    protected Layer4Match extractEntry(Match match) {
        ...
        if (l4match instanceof TcpMatch && ((TcpMatch) l4match).getTcpSourcePort() != null) {
            return l4match;
        }
        return null;
    }
    ...
    protected Uint16 extractMask(Layer4Match entry) {
        PortNumber portNumber = entry instanceof TcpMatch ? ((TcpMatch) entry).getTcpSourcePortMask() : null;
        return portNumber == null ? null : portNumber.getValue();
    }
}
