...
public class InstructionsMissTablePropertySerializer extends AbstractTablePropertySerializer<InstructionsMiss> {
    private final SerializerLookup registry;

    public InstructionsMissTablePropertySerializer(final SerializerLookup registry) {
        this.registry = requireNonNull(registry);
    }

    @Override
    protected void serializeProperty(final InstructionsMiss property, final ByteBuf byteBuf) {
        property.getInstructionsMiss()
                .nonnullInstruction()
                .values().stream()
                .sorted(OrderComparator.build())
                .map(Instruction::getInstruction)
                .forEach(instruction -> InstructionUtil.writeInstructionHeader(instruction,
                    EncodeConstants.OF13_VERSION_ID, registry, byteBuf));
    }
