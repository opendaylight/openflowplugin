...
public abstract class AbstractMultipartWriter<T extends DataContainer> {
     ...
     * @return true if we have correct statistics type
     */
    public boolean write(final DataContainer statistics, final boolean withParents) {
        if (getType().isInstance(statistics)) {
            LOG.debug("Writing statistics of type {} for node {}", getType(), getInstanceIdentifier());
