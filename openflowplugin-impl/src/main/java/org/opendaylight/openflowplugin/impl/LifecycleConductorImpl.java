            ...
            Futures.addCallback(deviceFlowRegistryFill, new FutureCallback<List<Optional<FlowCapableNode>>>() {
                ...
                public void onSuccess(@Nullable List<Optional<FlowCapableNode>> result) {
                    if (LOG.isDebugEnabled()) {
                        ...
                        int flowCount = result != null ? result
                                .stream()
                                .filter(Optional::isPresent)
                                .map(Optional::get)
                                .map(Tables::getTable)
                                .map(tables -> tables.stream().map(Table::getFlow).mapToInt(List::size).sum())
                ...
                public void onFailure(Throwable t) {
                    ...
                    if (t instanceof CancellationException || deviceFlowRegistryFill.isCancelled()) {
