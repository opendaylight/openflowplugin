...
final class LifecycleConductorImpl implements LifecycleConductor, RoleChangeListener, DeviceContextChangeListener, ExtensionConverterProviderKeeper {
    ...
    public void roleChangeOnDevice(final DeviceInfo deviceInfo, final boolean success, final OfpRole newRole, final boolean initializationPhase) {
        ...
        final DeviceContext deviceContext = deviceManager.gainContext(deviceInfo);
        final RpcContext rpcContext =  rpcManager.gainContext(deviceInfo);

        if (null == deviceContext) {
            LOG.warn("Something went wrong, device context for nodeId: {} doesn't exists", deviceInfo.getNodeId());
            return;
        }

        if (null == rpcContext) {
            LOG.warn("Something went wrong, rpc context for nodeId: {} doesn't exists", deviceInfo.getNodeId());
            return;
        }
