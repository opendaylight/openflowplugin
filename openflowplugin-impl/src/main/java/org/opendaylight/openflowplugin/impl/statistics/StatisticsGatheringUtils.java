...
public final class StatisticsGatheringUtils {
    ...
    /**
     *
     * @param statisticsGatheringService
     * @param deviceInfo
     * @param type
     * @param txFacade
     * @param registry
            ...
            new AsyncFunction<RpcResult<List<T>>, Boolean>() {
                ...
                public ListenableFuture<Boolean> apply(@Nonnull final RpcResult<List<T>> rpcResult) {
                    ...
                    if (rpcResult.isSuccessful()) {
                        ...
                        if (Objects.nonNull(rpcResult.getResult()) && !rpcResult.getResult().isEmpty()) {
                            ...
                            try {
                                allMultipartData = rpcResult
                                    .getResult()
                                    .stream()
                                    .map(reply ->  SinglePurposeMultipartReplyTranslator
                                        .translate(deviceInfo, reply, convertorExecutor))
                        ...
                        LOG.debug("Stats reply FAILED for node {} of type {}: {}", deviceInfo.getNodeId(), type,
                            rpcResult.getErrors());
    ...
    public static boolean writeStatistics(final MultipartType type,
                                          final List<? extends DataContainer> statistics,
                                          final DeviceInfo deviceInfo,
                                          final StatisticsWriterProvider statisticsWriterProvider) {
        ...
        try {
            statistics.forEach(stat -> {
                statisticsWriterProvider.lookup(type).ifPresent(p -> {
