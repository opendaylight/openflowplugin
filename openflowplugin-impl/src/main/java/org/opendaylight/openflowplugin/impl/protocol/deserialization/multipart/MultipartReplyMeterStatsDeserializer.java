...
public class MultipartReplyMeterStatsDeserializer implements OFDeserializer<MultipartReplyBody> {
    ...
    public MultipartReplyBody deserialize(final ByteBuf message) {
        ...
        while (message.readableBytes() > 0) {
            ...
            final Map<BandStatKey, BandStat> subItems;
            if (itemLength >= METER_BODY_LENGTH + METER_BAND_STATS_LENGTH) {
                final int meterCount = (itemLength - METER_BODY_LENGTH) / METER_BAND_STATS_LENGTH;
                final var builder = BindingMap.<BandStatKey, BandStat>orderedBuilder(meterCount);
                for (int i = 0; i < meterCount; ++i) {
                    builder.add(new BandStatBuilder()
                        .setBandId(new BandId(Uint32.valueOf(i)))
