...
public class ContextChainHolderImpl implements ContextChainHolder {
    ...
    public void onNotAbleToStartMastership(final DeviceInfo deviceInfo, @Nonnull final String reason, final boolean mandatory) {
        ...
        if (mandatory && contextChainMap.containsKey(deviceInfo)) {
            LOG.warn("Master ownership is mandatory, destroying context chain and closing connection.");
    ...
    public void ownershipChanged(EntityOwnershipChange entityOwnershipChange) {
        LOG.info("Ownership change received : {}", entityOwnershipChange);
        if (!entityOwnershipChange.hasOwner() || entityOwnershipChange.inJeopardy()) {
            ...
            if (entityOwnershipChange.inJeopardy()) {
                LOG.warn("Cluster is in jeopardy state and entity {} is disconnected from controller", entityName);
            }
            if (LOG.isDebugEnabled()) {
