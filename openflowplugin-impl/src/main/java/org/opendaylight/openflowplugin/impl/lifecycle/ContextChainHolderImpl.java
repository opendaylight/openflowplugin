...
public class ContextChainHolderImpl implements ContextChainHolder {
    ...
    private final Map<DeviceInfo, ContextChain> withoutRoleChains = Collections.synchronizedMap(new HashMap<>());
    private final Map<DeviceInfo, ContextChain> withoutRoleChainsQueue = Collections.synchronizedMap(new HashMap<>());
    ...
    public ContextChain createContextChain(final ConnectionContext connectionContext) {
        ...
        if (Objects.isNull(eosListenerRegistration)) {
            eosListenerRegistration = Verify.verifyNotNull(entityOwnershipService.registerListener
    ...
    public void ownershipChanged(EntityOwnershipChange entityOwnershipChange) {
        LOG.debug("Ownership change received : {}", entityOwnershipChange);
        if (!entityOwnershipChange.hasOwner()) {
            ...
            if (entityName != null ){
                ...
                for (Map.Entry<DeviceInfo, ContextChain> entry : contextChainMap.entrySet()) {
                    inMap = entry.getKey().getNodeId().equals(nodeId);
                    if (inMap) {
                        break;
                    }                    
                }
