...
//            deviceManager.removeDeviceFromOperationalDS(deviceInfo);
//        } else {
...
//                } else {
                    ...
                    if (chain.isMastered(ContextChainMastershipState.CHECK)) {
                        Futures.transform(chain.connectionDropped(), new Function<Void, Object>() {
                            ...
                            public Object apply(@Nullable Void aVoid) {
                                ...
                                try {
                                    deviceManager
                                            .removeDeviceFromOperationalDS(deviceInfo)
                                            .checkedGet(5L, TimeUnit.SECONDS);
                                } catch (TimeoutException | TransactionCommitFailedException e) {
                                    LOG.warn("Not able to remove device {} from DS, reason: {}",
                                            deviceInfo,
                                            e.getMessage());
                                }
                                return null;
