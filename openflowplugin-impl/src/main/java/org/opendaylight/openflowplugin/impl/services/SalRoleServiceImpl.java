...
public class SalRoleServiceImpl extends AbstractSimpleService<SetRoleInput, SetRoleOutput> implements SalRoleService  {
    ...
    public Future<RpcResult<SetRoleOutput>> setRole(final SetRoleInput input) {
        ...
        ListenableFuture<SetRoleOutput> taskFuture = listeningExecutorService.submit(
                new RoleChangeTask(nodeId, input.getControllerRole(), version, roleService));

        Futures.addCallback(taskFuture, new FutureCallback<SetRoleOutput>() {
            @Override
