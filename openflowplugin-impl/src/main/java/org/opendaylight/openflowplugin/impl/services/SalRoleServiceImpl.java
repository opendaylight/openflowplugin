...
public class SalRoleServiceImpl extends AbstractSimpleService<SetRoleInput, SetRoleOutput> implements SalRoleService  {
    ...
    public Future<RpcResult<SetRoleOutput>> setRole(final SetRoleInput input) {
        ...
        do {
            ...
            try {
                ...
                LOG.debug("Exception in setRole(), will retry:" + (MAX_RETRIES - roleChangeTask.getRetryCounter()) + " times.", e);
            }
    ...
    class RoleChangeTask implements Callable<SetRoleOutput> {
        ...
        public SetRoleOutput call() throws RoleChangeException {
            ...
            try {
                generationId = this.roleService.getGenerationIdFromDevice(version).get(3, TimeUnit.SECONDS);
                LOG.debug("RoleChangeTask, GenerationIdFromDevice from device is {}", generationId);

            } catch (Exception e ) {
                LOG.debug("Exception in getting generationId for device:{}. Ex:{}" + this.nodeId.getValue(), e);
                throw new RoleChangeException("Exception in getting generationId for device:"+ this.nodeId.getValue(), e);
            }


            LOG.debug("GenerationId received from device:{} is {}", nodeId.getValue(), generationId);
            ...
            try {
                ...
                LOG.debug("roleChangeResult after submitRoleChange:{}", roleChangeResult);
