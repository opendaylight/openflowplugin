...
public final class SalRoleServiceImpl extends AbstractSimpleService<SetRoleInput, SetRoleOutput> implements SalRoleService  {
    ...
    private ListenableFuture<RpcResult<SetRoleOutput>> tryToChangeRole(final OfpRole role) {
        ...
        return Futures.transform(JdkFutureAdapters.listenInPoolThread(generationFuture), new AsyncFunction<BigInteger, RpcResult<SetRoleOutput>>() {

            @Override
            public ListenableFuture<RpcResult<SetRoleOutput>> apply(final BigInteger generationId) throws Exception {
