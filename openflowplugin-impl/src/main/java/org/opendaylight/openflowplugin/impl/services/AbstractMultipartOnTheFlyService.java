...
public abstract class AbstractMultipartOnTheFlyService<I, T extends OfHeader> extends AbstractMultipartService<I, T> {
    ...
    protected final FutureCallback<OfHeader> createCallback(final RequestContext<List<T>> context,
        ...
        return canUseSingleLayerSerialization()
            ? new SingleLayerFlowMultipartRequestOnTheFlyCallback<>(context, requestType, getDeviceContext(),
                    getEventIdentifier(), statisticsWriterProvider)
            : new MultiLayerFlowMultipartRequestOnTheFlyCallback<>(context, requestType, getDeviceContext(),
                    getEventIdentifier(), statisticsWriterProvider, converterExecutor);
    }
}
