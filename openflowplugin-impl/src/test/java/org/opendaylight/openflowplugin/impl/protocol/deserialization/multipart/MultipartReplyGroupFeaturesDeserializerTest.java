...
public class MultipartReplyGroupFeaturesDeserializerTest extends AbstractMultipartDeserializerTest {
    ...
    private static final List<Long> ACTIONS_LIST = Arrays.asList(1l, 2l, 3l, 4l);
    ...
    public void testDeserialize() throws Exception {
        ...
        MultipartReplyGroupFeatures reply = (MultipartReplyGroupFeatures) deserializeMultipart(buffer);

        assertEquals(GROUP_TYPES_SUPPORTED.size(), reply.getGroupTypesSupported().size());
        assertEquals("GroupAll", reply.getGroupTypesSupported().get(0).getSimpleName());
        assertEquals("GroupSelect", reply.getGroupTypesSupported().get(1).getSimpleName());
        assertEquals("GroupIndirect", reply.getGroupTypesSupported().get(2).getSimpleName());

        assertEquals(GROUP_CAPABILITIES_SUPPORTED.size(), reply.getGroupCapabilitiesSupported().size());
        assertEquals("Chaining", reply.getGroupCapabilitiesSupported().get(0).getSimpleName());
        assertEquals("ChainingChecks", reply.getGroupCapabilitiesSupported().get(1).getSimpleName());
