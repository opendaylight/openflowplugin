...
public class TableFeaturesMatchFieldDeserializerTest {
    ...
    private void addValues(ByteBuf buffer, int oxmClass, int oxmFiled) {
        buffer.clear();
        buffer.writeShort(oxmClass);
        buffer.writeByte(oxmFiled << 1);
        buffer.writeZero(EncodeConstants.SIZE_OF_SHORT_IN_BYTES);
        buffer.writeByte(EncodeConstants.EMPTY_VALUE);
        buffer.writeZero(EncodeConstants.SIZE_OF_BYTE_IN_BYTES);
    }
}