...
public class SalAsyncConfigRpcsTest extends ServiceMocking {
    ...
    private SalAsyncConfigRpcs salAsyncConfigRpcs;
    private ClassToInstanceMap<Rpc<?, ?>> rpcMap;

    @Override
    public void setup() {
        salAsyncConfigRpcs = new SalAsyncConfigRpcs(
                mockedRequestContextStack, mockedDeviceContext);
        rpcMap = salAsyncConfigRpcs.getRpcClassToInstanceMap();
    }
    ...
    public void testSetAsync() throws Exception {
        ...
        final ListenableFuture<RpcResult<SetAsyncOutput>> setAsyncResult =
                rpcMap.getInstance(SetAsync.class).invoke(new SetAsyncInputBuilder().build());
    ...
    public void testGetAsyncTest() throws Exception {
        ...
        final Future<RpcResult<GetAsyncOutput>> getAsyncResult =
                rpcMap.getInstance(GetAsync.class).invoke(new GetAsyncInputBuilder().build());
