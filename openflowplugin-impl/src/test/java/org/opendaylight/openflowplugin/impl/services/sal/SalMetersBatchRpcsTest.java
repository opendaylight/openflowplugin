...
public class SalMetersBatchRpcsTest {
    ...
    @Before
    public void setUp() {
        salMetersBatchRpcs = new SalMetersBatchRpcs(salMeterRpcs, transactionRpc);

        Mockito.when(transactionRpc.getRpcClassToInstanceMap().getInstance(SendBarrier.class).invoke(any()))
    ...
    public void testUpdateMetersBatch_success() throws Exception {
        Mockito.when(salMeterRpcs.getRpcClassToInstanceMap().getInstance(UpdateMeter.class).invoke(Mockito.any()))
        ...
        final Future<RpcResult<UpdateMetersBatchOutput>> resultFuture = salMetersBatchRpcs.getRpcClassToInstanceMap()
            .getInstance(UpdateMetersBatch.class).invoke(input);

        Assert.assertTrue(resultFuture.isDone());
        Assert.assertTrue(resultFuture.get().isSuccessful());

        final InOrder inOrder = Mockito.inOrder(salMeterRpcs, transactionRpc);
        inOrder.verify(salMeterRpcs, Mockito.times(2)).getRpcClassToInstanceMap()
            .getInstance(UpdateMeter.class).invoke(updateMeterInputCpt.capture());
        ...
        inOrder.verify(transactionRpc).getRpcClassToInstanceMap().getInstance(SendBarrier.class)
    ...
    public void testAddMetersBatch_success() throws Exception {
        Mockito.when(salMeterRpcs.getRpcClassToInstanceMap().getInstance(AddMeter.class).invoke(any()))
        ...
        final Future<RpcResult<AddMetersBatchOutput>> resultFuture = salMetersBatchRpcs.getRpcClassToInstanceMap()
            .getInstance(AddMetersBatch.class).invoke(input);
        ...
        final InOrder inOrder = Mockito.inOrder(salMeterRpcs, transactionRpc);
        inOrder.verify(salMeterRpcs, Mockito.times(2)).getRpcClassToInstanceMap()
            .getInstance(AddMeter.class).invoke(addMeterInputCpt.capture());
    ...
    public void testAddMetersBatch_failure() throws Exception {
        Mockito.when(salMeterRpcs.getRpcClassToInstanceMap().getInstance(AddMeter.class).invoke(Mockito.any()))
    ...
    public void testRemoveMetersBatch_success() throws Exception {
        Mockito.when(salMeterRpcs.getRpcClassToInstanceMap().getInstance(RemoveMeter.class).invoke(any()))
        ...
        final Future<RpcResult<RemoveMetersBatchOutput>> resultFuture = salMetersBatchRpcs.getRpcClassToInstanceMap()
            .getInstance(RemoveMetersBatch.class).invoke(input);
        ...
        inOrder.verify(salMeterRpcs, Mockito.times(2)).getRpcClassToInstanceMap()
            .getInstance(RemoveMeter.class).invoke(removeMeterInputCpt.capture());
    ...
    public void testRemoveMetersBatch_failure() throws Exception {
        Mockito.when(salMeterRpcs.getRpcClassToInstanceMap().getInstance(RemoveMeter.class).invoke(Mockito.any()))
