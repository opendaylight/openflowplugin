...
public class SalBundleRpcsTest {
    ...
    @Before
    public void setUp() {
        rpcs = new SalBundleRpcs(experimenterMessageRpc);
        rpcMap = rpcs.getRpcClassToInstanceMap();
    }
    ...
    public void testControlBundle() {
        ...
        experimenterBuilder.setExperimenterMessageOfChoice(new BundleControlSalBuilder()
                ...
                .build());
        Mockito.when(experimenterMessageRpc.getRpcClassToInstanceMap().getInstance(SendExperimenter.class)
            .invoke(any())).thenReturn(SettableFuture.create());
        rpcMap.getInstance(ControlBundle.class).invoke(input);
        Mockito.verify(experimenterMessageRpc).getRpcClassToInstanceMap().getInstance(SendExperimenter.class)
            .invoke(experimenterBuilder.build());
    }
    ...
    public void testAddBundleMessages() {
        ...
        Mockito.when(experimenterMessageRpc.getRpcClassToInstanceMap().getInstance(SendExperimenter.class)
            .invoke(any())).thenReturn(SettableFuture.create());
        rpcMap.getInstance(AddBundleMessages.class).invoke(input);
        for (Message msg : innerMessages) {
            Mockito.verify(experimenterMessageRpc).getRpcClassToInstanceMap().getInstance(SendExperimenter.class)
                    .invoke(experimenterBuilder.setExperimenterMessageOfChoice(addMessageBuilder.setSalAddMessageData(
