...
public class RoleManagerImplTest {
    ...
    private final EntityOwnershipChange slaveTxEntity = new EntityOwnershipChange(roleManager.makeTxEntity(nodeId), true, false, true);
    ...
    public void testChangeOwnershipForMainEntity2() throws Exception {
        ...
        roleManagerSpy.changeOwnershipForMainEntity(masterEntity, roleContextSpy);
        inOrder.verify(roleContextSpy, Mockito.atLeastOnce()).isMainCandidateRegistered();
        inOrder.verify(roleContextSpy, Mockito.calls(2)).unregisterCandidate(Mockito.<Entity>any());
        inOrder.verify(roleContextSpy, Mockito.atLeastOnce()).close();
    }
