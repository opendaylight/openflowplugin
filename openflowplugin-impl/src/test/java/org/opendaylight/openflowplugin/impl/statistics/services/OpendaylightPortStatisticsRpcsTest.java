...
public class OpendaylightPortStatisticsRpcsTest extends AbstractSingleStatsServiceTest {
    ...
    @Override
    public void setUp() {
        portStatisticsRpcs = new OpendaylightPortStatisticsRpcs(rqContextStack, deviceContext, new AtomicLong(),
            notificationPublishService);

        Mockito.doAnswer(answerVoidToCallback).when(outboundQueueProvider)
                .commitEntry(eq(Uint32.valueOf(42)), requestInput.capture(), any(FutureCallback.class));
        rpcMap = portStatisticsRpcs.getRpcClassToInstanceMap();
    }
    ...
    public void testGetAllNodeConnectorsStatistics() throws Exception {
        ...
        final Future<RpcResult<GetAllNodeConnectorsStatisticsOutput>> resultFuture
                = rpcMap.getInstance(GetAllNodeConnectorsStatistics.class).invoke(input.build());
    ...
    public void testGetNodeConnectorStatistics() throws Exception {
        ...
        final Future<RpcResult<GetNodeConnectorStatisticsOutput>> resultFuture
                = rpcMap.getInstance(GetNodeConnectorStatistics.class).invoke(input.build());
