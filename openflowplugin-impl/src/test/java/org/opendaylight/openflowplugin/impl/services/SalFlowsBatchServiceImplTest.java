...
public class SalFlowsBatchServiceImplTest {
    ...
    @Test
    public void testRemoveFlowsBatch() throws Exception {
        ...
        Assert.assertTrue(result.isBarrierAfterPassed());
        Assert.assertEquals(2, result.getBatchFlowsOutput().size());
        Assert.assertTrue(result.getBatchFlowsOutput().get(0).isSuccess());
        Assert.assertEquals(flow1IdValue, result.getBatchFlowsOutput().get(0).getFlowId().getValue());
        Assert.assertTrue(result.getBatchFlowsOutput().get(1).isSuccess());
        Assert.assertEquals(flow2IdValue, result.getBatchFlowsOutput().get(1).getFlowId().getValue());
    ...
    public void testExtractNodeId() throws Exception {
        Assert.assertEquals(NODE_ID, SalFlowsBatchServiceImpl.extractNodeId(NODE_REF));
    }

    @Test
    public void testAddFlowsBatch() throws Exception {
    ...
    @Test
    public void testUpdateFlowsBatch() throws Exception {
        final UpdateFlowsBatchInput input = new UpdateFlowsBatchInputBuilder()
                .setNode(NODE_REF)
                .setBarrierAfter(true)
                .setBatchUpdateFlows(Lists.newArrayList(
                        createEmptyBatchUpdateFlow("ut-dummy-flow1", 42),
                        createEmptyBatchUpdateFlow("ut-dummy-flow2", 44)))
                .build();
