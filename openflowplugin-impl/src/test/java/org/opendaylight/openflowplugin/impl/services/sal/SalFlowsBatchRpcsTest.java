...
public class SalFlowsBatchRpcsTest {
    ...
    @Before
    public void setUp() {
        salFlowsBatchRpcs = new SalFlowsBatchRpcs(salFlowRpcs, transactionRpc);

        Mockito.when(transactionRpc.getRpcClassToInstanceMap().getInstance(SendBarrier.class)
            .invoke(ArgumentMatchers.any())).thenReturn(RpcResultBuilder.<SendBarrierOutput>success().buildFuture());
        rpcMap = salFlowsBatchRpcs.getRpcClassToInstanceMap();
    }

    @After
    public void tearDown() {
        Mockito.verifyNoMoreInteractions(salFlowRpcs, transactionRpc);
    }

    @Test
    public void testRemoveFlowsBatch_success() throws Exception {
        Mockito.when(salFlowRpcs.getRpcClassToInstanceMap().getInstance(RemoveFlow.class)
        ...
        final Future<RpcResult<RemoveFlowsBatchOutput>> resultFuture =
                rpcMap.getInstance(RemoveFlowsBatch.class).invoke(input);
        ...
        final InOrder inOrder = Mockito.inOrder(salFlowRpcs, transactionRpc);

        inOrder.verify(salFlowRpcs, Mockito.times(2)).getRpcClassToInstanceMap()
            .getInstance(RemoveFlow.class).invoke(removeFlowInputCpt.capture());
    ...
    public void testAddFlowsBatch_success() throws Exception {
        Mockito.when(salFlowRpcs.getRpcClassToInstanceMap().getInstance(AddFlow.class).invoke(ArgumentMatchers.any()))
        ...
        final Future<RpcResult<AddFlowsBatchOutput>> resultFuture =
                rpcMap.getInstance(AddFlowsBatch.class).invoke(input);
        ...
        inOrder.verify(salFlowRpcs, Mockito.times(2)).getRpcClassToInstanceMap()
            .getInstance(AddFlow.class).invoke(addFlowInputCpt.capture());
    ...
    public void testUpdateFlowsBatch_success() throws Exception {
        Mockito.when(salFlowRpcs.getRpcClassToInstanceMap().getInstance(UpdateFlow.class)
                .invoke(ArgumentMatchers.any()))
        ...
        final Future<RpcResult<UpdateFlowsBatchOutput>> resultFuture =
                rpcMap.getInstance(UpdateFlowsBatch.class).invoke(input);
        ...
        inOrder.verify(salFlowRpcs, Mockito.times(2)).getRpcClassToInstanceMap()
            .getInstance(UpdateFlow.class).invoke(updateFlowInputCpt.capture());
