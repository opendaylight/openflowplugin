...
public class SalPortRpcTest extends ServiceMocking {
    ...
    SalPortRpc salPortRpc;

    @Override
    protected void setup() {
        final ConvertorManager convertorManager = ConvertorManagerFactory.createDefaultManager();
        salPortRpc = new SalPortRpc(mockedRequestContextStack, mockedDeviceContext, convertorManager);
    }

    @Test
    public void testUpdatePort() {
        salPortRpc.getRpcClassToInstanceMap().getInstance(UpdatePort.class).invoke(dummyUpdatePortInput());
        verify(mockedRequestContextStack).createRequestContext();
    }

    @Test
    public void testBuildRequest() {
        final OfHeader ofHeader = salPortRpc.buildRequest(new Xid(DUMMY_XID), dummyUpdatePortInput());
    }
